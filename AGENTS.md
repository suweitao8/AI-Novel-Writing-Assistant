...
- The browser smoke self-test for UI changes runs against the local dev services of the checkout being developed — the main lane (`http://127.0.0.1:3100`, `http://127.0.0.1:5174`) when working in the main workspace, or the worktree's own lane ports (printed by `pnpm workflow:worktree` and stored in its `server/.env`) when working in a worktree — in ZCode's built-in browser (the Browser Use `iab` backend). The built-in browser is the default for all browser-based development verification: its tabs are isolated from the user's windows, it covers navigation, DOM snapshot, clicks, form input, select/range interactions, `evaluate`, console-error capture, and screenshots (including WebGL canvas pages). Do not open external Chrome, the user's browser, or a separate Playwright browser for smoke tests. Walk the primary user path of the changed page, verify rendering and key interactions with no console errors, and capture key screenshots as evidence. If a required capability is genuinely missing from the built-in browser (for example file-chooser upload flows), state the exact gap and the fallback used instead of silently switching browsers.
...
- Prefer `pnpm workflow:worktree <task>` for new work. It requires a clean `main`, creates a sibling `codex/<task>` worktree, provisions the worktree's isolated dev-port lane into its `server/.env`, and keeps `core.hooksPath` owned by the main workspace (hooks are shared repo-wide; the worktree must not hijack them).
...
- Development ports are per-checkout lanes so parallel sessions never fight over one pair:
  - The main workspace is the user-facing lane and stays fixed: API server on `3100` (single source of truth: `server/.env` `PORT=3100`), web client on `5174` (`client/vite.config.ts` reads `CLIENT_PORT` from `server/.env`, default `5174`; 5173 is permanently occupied on this machine by another long-lived Docker service). Do not change these two values in the main workspace.
  - Each `codex/*` worktree gets its own isolated lane: `pnpm workflow:worktree` deterministically derives an API port in `3101-3199` and a client port in `5180-5379` from the worktree path (collision-probed at creation) and writes both into the worktree's `server/.env` as `PORT=` and `CLIENT_PORT=`. The client dev proxy reads `PORT` from `server/.env` automatically, so the lane is self-consistent.
- Do not hardcode a lane port anywhere else; always read it from the checkout's `server/.env`. Worktree smoke tests must use their own lane ports (printed by `pnpm workflow:worktree`) and must never stop or occupy the main lane (`3100`/`5174`), which belongs to the user's interactive session.
- Never change `PORT`/`CLIENT_PORT` values as a conflict workaround after the lane is provisioned — silent port drift is what breaks the client `/api` proxy and running sessions. If a lane port is occupied at dev start, find the owner via `netstat -ano | findstr :<port>` plus the process command line: a process from another checkout must be left alone, and only a stale process of this same checkout may be stopped before restarting on the same port.
- If a port is held by an unrelated long-lived service rather than a stale dev process, report it to the user instead of killing blindly or switching ports.
- `pnpm dev:raw` runs `scripts/dev-service-supervisor.cjs`: a failed child is restarted with bounded exponential backoff, but once a child exhausts retries the supervisor terminates the whole development group and does not restart the terminated siblings. This prevents a surviving Vite process from presenting an endless service-connection screen. On startup in the main workspace the supervisor also repairs `core.hooksPath` back to the main checkout's `.githooks`; worktree dev starts never touch the shared hooks path.
- When the page remains on “正在连接本地创作服务”, first identify which checkout is serving the port (`netstat -ano` for the PID, then the process command line): a worktree checkout serving a shared port cannot see generated images stored under the main workspace (`server/storage/`), which surfaces as data loading but images 404. Check `Get-NetTCPConnection -State Listen -LocalPort <lane ports>`, request `http://127.0.0.1:<api port>/api/health`, and inspect the latest `.logs/*/*-dev.log`; do not use Prisma `--accept-data-loss` as a recovery shortcut.
