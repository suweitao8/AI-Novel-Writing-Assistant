...
- 模型 3D 预览新增三套固定 HDRI 环境：室内客厅、中央广场和草地自然，可在预览环境中直接切换；半球直径可在 5–30 米之间调节。
- 系统设置提供「通用资产」入口，统一管理旁白音色与三套模型/动画 HDRI 环境；每套 HDRI 都可以直接进入 3D 预览，半球直径支持 5–30 米调节。
- 通用资产的 HDRI 3D 预览与漫剧场景编辑使用同一套场景编辑体验；对象列表只保留环境对象，投射中心和半球直径可直接调整。
- 通用资产列表支持 2D 全景图放大预览，并隐藏内部资源路径，让资产名称、全景图和 3D 预览入口更清晰。
- 模型旋转相机时，HDRI 环境保持世界空间位置不变，不再随镜头漂移；环境边界和取景距离按当前半球直径约束。
