...
### 2026-09-02
- 漫剧分镜 3D 草图的 AI 构图整体升级：AI 现在只负责摆放角色与声明构图意图（本镜焦点角色、主体偏左/居中/偏右的三分构图、是否景深虚化），摄像机机位、视线、取景范围和景深改由系统按角色实际落位与镜头景别精确计算，视线始终正对叙事主体；镜头文本里标注的「画面左侧 / 右侧 / 前景」也会被换算成实际站位。近景更近、全景装得下、焦点更准，构图结果与分镜设定一致。
- 漫剧视频阶段页面重排：合成完成的视频播放器移到页面最上方并限制高度不超过一屏，时长、镜头、字幕、规格和生成时间在播放器右侧的固定信息栏展示，字幕下载与新窗口打开也在其中；合成设置、合成按钮和素材缺口统计依次排在下方，去掉与统计重复的顶部素材小卡。
- 漫剧分镜的 AI 画面改为严格跟随 3D 摆位草图：确认后的草图是 3D 摄像机的实拍取景，生成画面时取景范围、机位透视、角色站位、相对大小与前后关系都以它为基准，场景整图不再混入参考导致画面退回场景原图；已有项目在分镜列表点「重新生图」即可按新逻辑重出画面。
- 漫剧分镜列表默认直接展示每一镜的 3D 摆位草图（3D 摄像机实拍取景），不再默认显示 AI 画面；要看 AI 成图时再用「3D图 / AI图」切换。
- 漫剧 3D 分镜编辑器新增穹顶边界：编辑视角和场景摄像机都不能移出全景范围，视角推拉、平移和旧布局打开时都会自动收回边界内，分镜草图不再拍到穹顶外的灰底；场景图重新生成后，分镜列表会在用过旧背景的草图上显示「场景图已更新」，进编辑器重新截图即可换上新背景。
- 行走奔跑蹲伏的脚步落地修正：重定向管线补齐腿部末端校正——脚部落点按骨盆位置和腿长比例解算（与手臂同套方案），蹲伏前进、向前慢跑这类移动动作的脚印和膝弯不再漂移变形，姿势与源动画一致。
- 模型库第四批扩量：新增 138 个场景模型入库，全库达到 399 个精选模型；新增「建筑构件」分类，收录房门、商用门、百叶窗、通风口、水泥楼梯等搭建场景的结构件，并补齐医院（除颤仪、药瓶、医疗推车）、校园（课桌、储物柜、校门）、仓储和亚洲村落风格的道具组，全部经过尺寸、材质、视觉审核与前景门禁。
