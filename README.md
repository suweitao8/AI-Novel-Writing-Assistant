...
- 状态当场编辑、当场生成：外观状态的所有字段直接可编辑，点「保存状态」生效，点生成会先自动保存；「添加状态」在列表底部。音色可「生成」也可「选取」任意状态的现成音色；生成图片前就在图片区选参考图（任意状态或留空生成全新形象）。- 新增“画风管理”页面，角色、场景、道具各有独立画风卡片，可分别编辑、保存或恢复默认。
