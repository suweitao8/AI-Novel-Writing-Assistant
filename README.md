...
- 场景 3D 编辑器会根据场景默认状态的类型自动采用合适的默认半球直径：室内 10、室外 15、自然 20，投射中心高度统一为 2；同一场景的其他状态共享这组环境参数，手动调整过的参数会继续保留。
- 场景 3D 编辑器的比例参照角色统一为约 1.7 米并放在场景中心；选中标记改为半透明空心圆环，代理角色使用当前 HDRI 环境光进行明暗表现。
- 场景 3D 编辑器调整投射中心高度和半球直径时只更新当前预览，退出编辑器才一次性保存；分镜 3D 草图编辑也统一采用退出保存，连续调参不会反复触发保存请求。
- 场景全景图生成统一约束为严格的上下分区：下半区保持连续干净地面，家具和高大物体不得跨过中心安全带；场景 3D 编辑器可将全景地面分界调整为 40%–65%，分镜 3D 草图会复用该设置。
...
- 漫剧 3D 草图的选中角色标记改为深绿色空心圆环，中心保持透明，方便辨认角色位置而不遮住地面。
