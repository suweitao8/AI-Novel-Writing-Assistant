...
- 动画导入完成全量重建：277 条 Cine57 动作全部重新经过最新的骨骼重定向、手臂/腿部校正和 Biped 命名兼容处理，动画预览与分镜草图使用同一份最新动作资源，避免优化后的流程只作用于少数片段。
- 向前奔跑动作的骨骼重定向修复：按源动画逐帧对齐躯干、颈部、肩链和腿链的解剖骨段，移除会重复改变跑姿的通用胸腔与末端校正；该动作在动画库与分镜 3D 草图中不再出现低头、肩膀错位和脚步变形。
