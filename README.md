...
- 模型 3D 预览新增三套固定 HDRI 环境：室内客厅、中央广场和草地自然，可在预览环境中直接切换；中心到边界半径固定为 10 米、20 米和 50 米。
- 系统设置新增资产预设表，统一展示旁白音色与三套模型/动画 HDRI 环境；模型、动画和缩略图读取同一套固定预设。
- 模型旋转相机时，HDRI 环境保持世界空间位置不变，不随镜头漂移或动态缩放；环境边界和取景距离按所选固定半径约束。
