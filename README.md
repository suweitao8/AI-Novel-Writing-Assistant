...
- 角色和道具的参考图升级为透明背景：角色四视图/状态图与道具图直接输出可叠加的透明底 PNG（无背景色、无地面阴影），能更干净地作为分镜画面的参考素材；这两类参考图统一改用 Codex 图片通道生成，走已登录的订阅额度。场景全景、分镜首帧等画面类图片保持原有背景不变。道具参考图只画道具本身——周围的其它物品和环境不会进画面，提示词也会把道具的材质、颜色、造型、工艺与磨损细节写得更具体。
- 状态当场编辑、当场生成：外观状态的所有字段直接可编辑，点「保存状态」生效，点生成会先自动保存；「添加状态」在列表底部。音色可「生成」也可「选取」任意状态的现成音色；生成图片前就在图片区选参考图（任意状态或留空生成全新形象）。图片提示词可以 AI 微调——写一句要改的地方（如「去掉身上的伤」），AI 只改这一处、其余描述原样保留。生成图片/音色时可以随时关掉弹窗，生成会继续完成，重开弹窗直接显示最新结果。保存统一走弹窗底部一个「保存」按钮，姓名和全部外观状态一次保存；音色试听换成了吐字更清楚的参考句。
