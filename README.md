...
### 2026-08-31
- 模型库扩充到 79 个可直接摆放和交互的前景模型，新增家具、容器、厨房餐具、书籍办公、玩具装饰、户外和卫浴道具；自然物细分为石头、灌木、树木、草、花和盆栽。
- 食材/纸箱只保留两个代表模型，小摆件、雕像、奖杯和花瓶统一归入「玩具/装饰品」，模型查找更清楚。
- 动画库现在统一展示 UAL2 内置动作和 Cine57 导入动作，共 46 段，可按待机、移动、坐姿和其他动作筛选。
- 修正导入动画的站立与行走姿势，手臂不再贴近错误的 T 形基准；脚步、坐姿和分镜 3D 草图继续使用同一套蓝色 UAL2 角色。
- 动画卡片缩略图改为复用同一份角色动画资源，资源更新后会自动刷新，打开完整预览页即可逐帧检查动作。
- 完整预览页拖动时间轴后会立即同步当前帧时间，暂停、逐帧检查和设置卡片预览帧的操作保持一致。
- 修复模型贴图过度压缩导致的像素化问题，3D 预览中的岩石、木纹和砖石细节更清晰，模型卡片会自动刷新贴图更新后的预览。
- 模型库 3D 预览的滚轮缩放按模型实际尺寸自适应：大模型可以继续拉远，小模型可以继续拉近；HDRI 半球直径不再限制相机取景范围，聚焦和复位视角也会同步适配模型尺度。
- 模型 3D 预览与模型卡片增加环境补光，投影阴影更柔和、边缘更平滑，暗部材质细节和模型与地面的接触关系更清晰。
- 通用资产中的中央广场 HDRI 预览默认将投射中心设为半球圆半径的 10%（15 米直径对应 0.75 米），3D 编辑页仍可调整投射中心、半球直径和分界线。
- 漫剧场景 3D 预览会从 HDRI 全景图的高亮区域自动识别直射光方向，让角色受光面和地面阴影更贴合环境；半球直径统一按 5–30 米调节。
- HDRI 全景亮度经过归一化时，3D 预览仍会从实际太阳/高光区域计算主光方向，模型、角色受光和地面阴影保持与全景画面一致。
- 模型库聚焦角色可以摆放和交互的前景道具：移除纯色背景板、场景级巨石、建筑装饰条和无法组成完整道具的炉灶碎片，保留床、桌椅、书本、箱类组合、植物、灯具、地毯与小型自然物。
- 模型 3D 预览不再把独立背景板当作素材，统一使用 HDR 全景作为环境背景；过大的场景级模型不会再出现在前景道具选择中。
- 模型资源进入库前会检查实际尺寸、碰撞壳和高阶 LOD，减少模型预览出现白色包裹、比例失控或不可用建筑碎片的情况。
- 模型详情页提供只读 3D 预览：显示顶点数量和包围盒的长宽高，画面中同步标出包围盒；环境与模型变换由系统统一管理，详情页只保留相机查看操作。
- 模型详情页默认隐藏包围盒，勾选“显示包围盒”即可查看灰色尺寸线框，取消勾选即可恢复干净预览。
- 通用资产中的中央广场 HDRI 预览默认将投射中心设为半球圆半径的 10%（15 米直径对应 0.75 米），通用资产 3D 编辑页仍可调整投射中心、半球直径和分界线。
