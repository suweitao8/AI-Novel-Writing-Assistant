...
- 漫剧「参考」页签升级：本章没有参考文本时只读展示整本参考小说（解析、提取直接可用），可一键复制为本章参考或粘贴新文本，不会再把整本当成空输入框叠出重复内容；「提取」「解析」按钮不能点时旁边直接说明原因，提取页写明用的是本章文本还是整本小说，参考文本编辑时可见自动保存状态。
...
- 参考小说不再丢失：每章的参考正文随章节自动保存，重开页面后仍在；某章没有参考文本时自动展示整本参考小说，新建章节标题按参考小说预填。
