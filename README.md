...
- 角色音色更自然：音色生成会把「角色名的中文声音＋音色描述」作为声音设计指令传给语音模型，并自动降噪；没填音色描述（或只是默认占位）时，AI 会按角色的性别、年龄、形象和画面提示词推断一条合适的音色描述再生成的效——不再出现极低沉的机械怪声；分镜配音引用状态试听音频做音色克隆的链路也已真正接通。手写的音色描述永远优先。
