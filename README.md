...
- 场景 3D 编辑和分镜 3D 草图现在会按浏览器高度铺满工作区；桌面端左侧集中显示返回入口、场景对象和属性内容，右侧保留完整 3D 视口，对象项只显示图标和名称，HDRI 对象显示为「世界」、尺度校准人物显示为「参考角色」、空间标记会直接列出，列表与属性内容分别在各自区域内滚动。选中可移动角色时，属性区域会显示位置、旋转和大小，视口同步显示绿色外轮廓高亮。
