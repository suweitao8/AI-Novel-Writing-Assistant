...
- 「解析」一键双收：生成分镜初稿的同时提取本章的角色/场景/道具/世界观（随章节保存，不用也不丢），角色自动带外貌、性格、画面与音色提示词；剧情里的换装、受伤、昼夜、道具损坏会被识别为新的外观状态，生图配音自动跟进；资产页签里的角色、场景、道具都可删除。
- 漫剧「参考」页签「引用」按钮：一键带入参考小说同序号章节（按「第一章/第二章…」自动切分），解析只用编辑器里的内容，参考正文随章节自动保存。
- 设定卡片更轻：初稿右侧只显示类型和名字，点开弹窗看完整信息（含生图/音色提示词与外观状态）并可删除；「提取」建议卡片加「详情」查看全貌。
- 配音带上情绪：分镜台词按「角色（语气）：台词」生成，配音逐句把语气传给语音服务（VoxCPM 情绪提示），每句台词有听得出的情绪而非固定腔调；分镜列表每句台词旁显示语气。
- 世界地图升级为画布：亲手圈出地形（平地/山地/水域）、把城市摆上陆地，两城连线自动算距离与步行/骑马/车船耗时；选中城市可进入它的内部地图，面包屑随时回大地图。
- 参考小说不再丢失：每章的参考正文随章节自动保存，重开页面后仍在；新建章节标题按参考小说对应章节预填。
- 初稿分镜画面带上景别（特写/近景/中景等）与人物动作、神态、正在发生的事，一格看一眼就知道这一镜拍什么。
- 初稿解析升级为分镜式：一格分镜两行（分镜画面 + 旁白或角色台词，台词带表情神态），格子之间空行；初稿编辑器里对话格整组淡蓝、旁白格白色，一眼区分。
- 「提取」建议更全：有名字或有台词的角色、出现的地点、核心设定概念都会列入建议，勾选后创建。
- 漫剧「设定」页签分成三块：「世界观」把关键概念一条一条写清楚（比如丧尸是什么、异能怎么运作），每条概念名加一段解释，可增可删；「通用」收着参考小说、画面风格与分镜项目状态。
- 新增世界地图：点「AI 生成地图」按世界观规划地点、方位与通路，先预览再应用；地图可编辑——拖动地点摆位、增删地点、连线标注通路，再写一段地图总述讲清整体格局。
- 分镜与配音合并成一个页签：一行一个分镜（首帧图+镜头信息+台词）带它自己的配音段，可播放、可单镜重配。
- 章节管理更顺手：一行排 5 个章节、卡片只留章节号与标题；列表固定高度不再跳变，打开时自动定位到正在编辑的章节。
- 创建漫剧可拖入现成小说（txt）：作品名自动按文件名填好，小说正文进入「参考」页签直接解析；「提取」页签可从参考小说提取角色、场景与世界观设定。
