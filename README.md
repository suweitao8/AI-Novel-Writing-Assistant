...
- 创建漫剧可拖入现成小说（txt）：作品名自动按文件名填好，小说正文进入「参考」页签直接解析；新建章节只填标题，并自动预填参考小说对应章节的标题。
- 漫剧「参考/提取」页签：参考原文压缩成约 20 行初稿（自动忽略书名/章节标题/作者感言，优先保留动作神态与场景画面）；「提取」从参考小说提取角色、场景、世界观，确认后创建进项目设定。
- 漫剧工作室界面更清爽：空状态与提示只保留事实信息，去掉「先去某页签」「点某按钮」一类操作指引文案。
- 漫剧「资产 / 设定」页签整理：「资产」分角色 / 场景 / 道具（带数量），「设定」分世界观与项目，子页签条与「当前」同款居中样式；顶栏返回改为图标+项目名轻量样式，无按钮高亮。
- 漫剧「分镜」页签内嵌完整分镜工作台（分镜板/首帧/视频任务就地操作，不跳转）；切章自动同步小说最新内容到分镜。
- 漫剧顶栏：返回按钮和项目名合并为整体，点任意部分回到漫剧列表；当前章按钮去图标，旁边「＋」快速新建下一章。
- 漫剧工作室页签重组：顶部「当前 / 资产 / 设定」，「当前」的子页签（初稿 / 正文 / 分镜 / 配音 / 视频）都作用于当前章；初稿默认铺 20 行编号空行，正文可看节拍与 AI 已写成的本章正文，「设定」集中放世界观与画面风格。
- 章节管理简化：只留新建与搜索，卡片显示第几章、章节名和字数，点卡片即切换当前章；关闭走右上角 ×。
- 修复漫剧工作室写作被「简易模式项目当前仅供阅读」拦住的问题：写本章大纲、解析与保存细纲、新建章节现在都正常可用。
- 漫剧新增本地视频合成通道：本机 ffmpeg 直接把首帧图+配音合成真实视频片段，无需外部视频服务；漫剧项目可选画面风格（动漫/3D国漫/写实等六种）统一首帧图与角色图质感。
- 漫剧全链路可用现成小说直达成片：导入小说 → 分集 → 台本 → 分镜 → 首帧 → 配音 → 视频合成 → 整集成片。
- 图片放大查看：漫画格子、设计稿、资产图、分镜首帧点击即放大，支持下载。
- 漫剧分镜镜头可编辑：改台词/动作/景别/运镜/时长，台词改动后配音自动标记需重配；角色资产库支持搜索与筛选。
- 漫剧新增「整集合成」：视频工作台一键把整集镜头拼成竖屏成片，自动对齐配音时长、带片头片尾卡；缺视频片段的镜头用首帧图加配音做动态画面。字幕可烧进画面或下载 SRT，完成后直接播放、随时重合成。
- 移除「创作向导」页：首页同样展示创作进度和下一步建议，配置引导弹窗保留，导航更聚焦创作。
- 漫画/短剧图像规范统一：角色四视图、场景 360° 全景、道具单透视横版设计稿；设计参考图固定横版，分镜首帧与封面固定竖版。
- 漫剧工作室支持章节管理：顶栏「章节管理」进入，搜索/新建章节，每章可写大纲并让 AI 展开成细纲节拍，跟着节拍写正文。
- 漫剧「配音」升级为完整配音工作台：台词逐行展示（角色对白/旁白），每行显示已配音可播放、已过期需重配、未生成三种状态；支持按集批量生成、单镜重配，未变化的行自动复用。角色和旁白都可以用一句话描述声音并生成试听。
- 漫剧分镜焕新为卡片式分镜板：每个镜头一张竖版卡片，首帧图与镜头信息一目了然；支持勾选批量生成首帧、一键选未生成/失败、宫格预览、大图模式与单镜放大，生成中自动刷新进度、失败卡片上直接重试。
- 图片生成不再被服务重启卡死：中断的生成会自动提示重新生成；漫画项目页在有生成任务时自动刷新进度。
- 漫剧列表卡片更紧凑：宽度收窄一半、高度增加，显示章数与全文字数；新增删除按钮，可连同分镜、配音、视频数据一并清理项目。
- 漫剧列表卡片精简：去掉封面预览图，点击卡片任意位置即可打开漫剧工作室。
- 新增「画面风格」：封面、角色立绘、拆书角色形象图和漫画画风可选用统一的画面风格预设（3D玄幻国漫、动漫、写实现代、写实古装剧、民国年代剧、写实末日）；上传参考图即可由 AI 提炼生成自定义风格。风格只决定画面媒介与质感，不会改动角色与场景设定。
- 短篇「编辑正文」升级：支持关键词查找并高亮全部匹配、逐个跳转（Ctrl+F），编辑区左侧显示全篇行号，底部实时显示字数/行数/段落与光标所在行列。
- 新增「漫剧」完整工作流：侧边栏新增「漫剧列表」，一个作品名即可开始。先写小说（可自建设定、写简略大纲让 AI 推理分章细纲再逐章写），有成稿后生成分镜与首帧图，再做逐镜配音，最后合成动态漫视频；边写边做，新章节随时同步。漫剧项目独立管理，不混入小说列表。
- 产品定位为纯网页应用：移除「桌面与维护」等桌面版专属入口，网页端创作功能不受影响。
...
- 书架卡片去掉封面并收窄一半，一屏能同时看到更多作品；「AI 自动导演开书」入口暂时收起，新建小说从「创建小说」填写基础信息开始。
- 拆书结果支持删除：可清掉旧拆书记录随时重拆，来源文档保留；旧拆书任务重新解析时自动改用当前文本模型，不再因历史模型配置失败。
