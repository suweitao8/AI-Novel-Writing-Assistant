...
- 「初稿」+「正文」合并为「脚本」页签：线性列表一行一件事（场景切换/角色形象切换/分镜/分镜下的台词），点击即改、悬停增删移，角色场景道具名字按类别高亮——视频按什么顺序发生，脚本就按什么顺序排；解析产出直接当脚本用。
- 提取的角色直接填好性别、年龄段、体型（外貌只写样貌穿着），点开建议卡片即见表单预填完毕，确认即用。
- 提取建议逐条核对应用：点开即可查看并修改全部信息（与资产页签同一套表单），改好点「应用」创建这一条；同名资产拦截重复，外观状态由你手动管理。
- 美术风格独立成页签：默认画风点选即存——新增「3D写实电影」（虚幻引擎写实质感）为默认，共 7 种内置画风，也可给本书自定义多套（如「现代诡异」「末世爆发后」）；想换画风改选默认风格，整本统一。
- 世界观基本设定只留世界前提与时代背景（下拉选择、可自定义），基调与世界规则统一到关键设定条目里一条一条写。
- AI 实况里每个模型调用按环节标注标题（参考解析的两个调用、漫剧的策略/剧本/分镜等），一眼看清正在做什么。
- 初稿支持场景切换与角色形象切换：换场处自动插入「【场景：地点】」（优先对齐设定中心场景名），角色受伤、变身、换装时插入「【角色状态：角色：状态】」，后续画面生成按标记切换。
- 「解析」一键双收：生成分镜初稿的同时提取本章的角色/场景/道具/世界观（随章节保存，不用也不丢），角色自动带外貌、性格、画面与音色提示词；资产页签里的角色、场景、道具都可删除。
- 「解析」的提取结果解析完就保存：中途切到别的页面也不丢，回到「提取」页签仍然在。
- 「解析」一次模型调用同时产出初稿与设定提取；角色属性从简（外貌体型合一，主要看画面/音色提示词），角色/场景/道具可手动管理外观状态并配置生图参考（参考旧状态或不参考直接生成）。
