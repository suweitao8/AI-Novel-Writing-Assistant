...
- 漫剧 3D 草图与 Anim57 徒手攻击预览统一使用 UE5 原生 Manny / Quinn 角色资源：系统按性别、角色类型和体型自动选择默认模型，徒手动作与角色使用同一套原生骨架。
- 徒手战斗筛选严格保留指定的 4 条 Anim57 攻击动画；3D 草图进入时直接显示站立基础姿势，预览继续使用蓝色角色与中央广场 HDRI。
...
- 漫剧 AI 构图会让攻击、面对、持有、跟随以及位于目标上方的主动角色朝向目标；缺少专用躺/趴动作时，3D 草图保留贴地姿势语义，不会静默变成站立。
- 漫剧 3D 场景 HDRI 直接按世界方向采样原始全景源图，`.hdr` 与普通场景图使用对应颜色解码，环境细节更清晰。
- 漫剧 3D 场景修复 HDRI 地面投影：普通生成全景图的下半部不再作为透视纹理重新展开，避免远距离观察出现同心圆和漩涡；编辑器使用完整水平地面与外圈垂直圆墙，天空和远景继续按 HDRI 方向采样，模型库前景模型与角色可正常接地并接收阴影。
- 模型素材导入增加透明通道校验与真实三维详情预览检查；草、树叶、花和灌木的镂空材质保留正确显示，无法通过材质或预览检查的候选不会进入可用模型库，已替换的 54 个有损透明贴图输出移到仓库外的可恢复隔离清单。
- 模型库前景策展与导入保护：当前目录为 445 个精选静态模型（另含 1 个角色资源），39 个已确认不适合前景的候选保留在可恢复隔离位置；缺少真实颜色贴图、只含内嵌 1×1 Base Color 占位图的模型会在统一准入阶段拒绝。模型导入历史按源包路径、Mesh 和源指纹记录每次处理结论，未变化且已拒绝的模型会在转换前自动跳过；新候选必须先生成方形详情预览并通过尺寸、颜色来源、材质、请求和视觉门禁才能发布。
