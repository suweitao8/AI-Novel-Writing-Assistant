...
### 2026-08-28
- 漫剧分镜全面转向静态画面生产：分镜生成、镜头编辑与 3D 草图的「镜头设计」面板不再包含运镜与时长信息，界面只保留景别、动作、对白等静态分镜真正需要的字段；已生成分镜的历史数据不受影响。完整历史见 [docs/releases/release-notes.md](docs/releases/release-notes.md)。
