...
### 2026-08-30
- 编辑环境的预览区不再空白：未生成全景的状态直接显示内置中央广场全景，生成后自动替换，与卡片和 3D 预览画面一致。

- 模型与动画的 3D 预览统一使用中央广场 HDRI 环境：模型编辑器不再提供环境选择，室内客厅与草地自然已下线，通用资产页只保留中央广场环境卡片。

- 修复模型与动画预览切换到草地自然等高饱和 HDRI 时画面发白的问题，环境颜色在各页面保持一致。
- HDRI 环境按应用方向使用：室内用室内客厅、城市户外用中央广场、纯自然户外用草地自然，由使用场景选择环境，环境内不再有「设为当前全景」的切换；生效全景就是各环境的默认状态生成图。
- 通用资产的 HDRI 环境改用与场景资产一致的卡片浏览：卡片直接显示生效全景和状态数，点卡片即进入编辑；状态图生成后编辑器内出现「3D编辑」入口，可跳整页 HDRI 预览调整投射中心与半球直径。
- 通用资产的「编辑环境」与场景资产编辑器完全同款：状态列表、参考图、时代风格、时间天气、图片提示词和 AI 改写同一套交互，生成的全景自动用于对应方向的预览；生成失败后的「关闭失败提示」也已修复为只关闭当前显示的那次失败。
- 通用资产的 HDRI 3D 预览使用整页编辑布局：3D 视口铺满整个工作区，与漫剧场景 3D 编辑保持一致，方便在大画面下切换环境、调整投射中心、半球直径和分界线。
