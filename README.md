...
### 2026-08-28
- 场景全景图改为纯背景：新生成的场景状态图不再把床、桌、椅等家具画进全景，只保留墙面、地面、门窗、天空远景等背景内容，场景描述里写了家具也不会入画。
- 场景 3D 编辑支持手动摆放前景家具：在「空间标记」里选择床、椅子、桌子等类型点「添加标记」，再用拖拽手柄或数值摆到位；重新空间识别不会丢掉手动摆放的道具。
- AI 自动构图会识别场景里已摆放的前景家具：动作需要坐下、躺下或倚靠时，角色会直接坐到椅子上、躺在床上并配上对应姿势；没被用到的家具仍然作为障碍避让。
- 生成分镜首帧时会带上场景里摆放好的家具清单，床、桌、椅等会按摆位草图的位置与朝向画进画面。
- 顶部导航在「漫剧」旁新增「模型」入口：内置常用模型库，首批收录厨具、地毯、炉灶、装饰构件、背景板等 15 个模型；每个模型自动生成 3D 预览图，可按分类筛选查看。
- 模型库的「打开 3D 编辑」进入独立 3D 场景：模型按真实比例自动落地居中展示，支持 Unity 风格的移动 / 旋转 / 缩放手柄与属性面板精确键入（旋转仅绕竖轴、缩放仅整体等比），并提供聚焦、复位视角、右键环绕、滚轮缩放和导出快照。
- AI 自动构图会先理解角色之间的上下、前后和大小关系，再生成摆位；当角色需要叠放时，位于上方的角色会保持贴合在下方角色之上，并按真实身高校准体型比例，减少主客体颠倒和比例失真的分镜。
- 场景 3D 编辑器的半球直径可在 5–30 米之间调节；场景资产、空间标记、分镜快照和 3D 预览统一使用这组范围，投射中心高度会继续按当前直径与比例同步计算。
- 场景里的摄像机改为 Unity 风格的白色线框：机身、镜头和取景锥都以白色线框常驻显示，不再被暗色实体挡住布景；选中摄像机时线框变橙色，点线框即可选中并拖动摆位。
- 漫剧分镜全面转向静态画面生产：分镜生成、镜头编辑与 3D 草图的「镜头设计」面板不再包含运镜与时长信息，界面只保留景别、动作、对白等静态分镜真正需要的字段；已生成分镜的历史数据不受影响。
- 分镜 3D 草图右下角的取景预览画面更干净：只保留布景、角色和环境本身，不再混入摄像机机身、地面网格和各类辅助线；预览上新增三分构图线（两条横线、两条竖线），摆机位时可以直接对照构图参考调整。
- 3D 草图的角色面板按「名称 → Transform → 角色属性」重新组织：身高、静态姿势、模型外观收进 Transform 下方的折叠组件节；角色不再提供「从本镜移除」按钮；相机聚焦、复位视角、镜头取景等视口操作移出角色面板，只在镜头/世界与摄像机面板中提供。
- 3D 编辑器属性面板顶部的对象名称与面板上沿留出间距：场景 3D 编辑与分镜 3D 草图两处属性面板统一留白，名称不再紧贴边框。
