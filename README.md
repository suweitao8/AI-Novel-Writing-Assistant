...
- 小说列表简化为单一书架视图：作品以紧凑的横版卡片展示，不再区分“书架/工作台”两种几乎相同的视图；AI 驾驶舱入口直接放在作品卡片上。
- 写法引擎焕新：当前写法放大置顶、其他写法统一排列，点击即切换且选择会被记住；页面配色跟随应用主题。
- 拆书支持导入外部小说：新建拆书时直接选择本机 txt 文件（自动识别中文编码），选中后显示文件名与导入状态；温度/Tokens/预算等专家参数移除，统一使用系统设置的默认文本模型。
- 标题生成更快：默认一次给出 5 个精选标题候选；AI 一次多产出几个候选再自动挑出风格和句式最有差异的一组，不再整批重新生成，等待时间大幅缩短。
- 设定中心支持按提示现场生成实体：写一句提示（如「男大学生」），AI 生成包含随机姓名、性别、年龄段、性格、外貌等完整属性的角色草稿（场景/道具同理），可修改后保存。
- 角色「面部锚点」、场景「环境提示词」、道具「视觉提示词」按图片生成所需格式生成，为后续一键生成角色立绘与场景图做准备。
- 短篇与简易创作新增「设定中心」：作品页可切换管理角色、场景、关键道具与世界观（含世界地图与关键设定），支持手动编辑与 AI 生成。
- 动笔前先立设定：AI 自动生成设定草稿，短篇写作前一键确认即可开始；正文生成会遵守这些设定，减少“硬写”的漂浮感。
- 生成任务更省时：短篇整体规划采用更紧凑的输出格式，规划阶段的等待时间明显缩短。
- 模型设置的文本模型卡片提供“思考模式”开关：追求生成速度可以关闭；不提供思考模式的模型和本机订阅通道不受影响。
- 任务详情新增“模型调用耗时”：逐步显示每次模型调用用了哪个模型、花了多久、消耗多少 token，慢在哪一步一眼可见。
- 提示词管理页面跟随应用主题显示：深色、夜航、宣纸等皮肤下，提示词目录、槽位编辑、预览与上下文面板的配色与全站保持一致。
- 模型设置按能力分类：文本模型、图片模型、音频模型。配置好文本模型后，大纲、正文、审校、修复等全部文字任务自动使用该模型；更换供应商只需修改对应类别的服务地址、API Key 和模型名称。
- 音频模型可直接配置：默认连接本机 VoxCPM2 语音服务，用于角色配音与朗读；本地生成不消耗云端额度，也可以换成其他兼容 OpenAI /v1/audio/speech 接口的语音服务。
- 各创作页面的模型选择简化为直接选择文本模型；新手引导只需连接一次文本模型，检测通过后即可开始创作。封面、角色立绘、场景插图等图片任务统一使用图片模型类别的配置。
...
- 拆书失败时会明确显示失败状态和具体原因，可以一键重试；不再出现“任务完成但栏目全空”的假成功。
