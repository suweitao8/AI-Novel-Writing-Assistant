...
- 资产图片提示词更干净：解析和 AI 改写生成的图片提示词只描述画面内容本身（角色长相、场景环境、道具细节），不再混入画风、背景、四视图这类由系统统一管理的词——以前提示词里出现「写实动漫风格、纯白背景」会和透明背景、统一画风的要求互相打架，影响出图效果；已存的旧提示词不受影响，生成时会自动忽略这类多余词。
- 角色音色更自然：音色生成会把「角色名的中文声音＋音色描述」作为声音设计指令传给语音模型，并自动降噪；没填音色描述（或只是默认占位）时，AI 会按角色的性别、年龄、形象和画面提示词推断一条合适的音色描述再生成的效——不再出现极低沉的机械怪声；分镜配音引用状态试听音频做音色克隆的链路也已真正接通。手写的音色描述永远优先。音色试听的播放速度也已修正（此前会整体偏慢、音调偏低）。
- 角色和道具的参考图升级为透明背景：角色四视图/状态图与道具图直接输出可叠加的透明底 PNG（无背景色、无地面阴影），能更干净地作为分镜画面的参考素材；这两类参考图统一改用 Codex 图片通道生成，走已登录的订阅额度。场景全景、分镜首帧等画面类图片保持原有背景不变。
- 状态当场编辑、当场生成：外观状态的所有字段直接可编辑，点「保存状态」生效，点生成会先自动保存；「添加状态」在列表底部。音色可「生成」也可「选取」任意状态的现成音色；生成图片前就在图片区选参考图（任意状态或留空生成全新形象）。图片提示词可以 AI 微调——写一句要改的地方（如「去掉身上的伤」），AI 只改这一处、其余描述原样保留。生成图片/音色时可以随时关掉弹窗，生成会继续完成，重开弹窗直接显示最新结果。
- 新增“画风管理”页面，角色、场景、道具各有独立画风卡片，可分别编辑、保存或恢复默认。
- 角色资产和状态图固定使用横向四视图，场景使用 360° 全景，道具使用 45° 三点透视；固定规格与固定约束由系统统一维护。场景全景图生成后可以直接拖拽环视、滚轮缩放，像站在场景里转圈查看，也可切回平面图。
- 分镜首帧只注入当前镜头实际出现的资产类别，避免把角色四视图、场景全景或道具透视要求带进竖屏构图。
- 漫剧「提取」里与已有资产同名的建议，点开弹窗直接编辑该资产——状态列表载入已有数据并显示已生成的图片/音色，点「保存」更新；重新生成的状态图在各处立即更新。
