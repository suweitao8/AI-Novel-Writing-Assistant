...
- 参考小说按章节自动对应：参考小说按「第一章/第二章…」标题自动切分，工作室第 1 章只用参考小说第 1 章的内容做参考、解析与提取，不再把后面章节的剧情带进来；本章没有对应章节时会说明原因，可粘贴自己的文本。「参考」页签只读展示对应内容（解析、提取直接可用），一键复制为本章参考或粘贴新文本，不会叠出重复内容；「提取」「解析」按钮不能点时旁边直接说明原因，参考文本编辑时可见自动保存状态。
