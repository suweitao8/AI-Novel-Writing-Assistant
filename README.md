...
- 状态当场编辑、当场生成：外观状态的所有字段直接可编辑，点「保存状态」生效，点生成会先自动保存；「添加状态」在列表底部。音色可「生成」也可「选取」任意状态的现成音色；生成图片前就在图片区选参考图（任意状态或留空生成全新形象）。图片提示词可以 AI 微调——写一句要改的地方（如「去掉身上的伤」），AI 只改这一处、其余描述原样保留。
