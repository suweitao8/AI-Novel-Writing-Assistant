...
- AI 模型额度统一切换到 Codex 订阅：文本、视觉（空间识别/画风识别）与图片生成走同一份额度，统一由 GPT-5.6 Luna 驱动且不提供其它模型选项；文字与视觉任务默认采用高思考深度（实测日常任务响应速度几乎不变，复杂创作质量更好），图片生成保持快速出图；本地桥接会把场景图直接交给模型理解。Grok 与 OpenCode Go 通道保留注册，额度恢复后可在设置页切回。
