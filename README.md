...
- 漫剧工作台启动页会区分服务未响应与 HTTP 错误并持续重试；素材统计、批量配音、合成预检和实际配音取用统一按当前文本、音色和模型判断，切换章节时任务与批量选择也会自动隔离。
- 服务启动会先完成遗留任务恢复，再接受新的漫剧任务；重复点击批量生成或合成时会复用当前任务，静音镜头也按明确设置的镜头时长进入成片，不伪造配音时长。
- 漫剧工作室将整集合成结果统一放到「视频」页签；「分镜」页只保留镜头列表，生成分镜、生成配音和合成操作集中在页签上方。
- 系统旁白默认使用成年女声描述，统一用标准普通话测试文本试听；开发环境会自动准备本地 VoxCPM2 音频通道，连接失败时会显示具体地址。
- 旁白、角色和状态音色试听与配音统一响度，重新生成的音频会自动控制峰值，避免旁白过小或角色过响。
- 角色参考图、状态图、短剧/漫画角色设计稿和拆书角色图默认按中国/东亚人物形象生成：即使外貌资料不完整，也不会由图片模型自动补成欧美面孔；怪物等非人角色仍保持原设定。
- 角色、场景和道具缺少默认状态图时会自动并行补图：进入设置或漫剧工作室后系统会自动发现缺图并开始生成，资产卡片即时高亮显示“生成中”，失败时保留“生成失败”状态并可从详情重试。
- 漫剧整集合成会并行准备镜头素材和配音片段，并提高画面渲染并发；720P 开发与 1080P 正式输出的清晰度、音画同步和字幕样式保持不变，长集生成更快也更容易定位耗时。
- 漫剧分镜新增「摆位草图」：可在场景 360° 全景中摆放本镜角色，保存并确认后，分镜画面会锁定该视角与角色相对位置生成。
- 漫剧分镜新增独立「3D 摆位台」：可在真实 3D 视口中调整角色位置、相机、旋转、缩放和上下关系，并切换站立、坐着、躺着、趴着等代理姿势；保存后可继续编辑，确认结果继续作为严格 16:9 分镜生成参考。
- 角色、场景和道具的状态图片现在按资产、状态和生成批次分别保存，重新生成不会覆盖其他资产；生成失败时保留原图。无法安全判断旧图片归属的资产会明确标记为缺失，可直接重新生成。
- 分镜配音区按真实音频状态显示：没有可播放音频时只保留「生成配音」入口，音频生成后才显示可拖动的试听进度条，部分完成时只展示已有音频。
