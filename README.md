...
- 分镜列表收敛为紧凑四行结构：每一镜按「分镜设计」「场景/角色」「旁白/对白」「配音」对齐展示，场景与角色使用不同颜色的资产标签；没有音频时只显示生成入口，生成后显示可拖动的真实时长试听条。
- 分镜的 AI 摆位统一放在「编辑3D」里：点击「AI 自动构图」后，角色位置、姿势、相机和景深会直接显示在当前镜头预览中，确认前仍可继续手动调整。
- 场景 3D 编辑和分镜 3D 草图按浏览器高度铺满工作区：左侧集中显示返回入口、场景对象和属性内容，右侧保留完整 3D 视口；调整环境参数只更新当前预览，退出编辑器才一次性保存。
- 3D 草图和场景 3D 编辑中的选中角色改用贴合模型真实轮廓的绿色外描边，导出图片不会带上选中标记；分镜 3D 草图还支持调整选中角色的代理模型颜色并随布局保存。
- 漫剧 3D 场景 HDRI 改为连续的上下半球环境，减少地平线裂缝、漩涡和接缝拉伸；预览会从 HDRI 中识别窗户、太阳等高亮方向，为角色提供对应的基础方向光。
- 场景空间标记按全景图里的实际物体区域重新估算桌、椅、床、门窗等位置与尺寸，地面物体落到半球可用地面外圈；修改环境参数后旧标记会自动进入重新识别状态。
- 场景 3D 编辑器按场景类型（室内/室外/自然）自动采用合适的默认环境参数，半球直径可在 5–30 之间调整；场景全景图生成统一按固定构图分区（下半区连续地面、地平线以上远景带、顶部天空），场景状态编辑器的平面图显示 50%/70% 构图参考线。
- 角色进入分镜 3D 草图时会根据已有设定推断并保存近似身高，不同镜头里保持稳定的相对大小；角色状态编辑器可手动填写「身高（米）」，身高范围 0.50–10.00 米，5 米级怪物也能稳定参与 3D 分镜比例。
