...
- IndexTTS 2.5 的全局旁白音色统一在「系统设置 → 旁白音色」中配置，模型音色、参考音频和试听对整个应用生效；「分镜」页不再重复展示通用音色设置。
- 旁白音色设置统一为一个试听样本入口：参考音频可继续选择、上传和清除，但不会再和生成试听样本重复显示；IndexTTS 2.5 作为同一张旁白设置卡片中的语音合成来源。
- 漫剧旁白和角色音色可以选择 IndexTTS 2.5 的基础/训练音色与参考音频，也可以直接上传新的参考音频并单独保存音色来源；生成试听后会自动用于后续分镜配音，换音色会自动提示重新生成旧配音。
...
- 分镜画面生成会真正使用出场角色状态图、地点场景图和相关道具图作为参考，同一场景的连续镜头会保持统一光照关系。
...
- 漫剧分镜在同一项目内统一使用写实影视化媒介，时代氛围仍可随剧情变化；分镜工具栏提供「统一写实重生成」来明确重做已有画面。
...
- 系统旁白默认使用成年女声描述，统一用标准普通话测试文本试听；本地配音已切换到 IndexTTS 2.5，开发环境会自动准备 9005 API，连接失败时会显示具体地址。
...
- 漫剧分镜统一从「3D 草图」进入角色与相机摆位：角色姿势只截取静态关键帧，保存并确认后，分镜画面会锁定该视角与角色相对位置生成。
- 3D 草图把本镜场景状态图加载到内侧半球 HDRI 环境中，半球下半部同时提供弧形地面；环境固定在世界坐标，旋转或移动镜头只改变视点，不搬动地面，角色和地面处于同一 3D 场景，纯色地面只作为无环境图时的兜底。
- 3D 草图会根据场景图比例自动选择等距 HDRI 或 EnviroDome 式的普通场景图弧形地面投影；普通场景图的天空/远景与带贴图的下半球分层显示，右侧可以直接调整投射中心高度（0.6–10）、半球直径（20–100）、水平旋转和环境亮度，并随草图保存。
