...
### 2026-09-01
- 模型库统一收敛到现代写实资产，未经审核的其他风格模型不会进入可用目录；模型详情页支持带二次确认的可恢复隐藏，模型文件和已有分镜引用保持不变。
- 动画库入口卡片收敛为缩略图、动作名称、分类信息和帧数，移除卡片内重复的播放图标与用途徽标；整张卡片仍可直接进入 3D 预览，顶部用途筛选继续保留。
- 修复模型和动画预览中 HDRI 画面与角色受光、地面阴影方向相反的问题；可见 HDRI、环境光照和方向光现在共享同一方位，已有卡片缩略图会自动刷新。
- 动画、分镜和角色缩略图中的蓝色代理角色会用淡蓝色标出完整下颚线与外层颈部边界，正面和侧面都能更清楚地看见下巴位置。
- 模型库扩充到 208 个可直接摆放和交互的现代前景模型，重点补齐家具、收纳容器、厨房餐具与设备、办公电子、卫浴和灯具，并保留少量户外与日用模型。
- 新增模型沿用统一的现代写实风格，按真实尺寸和地面、墙面、台面或天花板摆放方式提供使用说明，适合住宅、办公室、商业空间和日常生活场景。
- 模型库与动画库缩略图缓存会随预览资源规则同步刷新，新增模型继续使用统一 HDRI 环境和落地投影阴影。
- 动画库分镜可用范围现在只收录原地播放的 Cine57 动作；通过来源和 GLB 根位移检查的 277 段动作可直接用于分镜，整体位移过大的动作会被排除，旧 UAL2 动作仍可在兼容动画中查看。
- 动画预览和分镜导入会同时检查手臂骨链与代表动作姿态，减少手部爆开、骨链断裂和角色整体漂移。
- 动画库筛选提供用途、动作分类和搜索三个入口；动作分类按待机、移动、坐姿、日常、互动、徒手战斗、拳击、剑术、武器战斗、魔法、受击/闪避、翻越/攀爬、睡眠、表演/手势、生物动作、潜行和配对互动等中等粒度浏览，当前范围没有片段的类别会自动隐藏，套装与姿态等详情仍保留在动画卡片中。
- 动画搜索支持点击按钮或按回车提交，搜索结果可以与用途和动作分类组合筛选。
- 动画预览支持“循环播放”开关：循环跨过动作边界时帧轴回到第 0 帧并保持同步，关闭后播放到最后一帧自动暂停。
- 模型库与动画库统一使用左侧分类、右侧搜索的筛选栏；搜索框和“搜索”按钮固定在桌面端右侧，窄屏时自动换行，模型搜索支持点击按钮或按回车提交。
- 点击模型进入详情时优先启动 3D 视口，后台卡片缩略图任务不会阻塞模型预览；HDRI 光照与材质仍按统一预览规则完成。
- 模型库会优先准备屏幕附近的卡片预览，浏览后续区域时自动继续加载，打开页面更快看到可用模型。
- 模型库按每页 24 个模型浏览，搜索或分类后自动回到第 1 页；当前页附近的预览优先准备，减少首次打开和切换结果时的等待。
