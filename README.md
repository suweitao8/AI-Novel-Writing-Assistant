...
### 2026-08-29
- 模型配色修正：办公桌、宫灯、瓷瓶、床品、婴儿床等模型显示源资产的真实颜色——宫灯恢复红色灯身，桌面、床架恢复木纹本色，瓷瓶显出釉色与纹样，过暗的床品、沙发恢复真实的布面与木色。
- 模型贴图更清晰：全部模型贴图按原始精度（最高 2048）载入，3D 编辑器里近距离观察木纹、砖石不再出现马赛克像素块。
- 模型库新增「动画」表格：已导出的角色动画（站立待机、行走循环、坐姿循环）统一收录在模型列表旁，点「预览」即可在弹窗中播放查看角色动作效果。
- 动画表格与模型表格同款分类页签：按待机、移动、坐姿分类并带数量统计，点页签即可筛选动画列表。
- 模型库修复所有模型的「白色包裹」：模型文件里混入的物理碰撞壳不再被渲染，家具、道具恢复真实轮廓；同时补齐纯材质件（玻璃、金属、墙漆、发光件）的真实颜色与质感，餐桌、椅子等不再显示为白模。
- 模型预览质感升级：所有模型带法线贴图与真实粗糙度，预览和 3D 编辑加入棚拍式三点布光、环境反射和电影级色调映射，砖石、织物、金属有了真实的光影层次。
- 模型库聚焦精选资产：保留 44 个常用模型（家具、装饰、灯具、厨房、地面、植物、自然、背景 8 类），更多道具会在质量达标后分批上架。
- 3D 场景全面转向「背景 + 摆放模型」的生产方式：场景全景图保留墙面、固定建筑、地面材质和远景自然景观，不再生成桌子、椅子、床等可交互家具，也不生成靠近镜头的石块、草丛、灌木等自然物；这些物件后续作为可交互的 3D 模型直接摆放进场景，角色可以与它们互动。
- 全景图画面更干净：下半区只保留一整片连续的地面材质，中景只保留远景背景（室内远墙门窗、室外远山天际线），任何区域都不再出现零散物件和地面杂物。
- 场景 3D 编辑与分镜 3D 草图不再显示全景图识别出的空间标记：识别入口、标记列表与标记检查器全部收起，3D 视图与对象列表回归世界、摄像机、角色等核心对象；已有场景中的标记数据同步清理。
- 3D 草图的「AI 构图」入口固定显示在顶部「AI 实况」左侧，进入页面即可快速生成或重新生成当前镜头构图；移动端也保留编辑器顶部入口。
- 自动构图处理叠放关系时，上方角色会使用蹲伏或跪姿；当前模型库缺少专用趴姿动画时不会再把角色渲染成仰面躺地。
- 漫剧工作流的系统设置聚焦于模型、旁白音色、画风、记录和视频输出，小说专用的创作可用性检查及相关配置入口暂时收起；保留直接地址，切回完整小说工作流时仍可恢复。
