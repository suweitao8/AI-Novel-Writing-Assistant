...
- 美术风格独立成页签：默认画风点选即存——新增「3D写实电影」（虚幻引擎写实质感）为默认，共 7 种内置画风，也可给本书自定义多套（如「现代诡异」「末世爆发后」）；初稿里用「【风格：风格名】」切换画风、「【角色状态：角色：状态】」切换角色形象，与【场景】同机制持续生效，解析按剧情自动插入。
- 世界观基本设定只留世界前提与时代背景（下拉选择、可自定义），基调与世界规则统一到关键设定条目里一条一条写。
- AI 实况里每个模型调用按环节标注标题（参考解析的两个调用、漫剧的策略/剧本/分镜等），一眼看清正在做什么。
- 初稿支持场景切换标注：解析自动在换场处插入「【场景：地点】」行（优先对齐设定中心场景名），编辑器里偏绿加粗一眼可见；对话格的蓝色也调淡到接近正文色，只带一点蓝。
