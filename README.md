...
### 2026-09-03
- 模型导入对透明材质采用 alpha 安全门禁：植物、草、花等带透明通道的贴图保留 PNG 与 opacity 映射，所有静态模型进入目录前经过真实 3D 详情页预览和资源指纹复核；异常旧输出放入可恢复隔离目录。
...
- 模型库分页会按当前窗口可用空间和网格列数自动展示完整行，宽屏不再只显示两三行；翻页、分类和窗口尺寸变化后仍保持稳定分页。
