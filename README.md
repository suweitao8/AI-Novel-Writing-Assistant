...
### 2026-08-30
- 通用资产的 HDRI 环境改用与场景资产一致的卡片浏览：卡片直接显示当前生效全景和状态数，点卡片即进入编辑；状态图生成后编辑器内出现「3D编辑」入口，可跳整页 HDRI 预览调整投射中心与半球直径。
- 通用资产的「编辑环境」与场景资产编辑器完全同款：状态列表、参考图、时代风格、时间天气、图片提示词和 AI 改写同一套交互，生成的全景自动用于模型与动画预览，可一键切换当前全景；生成失败后的「关闭失败提示」也已修复为只关闭当前显示的那次失败。
