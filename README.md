...
- 外观状态可以单独选时代风格：双穿或时代推进的故事里，同一个角色/场景/道具在不同时代各有自己的状态（现代形态、玄幻形态…）——给每个状态选好时代风格后，生成状态图时优先用它定画面氛围；不选时默认按「现代都市」出图，需要末世废土、东方玄幻等其他时代时直接在下拉里选择。跨时代的新状态默认参考上一状态的图，保持同一形象只换时代氛围。
