...
### 2026-08-30
- 通用资产的 HDRI 3D 预览修复黑屏，三套环境可正常查看；分镜、模型和动画预览继续使用角色阴影。
- 模型配置统一从「系统 → 模型设置」进入：配置一个文本模型并完成检测后，规划、正文、审校、修复等文字任务会自动使用该模型；创作过程中不再突然弹出全局快捷配置窗口，也不会拦截 AI 操作。
- 动画库修正站立待机、行走循环和坐姿循环的骨骼姿态与骨盆位移，角色动作不再保持 T 姿或出现异常深度偏移。
- 动画资源更新后会自动刷新动作缩略图，卡片预览与完整 3D 预览页使用同一套最新姿态。
- 场景 3D 编辑会根据当前状态全景图自动估算环境尺度与地面分界，并在缺少可靠尺度时使用 7.5 米圆半径、2 米投射中心的稳定默认值；已有场景会按原有实际尺度兼容读取。
- 场景 3D 环境统一使用 2.5–15 米圆半径，拖动半径时投射中心按比例跟随，分镜和场景预览使用同一套尺度。
- 角色和怪物状态的身高统一支持 0.5–10 米，5 米级怪物也能在角色卡片、自动构图和 3D 分镜中保持稳定比例，保存后再次打开仍保留身高基准。
- 用户手动保存的场景投射参数优先保留，换状态图后才重新分析；场景设置不再要求填写场景类型。
- 模型 3D 预览新增三套 HDRI 环境：室内客厅、中央广场和草地自然，可在预览环境中直接切换；半球直径默认 15 米，可在 5–30 米范围调节。
- 系统设置提供「通用资产」入口，统一展示旁白音色与三套模型/动画 HDRI 环境；每套 HDRI 都可以直接进入 3D 预览，半球直径调整会同步用于模型、动画和缩略图预览。
- 通用资产的 HDRI 3D 预览与漫剧场景编辑使用同一套场景编辑体验；对象列表只保留环境对象，投射中心和半球直径可直接调整。
- 通用资产列表支持 2D 全景图放大预览，并隐藏内部资源路径，让资产名称、全景图和 3D 预览入口更清晰。
- 模型旋转相机时，HDRI 环境保持世界空间位置不变，不随镜头漂移或动态缩放；环境边界和取景距离按当前半球直径约束。
- 模型卡片和动画卡片统一使用室内客厅作为默认预览环境，并刷新缩略图缓存，卡片效果更容易横向比较。
- 模型与动画的 3D 预览、卡片缩略图和漫剧场景统一使用固定在世界原点的半圆 HDR 环境；投射中心、半球直径和地面网格采用同一套规则，默认值为 2 米投射中心与 15 米直径。
- 分镜 3D 草图与动画库统一使用同一套 UAL2 角色骨架和动作资源，静态姿势选择依据真实片段，历史布局中的不支持姿势会安全回退。
- 动画卡片进入完整 HDR 3D 预览页后，可播放、暂停、拖动时间轴查看动作，并将任意一帧截图设为卡片预览图。
- 动画预览、动画卡片和分镜 3D 草图统一使用蓝色代理角色，旧的黄色预览截图会自动刷新。
- 修复模型、动画和分镜 3D 预览的 HDRI 半圆穹顶黑影：环境不再自投影形成黑块，角色阴影仍会保留在地面上。
