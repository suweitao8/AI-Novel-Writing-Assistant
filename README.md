...
- 角色支持登记别名/昵称：编辑角色的表单里新增「别名」——小说原文常用称呼指代角色（本名叶晨，正文里叫哥哥、晨哥），填进别名后，解析脚本、生成分镜都会把这些称呼归一到本名（脚本与画面按本名挂角色形象，称呼不会再对不上角色）；脚本正文里的别名也会按角色高亮。
...
- 漫剧「提取」里与已有资产同名的建议，卡片显示资产当前形象，弹窗直接展示已有资产完整内容（含状态图与音色），方便判断是否改名另建；重新生成的状态图在各处立即更新。
