...
### 2026-08-31
- 模型资源在进入模型库前会先生成真实 3D 预览并校验资源指纹；带透明材质的草、灌木和花叶会保留透明通道，贴图或 GLB 变化后旧预览不会继续误用。
- 原先出现贴图大色块的草模型已从发布目录撤下，替换为通过 GLB 清洗、透明材质检查和详情页预览的草丛模型；不合格候选会被隔离，不再混入模型卡片。
- 模型库名称以模型实际外观为准，新增模型先经过预览图复核；误标为“办公桌组”的模型已更正为“烟灰缸”。
- 模型卡片与详情页统一使用 45° 三分之四视角，主体约占画面 80%，不同尺寸的模型都能更稳定地完整展示。
- 模型详情首次打开会按实际显示区域自动取景，卡片缩略图按模型真实网格的投影范围重新构图并自动刷新，避免薄小模型显得过远或偏出画面。
- 模型和动画 3D 预览与卡片缩略图恢复完整 HDRI 环境补光，材质暗部保持清晰，同时继续显示柔和落地投影阴影；旧缩略图会自动刷新。
- 模型和动画卡片缩略图会显示清晰的落地投影阴影，卡片与详情预览保持一致的光影方向，旧缩略图会自动刷新。
- 模型和动画卡片缩略图只保留 HDRI、模型/角色和落地投影阴影，不再显示编辑器网格，旧缩略图会自动刷新。
- 模型库卡片缩略图改为轻量的 256px 级预览，并使用浏览器原生懒加载和异步解码；画面继续保持干净，不显示编辑器网格。
...
- 动画库现在统一展示 150 段 UAL2 动作：保留 46 段旧兼容动作，并加入 104 段通过 root-motion 轨道校验的 Cine57 动作；入口优先按「日常动作、日常互动、生活与表演、徒手战斗、武器战斗」浏览，旧动画收进兼容范围，不再给动作大类添加多余的来源前缀。
- Cine57 新导入动作只收录带有明确 root-motion 源证据且最终 GLB 含 root 平移通道的片段，目录名称按实际移动语义显示，原地动作不会进入分镜 3D 草图可用目录。
- 武器动作可以直接按剑、武士刀、刺剑、长枪与戟、双刃、弓箭、手枪、重锤、镰刀、匕首和法师武器筛选；徒手动作细分拳击、泰拳、咏春、忍者，怪物与人形生物动作按地面、爬行、飞行等语义归类。
- 动画入口按每页 24 段分页，只渲染当前页缩略图；来源和细分类压缩为两行筛选，选择分类或搜索后会回到第一页。
- 动画搜索框现在与来源筛选同排并置于右侧，点击“搜索”或按回车提交；搜索框旁不再显示单独的结果数量标签。
- 动画入口默认展示 104 段可用于分镜的 root-motion 动作，46 段旧动作归入“兼容动画”；可按套装、动作类型、姿态和武器继续筛选，分镜 3D 草图也会优先使用同一批可用动作。
...
- 动画详情页打开时会先停在该动作的默认代表帧，直接显示对应姿态，不需要先点击播放才能离开 T-pose；动画卡片缩略图也会在统一 HDRI 场景中重新生成。
...
- 蓝色代理角色的外层脖子按完整外轮廓保持与身体一致的蓝色，关节继续使用浅蓝色区分动作结构，切换视角时颈部结构保持清楚可见。
...
- 模型库只展示可直接摆放和交互的前景模型；UAL2 角色资源继续服务动画预览与分镜，不在模型库中提供角色预览。模型库和动画库支持按名称、文件名/片段名、分类、套装或动作类型搜索。
