...
- 漫剧工作室顶部整合：返回入口、作品名、阶段页签（小说/分镜/配音/视频）与小说子页签（大纲/细纲/设定）集中在顶栏，当前阶段的常用操作放在右侧。
- 漫剧小说阶段按「大纲 / 细纲 / 设定」组织：大纲页是带行号的编辑区（50 行起、回车加行、自动保存），页签行右侧「解析」让 AI 按大纲开始/继续逐章创作；逐章推进在顶栏「章节管理」——写当前章的大概故事，AI 展开成本章细纲节拍。
