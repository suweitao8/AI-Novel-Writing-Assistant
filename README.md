...
### 2026-09-03
- 工作台会按页面与项目记住上次查看的页签和筛选分类：返回模型、动画、小说/漫剧工作区、拆书、提示词和设置等页面时，会回到上次正在编辑的位置；带有效页签参数的链接仍按链接打开。
- 漫剧 3D 草图现在严格按每一镜的景别取景，近景和特写预览、保存草图都会使用场景摄像机的真实构图。
- 漫剧 3D 草图把编辑观察相机与分镜摄像机分开，重新构图和景深设置不会让主视口变模糊；右下角预览与最终草图使用同一台分镜摄像机。
- 漫剧 AI 自动构图会校验角色关系与上下姿势，血角兽等上方角色的承载关系、体量和低姿态代理会保持一致。
- 漫剧 3D 场景 HDRI 直接按世界方向采样原始全景源图，`.hdr` 与普通场景图使用对应颜色解码，环境细节更清晰。
- 模型素材导入增加透明通道校验与真实三维详情预览检查；草、树叶、花和灌木的镂空材质保留正确显示，无法通过材质或预览检查的候选不会进入可用模型库。
- 模型库第五批扩量：从废土与街景候选中筛选 95 个资产，最终可用目录达到 484 个精选静态模型（另含 1 个角色资源）；补齐废弃/末日场景道具（垃圾箱、瓦砾堆、锈桶、军用箱、旧家具、蛛网、旧书堆、破床垫）、后苏联街区生活件（阳台晾衣架、铁笼灯、伏特加瓶）、下水道与街区细节（通风管件、施工锥桶、报刊箱）和野花草丛植被，全部经过尺寸、材质、视觉审核与前景门禁。
- 模型库材质安全门禁收紧：无法解析真实材质回填的模型不再进入可用目录，模型详情不会再把导出占位色显示成整块黄色；原始资源保留在可恢复隔离清单中。
- 漫剧场景全景与前景模型分层：HDRI 只负责室内墙面、天花板、地板和室外远景；桌椅、书柜等可交互物改为从模型库独立添加到场景，并可在场景 3D 编辑器与分镜 3D 草图中继续摆放、调整和参与构图。
- 漫剧 3D 草图编辑器的滚轮缩放允许继续拉远查看完整场景；编辑视角不受 HDRI 半球边界截断，拍摄机位仍保持世界边界约束。
- 动画库卡片布局参考模型库优化：桌面端每行展示 10 张动画卡片，每页展示 50 张（5 行），浏览更多动作更紧凑。
- 动画导入流程改为四条攻击 smoke 验收：活动统一 GLB 现在只含 46 条 UAL2 内置基础动作与 Anim57 Attack 目录下的 4 条通过门禁的 root-motion 攻击；其它旧 Cine57 导入条目从活动目录移除并保存在外部运行备份中，后续按同一流程逐批恢复。
- 动画重定向管线补齐 Biped 命名兼容与逐帧骨骼校正；向前奔跑等后续动作恢复导入时，躯干、颈部、肩链和腿链会沿用同一套校正规则。
- 动画库的“徒手战斗”筛选现在只展示指定的 4 条测试动作，不会再混入旧的近战钩击和过肩投掷；徒手攻击预览的侧向扭转和脚底陷地问题也已修正，详情页会直接停在默认动作帧并自动刷新旧缩略图。
- 模型库与动画库分页布局统一：模型和动画卡片固定每行 10 个、每页 5 行，调整窗口宽度不会再改变每页行数；最后一页按剩余数量展示。
