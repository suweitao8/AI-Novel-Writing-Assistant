...
- 漫剧 3D 草图支持按镜头上下文自动规划出场角色、相机和景深，并在 PlayCanvas 预览中直接呈现规划结果；角色清单不一致时会阻止应用。
- 漫剧本地音频通道已恢复为 VoxCPM2：旁白、角色对白、状态音色和分镜配音统一使用本机 18761 语音服务，IndexTTS 2.5 暂时保留为显式兼容通道。
- 漫剧 3D 草图编辑器会在编辑后自动保存并自动确认，无需手动保存或确认；退出时会等待完成并立即刷新分镜预览，AI 图与场景图来源严格分开。
- AI 自动构图或草图自动保存进行中时，返回入口会暂时锁定，确保规划结果和最新草图完成保存后再离开。
- 重新设计旁白音色时，没有参考音频就会按新的描述重新生成，不会沿用上一轮试听音色，成年女性、成年男性等性别描述可以真正切换。
- 系统旁白的女性默认描述现在更强调自然亲和的女中音、真实讲故事感、清晰吐字和自然停顿，减少偏平或播音腔；描述仍可自行编辑。
- 漫剧 3D 草图编辑器退出时会自动保存并立即刷新分镜预览；AI 图与场景图来源严格分开，缺失或无效的 AI 图会提示重新生成。
- 从场景资产进入 3D 场景编辑后返回，会保留“资产 / 场景”页签位置，不再回到当前脚本页。
- 场景状态图片旁新增「3D编辑」，会打开当前状态自己的预览；2:1 全景图也会随投射中心高度和半球直径实时改变弧形地面。
- 3D 场景半球极点按各经度保留贴图坐标，地面与天空的三角扇不会把纹理拉成长条。
- 修复历史状态图在 3D 场景预览中因缺少制品指针而无法加载的问题；已有归属明确的图片会继续可用，不需要重新生成。
- 画风管理的通用美术风格统一为写实影视化：角色、场景和道具默认使用真实材质、自然光影与稳定的写实基线，不再回落到虚幻三维游戏渲染；现代、末世和年代等时代氛围仍可继续选择。
- 漫剧分镜页将批量入口收敛为一个「合成」按钮：点击后会并发补齐缺失画面和配音，准备完成后自动进入视频合成；逐镜生成入口继续保留。

- 漫剧 3D 草图的 HDRI 面板只保留投射中心高度和半球直径：高度默认 3、范围 1–10，直径默认 20、范围 10–50；水平旋转固定为 0，环境亮度固定为 1。
- 场景资产提供独立的「3D 场景编辑」入口：可以用约 1.8 米的比例参照角色校准场景尺度，并统一保存投射中心高度和半球直径，后续分镜自动继承场景参数。
- 普通场景图的半球地面使用连续弧形投影，调节尺寸或高度时不再出现圆形波纹；中键平移按屏幕方向工作，场景跟随鼠标拖动。
- 3D 草图会根据场景图上方亮部自动推断主光方向和色调，为角色代理提供基础方向光，无法读取图像时使用稳定的默认光向。
