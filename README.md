...
- 短剧工作台开放试用：可从已有小说、原创灵感或粘贴文本创建短剧项目，依次生成短剧策略、分集大纲和逐集台本，台本支持审查、合规检查与一键修复。
- 短剧角色可生成设计稿、头像与三视图立绘，分镜支持关键帧配图与视频提示词，分集内容与台本可导出为 Markdown/JSON；视频合成与配音默认为模拟联调通道，接入真实视频/配音服务后即可出片。
