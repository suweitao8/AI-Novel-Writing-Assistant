...
- 道具只留名字和画面提示词：道具表单、卡片与漫剧快捷新建都去掉了道具类型、持有者、重要度、剧情功能这些对生成画面没有作用的字段——一个道具就是名字加一句画面提示词（生成道具图直接用它）；旧道具的外观描述会自动带进画面提示词，不会丢。
...
- 地图改为三层：世界层摆国家的相对位置，点进国家看城市分布，再点进城市看具体地点；右侧列表随时切换层级的进入。地图编辑精简为画地形、添加当前层条目、点选编辑与保存。
- 「AI 标注场景」取代 AI 起草地点：AI 依据场景资产判断每个场景属于哪个国家哪座城市（缺的国家和城市会自动建好），直接放到地图上；没法定位的场景会标记「无法标注」，以后不再重复处理。地图上已有的内容不会被 AI 改动。
