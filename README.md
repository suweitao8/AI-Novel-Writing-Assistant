...
- 场景资产新增「识别空间」：从当前状态图识别床、桌、椅、门窗等固定物体，在场景 3D 编辑器和分镜 3D 草图中显示半透明空间标记，AI 自动构图也会参考这些标记安排角色位置。
...
- 状态图生成超时后可直接关闭失败提示，已确认图片、制品引用和重新生成入口继续保留；没有可用图片时回到普通缺图占位，即使新一轮生成出现相同错误文案，也不会被旧提示的关闭操作误清除。
- 漫剧 3D 草图编辑器在旋转、缩放和摆位时只更新当前编辑状态；点击返回分镜时才保存并自动确认，避免操作过程被保存请求打断。
...
- 漫剧 3D 草图编辑器退出时会保存最新布局并立即刷新分镜预览；AI 图与场景图来源严格分开，缺失或无效的 AI 图会提示重新生成。
- 漫剧 3D 场景地面按投射中心进行全景投影，修复中心环状纹理和经度接缝，调整半球参数后仍保持连续的地面贴图。
- 漫剧 3D 场景上下半球统一使用投影材质并沿用标准颜色空间输出，地面与环境的亮度保持一致；交界处连续采样全景图，减少半球接缝和贴图拉伸。
- 从场景资产进入 3D 场景编辑后返回，会保留“资产 / 场景”页签位置，不再回到当前脚本页。
- 角色、场景和道具资产页签在桌面端一行展示五张卡片，主视图突出方形图片、资产名称和状态；大纲侧栏继续保留紧凑信息布局。
- 角色、场景和道具状态图会按原图比例完整展示，避免固定容器造成两侧留白；场景状态的「3D编辑」与「生成图片」操作位于同一行，HDRI 背景会随投射中心高度和半球直径实时改变弧形地面。
- 3D 场景编辑器的右键水平旋转支持无缝跨过 0°/360° 接缝，拖动到边界不会停止。
- 场景 HDRI 全景图按中线划分环境与地面：上半部集中环境物体，下半部以连续地面为主，减少地面物体拉伸。
- 场景状态编辑器直接展示普通 2:1 图片，需要空间预览或摆位时从「3D编辑」进入独立编辑页面。
...
- 状态图重新生成失败时会保留最后可用预览，相关状态引用也会继续使用；制品异常时只从同一资产状态的历史图片恢复，不会串用其他资产图片。
...
- 漫剧 3D 草图的 HDRI 面板只保留投射中心高度和半球直径：高度默认 2、范围 1–10，直径默认 15、范围 10–50；水平旋转固定为 0，环境亮度固定为 1。
- 场景资产提供独立的「3D 场景编辑」入口：可以用实际约 1.8 米且位置固定的比例参照角色校准场景尺度，并统一保存投射中心高度和半球直径，后续分镜自动继承场景参数。
- 普通场景图的半球地面使用连续弧形投影，调节尺寸或高度时不再出现圆形波纹；中键平移按屏幕方向工作，场景跟随鼠标拖动。
- 3D 草图使用当前 HDRI 生成 PlayCanvas 环境光照，角色不再叠加独立方向光和补光；选中角色以绿色标记，场景比例参照角色保持约 1.8 米并固定位置。
- 修复 3D 场景 HDRI 上半球显示：上下半球统一进行投射中心投影，环境光照仍由 HDRI 提供，交界处和地面颜色保持连续。
