...
> = {
  ...
  semanticRetryPolicy: {
    ...
    buildMessages: ({ baseMessages, validationError }) => [
      ...
      new HumanMessage([
        ...
        "对于 on_top_of，subject 是上方主体，object 是下方承载者：object 贴地并使用 lying/prone，subject 默认使用 crouching 或 kneeling；如果动作明确要求 subject 伏在、趴在或压在 object 身上，可使用 prone 表示贴地的上方主动姿态；不要给 subject 使用 lying，因为那表示下方仰卧承载者；sizeRelation 必须表达真实体量关系。",
        "facing 表示 subject 面向 object；attacking、holding、following 表示 subject 对 object 做有方向的动作，也必须让 subject 面向 object。on_top_of/under 的上方主动方同样必须面向下方承载者；不要只依赖 yawDeg 猜测方向，关系端点必须准确。",
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "输出 actors 时必须使用输入名单中的全部角色，每个角色恰好出现一次，不得遗漏、改名、合并或创造角色；数组顺序只用于稳定回载，不代表关系主动方或上下层级；叙事焦点由 camera.focalCharacterName 声明，服务端围绕该主体取景。",
      ...
      "先从镜头动作中识别有方向的角色关系，再根据关系规划坐标、姿势和大小；relations 的 subject 是有向关系的主动/参照方，object 是被作用/承载方；仅在 on_top_of 中 subject 是上方主体。facing 表示 subject 面向 object；attacking、holding、following 也表示 subject 的动作目标，必须让 subject 朝向 object；on_top_of/under 的上方主动方必须面向下方承载者。服务端会把这些结构化关系落实为确定性 yaw，关系端点比模型自行填写的 yawDeg 更权威。",
      "on_top_of 表示 subject 位于 object 上方：object 必须是贴地的承载者并使用 lying 或 prone，subject 默认使用 crouching 或 kneeling；如果动作明确要求 subject 伏在、趴在或压在 object 身上，可使用 prone 表示贴地的上方主动姿态；不要给 subject 使用 lying，因为那表示下方仰卧承载者；不要把上下角色颠倒。under 表示 subject 在 object 下方。",
      "方向示例：如果动作同时出现“角色 A 躺在地面、角色 B 伏在 A 身前/压住 A/咬住 A”，必须输出 subjectCharacterName=B、objectCharacterName=A、relation=on_top_of，B 使用 crouching、kneeling 或 prone（贴地伏压时），A 使用 lying 或 prone；绝对不要因为 A 在 actors 数组前面就把关系写反。",
      ...
      "双人对话遵守 180° 轴线规则：输出 facing 关系让两人相向而立，服务端会把相机放在二人连线的同一侧让左右关系清楚；正在说话的角色面向听者；三人以上按主次分前中后层次，避免所有人并排一条直线。",
