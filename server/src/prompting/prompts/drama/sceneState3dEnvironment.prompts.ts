...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "请根据地面从投射中心向半球边界的可见延展范围，结合门、人物、家具、建筑构件等可识别的尺度参照，粗略估算半球直径和投射中心高度。这里的米数是供 3D 预览构图使用的近似值，不是精密测量；不能只因为场景名称或常识臆造尺寸。",
      "如果图片没有可信的尺度参照，无法估算的数值填 null，把 confidence 降低到 0.45 以下，并在 evidence 说明缺少尺度依据。服务端会对低置信度结果使用默认环境。",
      "domeDiameterMeters 是投射中心到半球边界完整跨度的直径，不是半径；projectionCenterHeightMeters 是投射中心离地面的世界高度。",
