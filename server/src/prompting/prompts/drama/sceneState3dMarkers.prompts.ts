...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      // 2026-08-28 起全景图只做背景：可移动家具通常不在画面里，识别以固定结构为主，
      // 绝不为看不见的家具编造标记；旧的全景图里若仍画有家具则照常识别。
      "全景图是纯背景画：床、桌、椅、沙发等可移动家具通常不在画面里（前景家具由用户单独摆放）；只标注画面中真实可见的物体，没有看到就返回更少的标记或空数组，不要按场景名称或常识脑补家具。旧图中若仍画有可移动家具则照常识别。",
      ...
      "穷尽式覆盖要求：按从左到右的顺序分段扫描整条 360° 水平视野，画面里每一件属于可用类别的可见固定物体都必须各有一个 marker——背景全景常见的固定结构组合如 门+窗户+楼梯+固定柜体 需要全部出现，同类多个实例分别输出独立 marker 并用不同 label 区分（例如 窗户1、窗户2）；被其他物体部分遮挡但仍能辨认轮廓的固定物体也要标注，只有完全无法辨认是什么的碎片才跳过。输出前逐类别清点一次数量，画面里可见的宁可多标不可漏标；不可见的家具绝不为凑数而编造。",
