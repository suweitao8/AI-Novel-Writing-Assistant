...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "size 只填写按人物尺度估算的近似米制尺寸；服务端会结合 imageRegion 的可见跨度和 kind 类别范围校准最终长方体大小和贴面厚度。框必须完整覆盖物体的可见主体：矩形应把整个物体框进去，可以稍微超出 5% 左右留出边距，但不要把大块墙面、地面或天空一起框进去；门的框底部必须贴住门底与地面的接触线；家具的框要包含腿脚或落地部分；漏掉物体的任何主体部分都会让标记盖不住它。",
