...
// v14（2026-08-23）：props 加复用度硬门槛——行李箱/糖果零食/银行卡这类只在个别分镜
// 用一下的通用日常物品不再提取（用户实测第一章提出一堆低复用道具：换故事也一样用、
// 单独建形象没有价值）；只收反复出现或直接推动关键情节的贯穿性道具。
// v15（2026-08-23）：场景条目新增结构化 sceneType（interior=室内/exterior=室外/
// nature=自然）——用户实测提取出的场景应用后「场景类型」是空的，得手动补；
// 与 timeOfDay/weather 同规则：按原文形态推断，推不出填 null。
// v16（2026-08-23）：资产首状态统一改名「默认」（原「初始状态/初始形象」），
// stateSwitches 起始标记与提取提示词的措辞同步（数据层由 normalize 就地归一）。
...
// 场景条目在通用条目之上多三个结构化字段：类型、时间与天气（影响场景图取景与光线氛围）。
const referenceParseSceneSchema = referenceParseItemSchema.extend({
  sceneType: z.enum(["interior", "exterior", "nature"]).nullable().default(null),
...
export interface ChapterReferenceParsePromptInput {
  ...
  /** 角色与其已登记的外观状态，格式如「李火旺：正常、重伤、癫狂」（名单第一个是默认状态）；未登记的角色不出现 */
...
export const chapterReferenceParsePrompt: PromptAsset<ChapterReferenceParsePromptInput, ChapterReferenceParseOutput> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "- 长相与穿搭按角色各自设计（2026-08-23 用户要求）：全书会生成很多角色，不同角色必须明显区分，禁止都长成同一张标准帅哥/美女脸。原文写明的长相穿着照原文；原文没写或写得笼统的，按角色在原文里表现出的性格、年龄与身份（学生/职业/阵营）推测设计——脸型、眉眼、鼻型、下颌线条、发色发型、肤质体态各给具体值，可加标志性小细节（痣、疤、耳饰、眼镜、发卡等）。好看程度按戏份伸缩：主角与核心角色明显好看有魅力，普通配角端正顺眼但不惊艳，任何角色都不要难看。",
      "- appearance＝外貌体型一句话：体型（高瘦/娇小/壮实/魁梧）、发型发色、五官特点、穿着、标志性特征——性别与年龄段已在结构化字段里，不在此重复（2026-08-20 起 physique/personality 不再单列：做视频只关注画面与音色提示词，属性从简）。原文没写的部分按上一条设计原则补全具体特征，不写「长相普通」这类空话。",
      "- imagePrompt＝图片提示词（角色形象，中文，80～150 字）：只写「这个人长什么样」——性别年龄段、发型发色、五官特点（脸型/眼睛/肤色，如 黑色短发、瓜子脸、丹凤眼）、体型、服装配饰、气质神态，照着就能画出这个人。穿搭按性格与身份设计出有风格的搭配，写具体到单品与配色、与本书其他角色错开——女生可按角色走 地雷系（黑紫暗色、蕾丝蝴蝶结、厚底鞋、双马尾或挑染）、乖巧系（浅色连衣裙/白裙、针织衫、柔顺直发）、运动系、学霸系（衬衫马甲、眼镜）、通勤简约、街头潮、国风、哥特、Y2K 等路线，男生可选 机能、运动、商务通勤、学院、工装、街头、复古 等，不写「穿着时尚」这类空话。画风、背景、视图、时代氛围一律由系统统一管理，提示词里禁止出现：画风/风格/渲染类词（写实、动漫、CG、3D、虚幻引擎、高清等），背景类词（纯白背景、白底、透明背景等），视图/构图规格（全身像、四视图、特写、正面等），时代/画风氛围词（末世风格、末世感、玄幻氛围这类——生成时按所选时代风格自动注入，写了反而互相打架）；也不要写动作场景。",
      "- voicePrompt＝音色提示词（中文，30～60 字）：音高（低沉/清亮）、音质（沙哑/柔/冷）、说话气质（如 急躁少年音/疲惫沙哑的中年男声），按角色身份与言行推测。",
      "scenes＝场景地点（每个独立空间都算）：name＝具体地点名——与 segments 里用的 scene 名逐字一致，带归属或特征限定（如 叶城大学宿舍，不写 卧室 这类通用词，一般 4～12 字），已有的 existingScenes 名单优先沿用；description＝环境特征一句话（列表摘要用）；sceneType/timeOfDay/weather 是结构化字段：类型按 interior（室内）/exterior（室外，如 街道、楼顶、庭院）/nature（自然风光，如 旷野、森林、海边）输出，时间按 morning（早上）/noon（中午）/night（晚上）输出、天气按 sunny（晴天）/cloudy（阴天）/rainy（雨天）输出——按这个场景在原文里的常态推断（一个场景多次出现就按主要时段），原文写不出就填 null；imagePrompt＝图片提示词（场景环境，中文，60～120 字）：光线、空间结构、材质与氛围——类型、时间与天气已走结构化字段，不在此重复；画风与时代氛围由系统统一管理，禁止出现画风/风格/渲染类词（写实、动漫、CG、3D 等）、画幅/构图规格（全景、360 度等）与时代/画风氛围词（末世风格、玄幻氛围这类——生成时按所选时代风格自动注入）。",
      "props＝贯穿剧情的重要道具，复用度是硬门槛（武器/关键信物/核心剧情物品）：只收后续还会反复出现、或直接推动关键情节的物品；只在个别分镜用一下的通用日常物品一律不收——行李箱、背包、糖果零食、银行卡/现金、手机、普通衣物餐具这类，换个故事也一样用，单独建形象没有价值（v14 用户要求，宁缺勿滥，本章确实没有就返回空数组）。name＝物品名；description＝用途与来历一句话；imagePrompt＝图片提示词（道具实物，中文，40～80 字）：只写这一件道具本身——材质、颜色、造型比例、工艺细节、磨损痕迹、表面文字图案等，写具体写满，它是成图的唯一主体；道具周围的环境与其它物品（抹布、桌面、地面、附近杂物等）一律不写，它们属于场景；背景与画风由系统统一管理，禁止出现背景类词（中性背景、白底等）、画风/风格类词、视图规格（45 度透视等）与时代/画风氛围词（末世风格、玄幻氛围这类——生成时按所选时代风格自动注入）。",
      "worldview＝世界观条目：力量体系、金手指/系统、势力组织、关键规则、时代背景等（name＝条目名，description＝一句话说明），不需要提示词。",
      "每类上限：characters 16、scenes 16、props 12、worldview 16；原文里确实没有的类别返回空数组。外观状态不在解析环节生成（用户手动管理），提示词一律写资产当前的默认形象。",
