...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "4. 每个角色都必须输出 gender、actorKind、bodyBuild；gender 允许值只有 male、female、other、unknown，actorKind 允许值只有 human、monster、other、unknown，bodyBuild 允许值只有 slender、standard、broad、unknown。",
      "5. castRole 只能使用：protagonist, antagonist, ally, foil, mentor, love_interest, pressure_source, catalyst。",
      "6. 每个角色必须输出 personality、background、development、actorKind、bodyBuild，不得只给 shortDescription。",
...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "4. 每个角色都必须有 gender、actorKind、bodyBuild；gender 允许值只有 male、female、other、unknown，actorKind 允许值只有 human、monster、other、unknown，bodyBuild 允许值只有 slender、standard、broad、unknown。",
...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "1. 所有展示文本必须改写为自然简体中文。",
      "2. 保留原有语义、关系含义和角色功能，不得改变设定逻辑。",
      "3. castRole、gender、actorKind 和 bodyBuild 枚举值必须保持原样，不得翻译或改写。",
...
> = {
  ...
  render: (_input, context) => [
    new SystemMessage([
      ...
      "2. 每个候选都必须输出 gender、actorKind、bodyBuild；gender 拿不准时填 unknown，actorKind 和 bodyBuild 也只能使用结构化枚举值，不得省略。",
