...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "角色：随机起一个符合题材与世界气质的中文姓名（不与已有角色重名）；给出性别、年龄段（child/youth/middle/elder）、身份定位、体型、性格（含说话方式与行动倾向）、外貌、默认着装、背景，并给出默认状态的 voicePrompt（中文，描述音高、音质、说话气质）。",
      "角色的 facePrompt 是角色立绘的面部锚点，必须按「[性别]，[年龄段]，[发型发色]，[眼睛特征]，[肤色]，[脸型]」模板写纯面部特征，禁止出现任何服装信息。",
      "场景：给出场景名、类型（interior 室内/exterior 室外/nature 自然）、时间（morning/noon/night，可为空）、天气（sunny/cloudy/rainy，可为空）、氛围概述、故事作用，以及 environmentPrompt——一段 220～320 字的完整空间环境描述，覆盖正面/左侧/右侧/背面的可见布局、光源与材质风格，不含人物与临时道具；缺失的信息要合理补全，不要写「未提及」。",
