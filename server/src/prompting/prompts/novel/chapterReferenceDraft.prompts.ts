...
const referenceDraftSegmentSchema = z.object({
  ...
  /** 美术风格切换：从这一格起改用新画风（名字必须来自 artStyles 名单）；不切换为空串 */
  styleSwitch: z.string().max(30).default(""),
...
export interface ChapterReferenceDraftPromptInput {
  ...
  /** 可选美术风格名单（第一个是本书默认风格）：剧情画风转变时从名单里选名字切换 */
  artStyles?: string[];
...
export const chapterReferenceDraftPrompt: PromptAsset<ChapterReferenceDraftPromptInput, ChapterReferenceDraftOutput> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "styleSwitch 是美术风格切换：只有剧情发生明确的画风转变才填（如末世爆发、进入幻觉/异世界、时代整体跳转），风格名必须从 artStyles 名单里逐字选（名单第一个是本书默认风格，通常不需要切换）；没有转变、或没有名单时一律留空串。切换后一直保持新风格，不要反复来回切，也不要微调措辞——初稿会按它生成「【风格：…】」标记，后续画面与视频按它换画风。",
