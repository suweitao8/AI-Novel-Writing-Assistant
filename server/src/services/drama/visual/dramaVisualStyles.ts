...
export const DRAMA_VISUAL_STYLE_PRESETS: DramaVisualStylePreset[] = [
  ...
  },
  {
    ...
    // 破败脏旧只施加在场景与道具等环境上（2026-08-23 拆分）：污渍/血渍/磨损是通用的
    // 角色状态属性，跟着外观状态走（战斗后带血、逃亡带尘土都建对应状态），时代风格
    // 不再自动把角色弄脏——否则末世书里所有角色无条件全身脏旧，干净状态做不出来。
    styleInstructions:
      "末世废土氛围：低饱和色调、尘土灰褐、硬朗自然光；风化、开裂的混凝土、锈蚀金属、剥落的油漆、疯长的植被与文明崩溃后的实用生存细节施加在场景与道具等环境上，不自动施加到角色身上——角色身上是否有污渍、血渍、磨损、尘土，只由角色资料与当前状态描写决定。不覆盖明确写出的崩溃前、当代或古代细节。",
    avoidInstructions:
      "崩溃场景中不要崭新奢华或光鲜时尚的基调。不主动给角色添加污渍与破败，角色身上的脏旧只在角色资料或状态明确描写时出现。不要文字、水印或标签。",
