 ...
 *
 * Background-only rule (2026-08-28/29 product decision): the panorama is a pure
 * BACKGROUND layer. Beds, tables, chairs and other movable furniture are
 * foreground props placed separately as interactive 3D models in the scene
 * editor, and the same applies to near-field outdoor rocks, grass and bushes,
 * so generation must keep all of them out of the painting in any zone — even
 * when the scene description mentions them.
 *
 ...
 * - v=0.3-0.5  far background band: fixed architecture and distant scenery only;
...
export const SCENE_PANORAMA_LAYOUT_PROMPT_LINES = [
  ...
  // 2026-08-28/29 产品方向：全景退化为纯背景，桌椅床等家具与室外近景石块、草丛、
  // 灌木一律不入图（场景文案提到也不画）；前景道具改为后续摆放可交互 3D 模型，
  // 避免背景里长死道具、角色无法与道具交互。
  "background-only panorama: this image is a pure backdrop for a 3D scene that gets its foreground props placed later as separate 3D models, so the panorama itself contains no placeable props in any zone — no beds, tables, chairs, sofas, desks, cabinets, shelves, counters, rugs with objects, and no near-field rocks, stones, grass tufts, bushes, shrubs, plants, logs, crates or ground clutter, even when the scene description mentions them",
  // 2026-08-26 用户反馈室内家具仍大量越过 50% 线：给模型一个物理上可执行的逃逸规则——
  // 装不下就画小画远，而不是跨界；物体最低点（含接触阴影）以中线为硬顶。
  "hard middle-line ceiling: the lowest point of every object that remains in view — wall bases, window sills, building feet, tree trunks and their contact shadows — must stay above v=0.5; when perspective would push any part of a structure below the middle of the image, redraw the whole room smaller and farther away instead of crossing the line",
  "middle distant zone v=0.3-0.5 holds only the far background of the space: indoor far walls with windows, doors, stairs and other fixed architecture, outdoor skylines, mountains, distant tree lines and far buildings, all small, distant and fully contained between v=0.3 and v=0.48 with a clean safety margin; bodies, legs, feet and hard fragments must not enter the center band or the ground zone, and nothing stands in the foreground or on the ground",
  ...
  "background layer allowed content: fixed non-interactive surfaces and architecture such as walls, ceilings, floor or terrain materials, built-in doors and windows, wall-mounted non-interactive decor such as posters, paintings, clocks or mirrors, distant facades, mountain ridges and far tree lines may be rendered when supported by the scene context; these are background elements, not placeable foreground props",
 ...
 * band plus a separate flooring swatch below. Since 2026-08-28/29 the backdrop shows
 * fixed décor only; movable furniture never enters the panorama, not even small
...
export const SCENE_PANORAMA_INTERIOR_PROMPT_LINES = [
  ...
  // 2026-08-28/29 起背景板只画固定装修；可移动家具是前景道具，一律不入画。
  ...
  "the room is completely unfurnished: no beds, tables, chairs, sofas, desks, cabinets, shelves, counters, stoves, rugs with objects or any other furniture or standing props anywhere on the backdrop or the floor, not even small pieces against the far wall, even when the room description names them — an empty-looking room is the correct result; furniture is placed later as separate 3D models in the scene editor",
...
export const SCENE_PANORAMA_LAYOUT_NEGATIVE_PROMPT =
  "any furniture anywhere in the image, beds, tables, chairs, sofas, desks, cabinets, shelves, counters, furniture silhouettes or furniture shadows on the backdrop or floor, near-field rocks, stones, boulders, grass tufts, bushes, shrubs, potted plants, logs, crates, ground clutter props, scattered debris on the ground, furniture or objects in the lower half, furniture painted or cloned on the floor, furniture legs below the horizon, skirting board or wall base in the lower half, furniture legs crossing the horizon, oversized foreground furniture, close-up furniture crossing the middle of the image, furniture bottoms cropped by the horizon, objects crossing the center line, object fragments in the center safety band, split furniture, structure tops or distant objects above the sky line, objects crossing the sky boundary, visible horizon line, seam, stripe, split-screen, collage, stretched props on the ground, cluttered floor, repeated ground objects, loose props and clutter, unprompted framed portraits, extra photographs beyond the described one, many framed photos on one wall, photo collage wall, frameless loose photos";
