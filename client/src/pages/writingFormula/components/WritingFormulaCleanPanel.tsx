  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-5">
        {selectedProfile ? (
          <div className="rounded-2xl border bg-muted/40 px-4 py-3 text-sm leading-7 text-foreground">
        ...
        <div className="space-y-4 rounded-2xl border p-4">
          <div className="space-y-1">
            <div className="text-base font-semibold text-foreground">当前会优先参考的反 AI 约束</div>
          ...
          {antiAiRuleNames.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {antiAiRuleNames.map((ruleName) => (
                <div key={`${selectedProfile?.id}-${ruleName}`} className="rounded-full border bg-muted/40 px-3 py-1 text-sm text-foreground">
                  {ruleName}
                </div>
              ))}
            </div>
        ...
        <div className="space-y-4 rounded-2xl border p-4">
          <div className="space-y-1">
            <div className="text-base font-semibold text-foreground">检测正文</div>
            <div className="text-sm leading-6 text-muted-foreground">
          ...
          {detectionReport ? (
            <div className="space-y-3 rounded-2xl border p-4 text-sm">
              <div className="space-y-1">
                <div className="font-medium text-foreground">风险分：{detectionReport.riskScore}</div>
                <div className="leading-6 text-muted-foreground">{detectionReport.summary}</div>
              </div>
              <div className="space-y-2">
                {detectionReport.violations.map((item, index) => (
                  <div key={`${item.ruleId}-${index}`} className="rounded-xl border p-3">
                    ...
                    <div className="mt-1 text-xs leading-6 text-muted-foreground">{item.reason}</div>
                    <div className="mt-2 whitespace-pre-wrap rounded-lg bg-muted/40 px-3 py-2 text-xs text-foreground">
                      {item.excerpt}
                    </div>
                  </div>
                ))}
          ...
          {rewritePreview ? (
            <div className="space-y-2">
              <div className="text-sm font-medium text-foreground">修正结果</div>
