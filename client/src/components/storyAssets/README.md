...
- `StoryAssetPreview.tsx`：统一默认状态图片的方形容器、角色最左视图的等比例居中裁切、场景/道具居中裁切，以及缺图/加载失败占位。
- `StoryAssetCard.tsx`：统一左图右信息的卡片结构和点击打开行为。

卡片预览是展示投影，不写入数据库或生成新的派生图片。角色使用默认状态四视图板的最左视图，按源图比例放大并在方形框内垂直居中；场景和道具使用默认状态图的水平居中方形。
