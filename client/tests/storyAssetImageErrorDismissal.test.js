...
test("客户端提供状态图失败提示的关闭 API", () => {
  ...
  assert.match(apiSource, /expectedAttemptId\?: string/);
  assert.match(apiSource, /error: expectedError/);
  assert.match(apiSource, /attemptId: expectedAttemptId/);
});
...
test("状态编辑器的关闭动作只针对失败提示，且有键盘可达的可访问名称", () => {
  ...
  assert.match(editorSource, /selectedImageAttemptId/);
  assert.match(editorSource, /expectedError/);
  assert.match(editorSource, /expectedAttemptId/);
});
