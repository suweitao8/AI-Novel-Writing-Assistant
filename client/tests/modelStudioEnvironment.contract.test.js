...
test("HDRI 预览页复用共享环境运行时并提供完整直径交互", () => {
  ...
  assert.match(previewAppSource, /buildBlocking3dGroundGridLines/);
  assert.doesNotMatch(previewAppSource, /setupStudioLighting/);
});
...
test("纯 HDRI 预览关闭空阴影接收器，场景预览仍保留阴影路径", () => {
  ...
  assert.match(previewAppSource, /enableShadowCatcher:\s*false/);
  assert.match(runtimeSource, /enableShadowCatcher:\s*options\.enableShadowCatcher/);
});
