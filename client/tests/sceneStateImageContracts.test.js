...
test("场景状态编辑器只展示静态状态图，3D效果通过独立编辑入口进入", () => {
  ...
  assert.match(assetFormsSource, /fit="natural"[\s\S]*className="w-full rounded-lg border-0"/);
  ...
  assert.doesNotMatch(assetFormsSource, /<div className="flex justify-end pt-2" role="group" aria-label="场景 3D 操作">/);
  assert.match(assetFormsSource, /className="flex flex-wrap items-center gap-2" role="group" aria-label="状态图片操作">[\s\S]*3D编辑[\s\S]*重新生成图片/);
});
