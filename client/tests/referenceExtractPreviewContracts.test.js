...
test("提取资产卡使用共享方形预览并让世界观保持纯文字", () => {
  ...
  assert.match(source, /const existingPresentationFor = \(group: ExtractGroup, name: string\): StoryAssetPresentation \| null =>/);
  ...
  assert.match(source, /const existingPresentation = existing \? existingPresentationFor\(group, item\.name\) : null/);
  assert.match(source, /<StoryAssetPreview[\s\S]*preview=\{existingPresentation\?\.preview \?\? null\}/);
