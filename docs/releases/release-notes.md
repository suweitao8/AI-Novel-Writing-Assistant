...
- 场景 3D 编辑和分镜 3D 草图现在会按浏览器高度铺满工作区；右侧拆分为「场景对象」和固定高度的「属性面板」两区，对象项只显示图标和名称，长内容只在对应面板内滚动。HDRI 对象显示为「世界」，尺度校准人物显示为「参考角色」，已识别的空间标记会直接列在对象列表中；对象区约占右侧三分之一，为属性编辑留出更多空间。
