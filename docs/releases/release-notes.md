...
- 漫剧每个分镜都可以在「摆位草图」中用场景 360° 全景摆放本镜角色：拖动调整场景视角与角色位置，支持大小、左右翻转、前后层级和重新加入角色；保存后可随时继续调整。
- 漫剧分镜新增独立「3D 摆位台」：可在带网格的真实 3D 视口中加入、选择和拖动角色，调整相机、旋转、缩放与上下位置，并切换站立、坐着、躺着、趴着等代理姿势和动作播放。
- 3D 摆位保存后会同时保留可继续编辑的空间快照与摆位 PNG；确认后仍会把 PNG 锁定为分镜画面生成的首位参考图，已有二维草图也可以继续使用。
- 3D 摆位 PNG 按严格 16:9 的 1280×720 开发基准捕获，保存和确认期间会锁定编辑，避免空间快照与分镜参考图来自不同状态。
- 确认摆位草图后，分镜画面生成会把它作为锁定的第一张参考图，以保持场景视角和角色相对位置；尚未确认的草图不会消耗图片额度或参与批量生成。
