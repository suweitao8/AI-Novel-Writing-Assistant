...
- 漫剧小说阶段按「大纲 / 细纲 / 设定」三个子页签组织，写作以逐章推进为主：大纲页是代码编辑器式的大纲编辑区（默认铺满 50 行编号空行、行号固定在最左侧且只读、软换行、当前行高亮、回车加行、修改后自动保存），右侧是固定在原位、内部滚动的设定资产面板：角色、场景、道具卡片按最近更新排列，工具栏左边新增、中间搜索框、右边搜索按钮，新增走弹窗（选类型、填名称和一句话说明），创建的名字会自动在大纲正文里高亮，三类用不同颜色区分；子页签行右侧的「解析」按钮让 AI 按大纲开始或继续逐章创作；细纲页可逐章手写分章剧情（可增删调序，确认后作为剧情契约）；设定页管理角色、场景、道具与世界观。写到当前这一章时，在顶栏「章节管理」写下本章的大概故事，AI 会展开成这一章的细纲节拍，跟着节拍写正文。
