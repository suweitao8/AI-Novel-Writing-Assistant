...
- 参考小说按章节自动对应：上传的参考小说会按「第一章/第二章…」标题自动切分，工作室第 1 章的参考、解析、提取只用参考小说第 1 章的内容，不再把后面章节的剧情带进来；本章在参考小说里没有对应章节时会明确说明，并可粘贴自己的文本。
- 漫剧「参考」页签看得更明白了：本章还没有参考文本时，改为只读展示对应的参考内容（解析、提取直接可用），想单独编辑可以一键「复制为本章参考」或从空白粘贴新文本——不会再出现把参考内容当成空输入框、粘贴后叠出好几份重复内容的情况。
- 「提取」「解析」按钮不能点时直接在按钮旁说明原因（如「还没有参考内容」），提取页会写明这次用的是本章参考文本还是参考小说对应章节，提取中也有「十几秒」的耗时提示。
- 「参考」页签编辑时能看到自动保存状态（保存中/还有未保存的修改），与初稿页签一致。
