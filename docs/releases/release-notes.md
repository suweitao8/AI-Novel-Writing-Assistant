...
- 漫剧工作室顶部整合为一条工具栏：返回漫剧列表、作品名、居中的阶段页签（小说/分镜/配音/视频）与小说阶段的子页签（大纲/细纲/设定）都在顶栏，当前阶段的常用操作（章节管理、同步最新章节、打开分镜/视频工作台等）集中在顶栏右侧，AI 写作进度也在这里实时显示。
- 漫剧小说阶段改为按当前章创作：顶栏章节按钮显示当前章（如「1 · 重生」），点开即可切换章节或新建；「大纲」页签写当前章的故事（代码编辑器式、默认铺满 50 行编号空行、行号只读、自动保存），右侧设定面板支持搜索与新增、名字在大纲里高亮；「细纲」页签查看与修改本章的 AI 节拍（3～10 拍，逐拍可改可增删）；页签行右侧「解析」按本章大纲生成本章细纲；让 AI 开始或继续整本创作从章节管理弹窗进入。
