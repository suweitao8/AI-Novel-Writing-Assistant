...
- 漫剧工作室页签重组：顶部页签改为项目级的「当前 / 资产 / 设定」；「当前」下的子页签（初稿 / 正文 / 分镜 / 配音 / 视频）都作用于当前章——初稿写本章故事（代码编辑器式、行号只读、默认铺 20 行编号空行、自动保存），正文查看与修改 AI 解析的节拍（3～10 拍逐拍可改可增删）、AI 写出正文后直接阅读本章成稿，分镜/配音/视频按当前章推进。「设定」页签集中放画面风格等项目配置；「资产」页签查看角色、场景、道具与世界观；顶栏右侧是当前章按钮（点开章节管理切换或新建）与 AI 写作进度。
