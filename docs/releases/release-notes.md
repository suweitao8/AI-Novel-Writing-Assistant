...
- 漫剧工作室顶部整合为一条工具栏：返回漫剧列表、作品名、居中的阶段页签（小说/分镜/配音/视频）与小说阶段的子页签（大纲/细纲/设定）都在顶栏，当前阶段的常用操作（章节管理、让 AI 开始/继续创作、同步最新章节、打开分镜/视频工作台等）集中在顶栏右侧，切换阶段不用来回找按钮。
- 漫剧小说阶段按「大纲 / 细纲 / 设定」三个子页签组织：大纲页写简略大纲、保存后一键让 AI 推理分章细纲；细纲页集中查看和修改 AI 的分章细纲（全书梗概与逐章标题、梗概可改可排序，确认后作为剧情契约交给 AI 写作）；设定页管理角色、场景、道具与世界观。工作室里的阶段性说明文案同步精简。
