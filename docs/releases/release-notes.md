...
- 场景空间识别直接把当前状态图交给视觉模型，并保留每个标记的图像证据区域；空间标记按全景图里的实际物体区域和投射中心重新估算桌、椅、床、门窗等位置与尺寸，墙面物体稳定贴近对应外侧墙面，单张全景图无法确定的绝对深度仍以构图用的近似距离处理。
- 桌子、椅子、床等地面标记会沿图片中的水平位置落到半球可用地面外圈，不再集中在球体中心；修改投射中心高度或半球直径后，旧标记会自动进入重新识别状态，不会把旧坐标带进 3D 预览和分镜摆位。场景 3D 编辑器会显示投射中心的高亮蓝色线框及高度线，线框缩小且取消实体填充，不遮挡 HDRI 原图。
- 场景 3D 编辑器会根据场景默认状态的类型自动采用合适的默认环境参数：室内投射中心高度 1 米、半球直径 8 米；室外 1.7 米、10 米；自然 1 米、20 米。半球直径可在 5–30 之间调整，场景资产预览与分镜 3D 草图使用同一范围；同一场景的其他状态共享这组环境参数，手动调整过的参数会继续保留。
- 场景全景图生成统一按固定构图分区：下半区（50% 以下）只保留连续干净的地面，远山、天际线和高大物体集中在地平线以上的远景带，再往上（70% 以上）只保留天空；中心过渡带保持空净，物体不再跨越地平线。场景 3D 编辑器与分镜 3D 草图不提供可调地面分界，历史参数按 50% 兼容读取。
- 场景状态编辑器的全景平面图显示两条构图参考线：50% 位置是地平线（地面与远景分界），70% 位置是远景与天空的分界，生成后可直接对照检查画面分区。
- 角色进入分镜 3D 草图时会根据年龄、体型、外貌等已有设定推断近似身高并保存为角色级比例基准，高大角色、儿童等在不同镜头里保持稳定的相对大小；角色资产卡片会自动显示这份分镜比例基准，从参考文本提取并应用的角色也使用同一份身高档案。
