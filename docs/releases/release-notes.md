...
- 角色身上的污渍、血渍、磨损改由外观状态决定：末世废土等时代风格只负责时代氛围和环境破败（废墟、锈蚀、荒芜的场景照常呈现），不再自动把每个角色都弄得又脏又旧——角色干不干净看角色资料和当前状态怎么写，没写就默认干净整洁。想要战斗后带血、逃亡后满身尘土的画面，建一个对应的外观状态把细节写进去即可；换时代风格重新生成时服装设计仍会按新时代明显改变。
