...
- 角色基础资料更清爽：基础信息只保留姓名和性别，年龄、外貌、画面提示词与音色提示词统一放进角色状态；新建角色会自带一个初始状态，旧角色也会自动整理出初始状态，已有图片和音色不会丢。
- 新增角色状态时只需填写状态名和一句变化描述，年龄段与图片参考默认沿用上一状态；音色可以直接沿用，也可以按需要生成新的音色。高级图片提示词按需展开，初始状态不可误删。
