...
- 场景 3D 编辑器会根据场景默认状态的类型自动采用合适的默认环境参数：室内投射中心高度 1 米、半球直径 8 米；室外 1.7 米、10 米；自然 1 米、20 米。投射中心高度可在 0.5–2 米之间调整，半球直径可在 5–20 之间调整，场景资产预览与分镜 3D 草图使用同一范围；同一场景的其他状态共享这组环境参数，手动调整过的参数会继续保留。
- 场景全景图生成统一按固定 50% 中线切分：中心区域保持空净，家具和高大物体完整位于上区，下半区只保留连续地面；场景 3D 编辑器与分镜 3D 草图不再提供可调地面分界，历史参数按 50% 兼容读取。
