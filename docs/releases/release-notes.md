...
- 角色、场景和道具资产卡统一使用左侧方形预览：角色展示四视图板左上格，场景与道具展示默认状态图的水平居中内容；没有图片时保留统一占位。
- 修复资产预览裁切与串图：角色预览保持源图比例并在方形框内垂直居中，场景和道具取水平居中的方形内容；不同资产即使使用同名「默认」状态，也会各自保存和读取图片，不再把一个角色的图片显示到另一个角色上。
