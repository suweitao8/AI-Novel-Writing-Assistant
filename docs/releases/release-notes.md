...
- 室内场景的默认 3D 环境参数调整为投射中心高度 0.5 米、半球直径 5 米：室内空间较小，调小默认值后角色模型在预览和分镜构图里的比例更自然；未手动改过参数的存量室内场景会自动采用新默认，手动保存过的参数保持不变。
...
- 修复分镜 3D 摆位的舞台边界圈位置错误：此前把“半球直径”数值当成半径计算，边界圈画到了半球外面、角色站位限制也随之过远；现在边界圈正确画在半球边缘向内 1 米处（真实半径减 1 米），角色活动范围与穿模防护同步修正。### 2026-08-26
