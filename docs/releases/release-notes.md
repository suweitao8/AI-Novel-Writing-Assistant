...
- 美术风格独立成页签：整本画面有了明确的画风体系。默认风格点选即存——新增「3D写实电影」（虚幻引擎写实质感，类似《黑神话：悟空》）作为全类型默认，另有动漫、3D玄幻国漫、写实现代、写实末日、写实古装、民国年代共 7 种内置；还可以给这本书自定义多套画风（例如「现代诡异」「末世爆发后」），风格名在初稿里直接可用。
- 初稿支持画风与角色形象切换：剧情画风转变时（末世爆发、进入异世界）解析自动插入「【风格：风格名】」，角色受伤、变身、换装时插入「【角色状态：角色：状态】」（优先用设定里登记过的状态名）；与「【场景：…】」同一套机制——切换后一直生效、可以反复切换，编辑器里风格行偏紫、角色状态行偏琥珀加粗显示。
...
- 初稿支持场景切换标注：解析自动按剧情在换场处插入「【场景：地点】」行（地点名优先对齐设定中心的场景），编辑器里换场行偏绿加粗、一眼可见；后续生成分镜与视频时按它切换到对应场景。手写一行「【场景：客厅】」也一样生效。
