...
- 漫剧「参考」页签看得更明白了：本章还没有参考文本时，改为只读展示整本参考小说（解析、提取直接可用），想单独编辑可以一键「复制为本章参考」或从空白粘贴新文本——不会再出现把整本小说当成空输入框、粘贴后叠出好几份重复内容的情况。
- 「提取」「解析」按钮不能点时直接在按钮旁说明原因（如「还没有参考内容」），提取页会写明这次用的是本章参考文本还是整本参考小说，提取中也有「十几秒」的耗时提示。
...
- 参考小说不再丢失：每章的参考正文随章节自动保存，重开页面后仍在；某章没有自己的参考文本时自动展示整本参考小说，新建章节的标题也会按参考小说对应章节预填。
