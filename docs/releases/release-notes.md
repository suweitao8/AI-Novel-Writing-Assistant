...
- 漫剧小说阶段按「大纲 / 细纲 / 设定」三个子页签组织，写作以逐章推进为主：大纲页是代码编辑器式的大纲编辑区（行号固定在最左侧且只读、软换行、当前行高亮，默认 50 行起、回车加行，修改后自动保存），子页签行右侧的「解析」按钮让 AI 按大纲开始或继续逐章创作；细纲页可逐章手写分章剧情（可增删调序，确认后作为剧情契约）；设定页管理角色、场景、道具与世界观。写到当前这一章时，在顶栏「章节管理」写下本章的大概故事，AI 会展开成这一章的细纲节拍，跟着节拍写正文。
