...
- 室内场景的全景图生成追加专门约束：墙面、门窗和床桌椅柜全部贴在地平线以上的视平带，墙地交界压在地平线上，下半区只出纯地板材质，减少家具被画进地板、3D 摆位时地板上长出桌椅的问题。墙上默认只生成海报、画作、挂钟、镜子等媒体装饰；场景描述里明确提到的照片（如老照片）会带相框出现在墙上，描述未写数量时最多一张，不会额外铺开成照片墙。
