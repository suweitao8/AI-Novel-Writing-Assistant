...
- 场景空间标记会按全景图里的实际物体区域和投射中心重新估算桌、椅、床、门窗等位置与尺寸；墙面物体会稳定贴近对应外侧墙面，修改投射中心高度或半球直径后，旧标记会自动进入重新识别状态，不会把旧坐标继续带进 3D 预览和分镜摆位。
- 桌子、椅子、床等地面标记会沿图片中的水平位置落到半球可用地面外圈；修改投射中心高度、半球直径或全景地面分界后，旧标记会自动要求重新识别，避免家具和门窗坐标漂移。
