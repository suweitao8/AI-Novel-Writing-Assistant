...
- 分镜列表右侧统一放置「生成缺失首帧」「生成缺失配音」「全部重新配音」「合成」等操作：按当前章节把镜头合成为横屏视频，实时显示进度，完成后可直接预览成片并下载字幕。
...
- 分镜合成改为静态首帧画面：整镜保持同一张横屏图片，不再使用推拉、平移等运镜；字幕和音频仍按配音时间轴同步。
- 修复长镜头整集合成可能停在渲染中的问题：横屏 16:9 的 720p 开发视频现在可以继续完成渲染、混流并生成字幕文件。
- 漫剧成片字幕统一改为无底板白字，并使用柔和的模糊阴影提升复杂画面上的可读性。
- 分镜首帧、视频提示词和整集视频统一使用横屏构图；配音页面取消通道选择，直接使用系统设置里配置的音频模型，整站配音口径一致。
