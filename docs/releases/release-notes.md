...
- 分镜 3D 草图和场景 3D 编辑中，选中角色、怪物或参考角色会显示贴合模型真实轮廓的绿色外描边，不再显示矩形包围盒；导出的 3D 草图图片不会带上编辑器选中标记。
...
- 场景 3D 编辑和分镜 3D 草图现在会按浏览器高度铺满工作区；桌面端左侧集中显示返回入口、场景对象和属性内容，右侧保留完整 3D 视口，对象项只显示图标和名称，HDRI 对象显示为「世界」、尺度校准人物显示为「参考角色」、已识别的空间标记会直接列在对象列表中，列表与属性内容分别在各自区域内滚动。选中可移动角色时，属性区域会显示位置、旋转和大小，视口同步显示绿色外轮廓高亮。
...
- 桌子、椅子、床等地面标记会沿图片中的水平位置落到半球可用地面外圈；即使家具被固定 50% 全景中线限制在上半区，也不会再集中在球体中心。
...
- 场景全景图生成统一按固定 50% 中线切分：中心 `v=0.48–0.52` 保持空净，家具和高大物体完整位于上区，下半区只保留连续地面；场景 3D 编辑器与分镜 3D 草图不再提供可调地面分界，历史参数会按 50% 兼容读取。
