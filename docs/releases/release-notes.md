...
- 漫剧「提取」建议与已有资产同名时，卡片直接显示该资产当前形象缩略图，应用弹窗先展示已有资产的完整内容（基础信息、外观状态与已生成的图片、试听音色），一眼看出重名的是哪一个角色/场景/道具，再决定要不要改名另建；还没生成过图的资产保持原样显示类型图标。资产重新生成状态图后，各处展示会立即显示最新图片，不再停留在旧图。
