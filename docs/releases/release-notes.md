...
### 2026-08-27
- 3D 编辑器视口对齐 Unity 场景视图：左上角新增「移动 / 旋转 / 缩放」工具条，选中角色或空间标记后视口内出现彩色三轴手柄，直接拖拽即可调整位置、朝向和大小（导出的摆位草图不包含手柄）；分镜草图的角色支持像 Unity Transform 一样直接键入位置 / 旋转 / 缩放数值（原方向按钮网格改为数值与手柄），场景编辑器里的空间标记拖拽或改数值后都会自动保存。
