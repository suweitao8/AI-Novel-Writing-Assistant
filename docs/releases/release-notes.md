...
- 3D 草图和场景 3D 编辑中的选中角色、怪物或参考角色改用贴合模型真实轮廓的绿色外描边，不再显示矩形包围盒和脚下选择圆盘；导出的 3D 草图图片不会带上编辑器选中标记。分镜 3D 草图还支持调整选中角色的代理模型颜色，颜色实时更新并随布局保存。
...
- 场景 3D 编辑器的比例参照角色统一为约 1.7 米并放在场景中心；选中角色和比例参照使用绿色外轮廓高亮，代理角色使用当前 HDRI 环境光进行明暗表现。
...
- 漫剧 3D 草图的选中角色使用绿色外轮廓高亮，不再显示脚下选择圆盘，方便辨认角色位置而不遮住地面。
...
- 场景 3D 编辑器的比例参照角色统一为约 1.7 米并放在场景中心，使用绿色外轮廓高亮，代理角色使用当前 HDRI 环境光进行明暗表现。
