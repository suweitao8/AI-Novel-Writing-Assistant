...
- 模型 3D 预览与模型卡片增加环境补光，投影阴影改为更柔和的高质量过滤，减少暗部黑块和阴影锯齿，让材质细节与模型落地关系更清晰。
- 通用资产中的中央广场 HDRI 预览默认将投射中心设为半球圆半径的 10%（15 米直径对应 0.75 米），3D 编辑页仍可调整投射中心、半球直径和分界线。
- 动画库统一收录 UAL2 内置动作与 Cine57 导入动作，共 448 段（46 段旧动画与 402 段虚幻导入动作），可按来源、套装和动作类型筛选浏览。
...
- 动画库现在按「旧动画」和五个虚幻动作大类分开浏览；虚幻资产继续按具体套装和动作类型细分，日常、互动、拳击、剑术、武器战斗等动作可以直接筛选。
- 虚幻动画目录新增 402 个经过骨架兼容性校验的代表动作；同一套装的重复非待机动作只保留一个，待机动作保留多个自然变体，旧动画与新动作仍共用同一蓝色 UAL2 角色。
