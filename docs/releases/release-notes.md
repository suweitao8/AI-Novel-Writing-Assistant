...
- 状态编辑改成直接编辑：角色/场景/道具的外观状态所有字段当场就能改（状态名、状态变化、图片提示词、音色提示词、年龄段/时间/天气），改完点「保存状态」即可；点「生成图片/生成音色」也会先把未保存的修改自动存好再生成，不用来回点。「添加状态」挪到状态列表底部，区块标题去掉，图片/音色提示词支持多行书写。
...
- 漫剧「提取」建议与已有资产同名时，卡片直接显示该资产当前形象缩略图；点开弹窗就是编辑这个已有资产——姓名/性别/别名和状态列表载入的都是已有数据，状态列表里能看到已生成的图片与音色（没生成过也能当场点「生成图片/生成音色」），改完点「保存」更新该资产，这条建议随即从列表移除。还没生成过图的资产卡片保持原样显示类型图标。资产重新生成状态图后，各处展示会立即显示最新图片，不再停留在旧图。
