...
- 角色状态编辑改成左右面板：左侧切换状态，右侧查看图片、基础描述、图片提示词和音色状态；可单独生成/重新生成状态图片与试听音色。新状态默认沿用上一个状态的音色，剧情进入老年、受伤等需要变声的阶段时可以改为生成新音色。
- 场景参考图严格按空环境生成：人物、动物、怪物等叙事主体只转成爪痕、血迹、破坏痕迹等环境线索，不再把角色画进场景图；角色、场景、道具状态编辑统一为紧凑左右布局，状态列表独立滚动，图片区域不会撑出大块空白。
