...
- 漫画与短剧的图像资产统一为固定规格：角色设计稿改为四视图（正面、45°、侧面、背面全身 + 面部特写），场景参考图改为 360° 全景横图，武器道具等资产改为单张透视的横版设计稿（服装多视图横向并排）；各生图场景画幅统一为「设计参考类横版、分镜首帧/封面竖版」并集中管理。已生成的旧图不受影响，重新生成即按新规范输出。
