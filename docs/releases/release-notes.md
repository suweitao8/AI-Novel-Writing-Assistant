...
### 2026-08-30
- 通用资产的 HDRI 环境改用与场景资产一致的卡片浏览：每套环境是一张资产卡片，卡片直接显示当前生效的全景（生成图优先，未生成时显示内置环境预览）和状态数，点卡片即进入编辑，不再有单独的「编辑环境」「3D 预览」按钮；状态图生成后编辑器内出现与场景资产相同的「3D编辑」入口，可跳整页 HDRI 预览调整投射中心与半球直径。
- 通用资产的「编辑环境」弹窗与场景资产编辑器完全同款：状态列表、参考图、时代风格、时间与天气、图片提示词和 AI 改写使用同一套交互；环境状态可以用 AI 把提示词改写成想要的氛围（例如夜晚下雨），生成的全景自动成为模型库与动画库预览使用的 HDR 环境，并可一键切换哪个状态作为当前全景。
