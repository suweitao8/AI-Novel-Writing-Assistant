...
### 2026-09-03
- 漫剧 3D 草图的 AI 构图会根据结构化角色关系校正主动方朝向：攻击、面对、持有、跟随以及“位于其上方”的角色会朝向目标角色；缺少专用躺/趴动作时，预览保留贴地姿势语义，不会静默变成站立。
- 模型库中的草、花、树叶等透明镂空模型现在从正反两面完整显示，详情预览、模型卡片和分镜前景使用一致的显示效果。
- 分镜 3D 草图与 Anim57 徒手攻击预览统一使用 UE5 原生 Manny / Quinn 角色资源；系统会按角色性别、角色类型和体型自动选择男性标准体型或女性纤细体型，也可以在分镜中手动指定模型。
- Anim57 徒手战斗目录现在严格保留指定的 4 条攻击动画；Manny 与 Quinn 共用同一套 `SK_Mannequin` 原生动作，不再经过跨骨架重定向，进入 3D 草图时会直接使用站立基础姿势。
- 模型导入颜色外观门禁收紧：GLB 内嵌的 1×1 Base Color 占位图不再被当作真实贴图；没有可验证颜色来源的 17 个灰模已从可用目录移出，当前目录保留 445 个精选静态模型和 1 个角色资源，源模型与拒绝原因写入可追溯历史台账，后续同源指纹不会重复转换。
- 漫剧 3D 草图把编辑观察相机与分镜摄像机彻底分开：点击重新构图或调整景深时，主视口保持清晰；右下角取景和最终 PNG 使用同一台分镜摄像机，导出时才按设置启用景深后处理，对象列表也明确显示「分镜摄像机」。
- 漫剧 AI 自动构图增加角色上下关系校验：当结构化关系与角色姿势互相矛盾时，系统会按姿势关系归一化承载者、上方主体和体量，血角兽等上方角色会保持在正确位置和低姿态代理动作。
- 漫剧 3D 场景可见 HDRI 直接从原始 2:1 全景源图按世界方向采样，保留源图清晰度；`.hdr` RGBE 与普通场景图分别使用对应的颜色解码，环境光照仍沿用同一张源图。
- 漫剧 3D 场景修复 HDRI 地面投影：普通生成全景图的下半部不再作为透视纹理重新展开，避免远距离观察出现同心圆和漩涡；编辑器使用完整水平地面与外圈垂直圆墙，地面保留与源图匹配的稳定色，天空和远景继续按 HDRI 方向采样，模型库前景模型与角色可正常接地并接收阴影。
- 工作台会按页面与项目记住上次查看的页签和筛选分类：返回模型、动画、小说/漫剧工作区、拆书、提示词和设置等页面时，会回到上次正在编辑的位置；带有效页签参数的链接仍按链接打开。
- 页签链接带有失效的 `tab`、`view` 或 `stage` 参数时，会稳定回到对应页面的默认页签，不会被其他页面的旧记忆覆盖。
- 漫剧 3D 草图现在严格按每一镜的景别取景：近景、特写会保持对应的主体距离和画面占比，打开已有布局或保存草图时不会被编辑器总览视角覆盖。
- 模型素材导入增加透明通道校验与真实三维详情预览检查；草、树叶、花和灌木的镂空材质保留正确显示，无法通过材质或预览检查的候选不会进入可用模型库；已替换的 54 个有损透明贴图输出移到仓库外的可恢复隔离清单。
- 模型库前景策展与导入保护：从可用目录移出 22 个已确认不适合前景的碎屑、草类和过小模型，保留源产物到可恢复隔离位置；当前目录为 462 个精选静态模型（另含 1 个角色资源）。模型导入新增历史台账，按源包路径、Mesh 和源指纹记录每次处理结论，未变化且已拒绝的模型会在转换前自动跳过；新候选必须先生成方形详情预览并通过尺寸、材质、请求和视觉门禁才能发布。
...
- 动画导入流程改为四条攻击 smoke 验收：活动统一 GLB 现在只含 46 条 UAL2 内置基础动作与 Anim57 `/Game/Characters/Mannequins/Anims/Unarmed/Attack` 下的 4 条通过门禁的 root-motion 攻击；其它旧 Cine57 导入条目从活动目录移除，源导出、中间产物和旧发布包保存在外部运行备份中，后续按同一流程逐批恢复。
- 动画重定向管线补齐 Biped 命名兼容与逐帧骨骼校正；向前奔跑等后续动作恢复导入时，躯干、颈部、肩链和腿链会沿用同一套校正规则。
- 动画库的“徒手战斗”筛选现在只展示指定的 4 条测试动作，不会再混入旧的近战钩击和过肩投掷；旧动作仍保留在内置动画范围内。
- 徒手攻击预览修正了不同骨架脊柱层级造成的侧向扭转，并按整段蒙皮几何校正脚底接地；打开动作详情会直接停在默认动作帧，继续使用蓝色角色和中央广场 HDRI，旧缩略图会自动刷新。
- 模型库与动画库分页布局统一：模型和动画卡片固定每行 10 个、每页 5 行，调整窗口宽度不会再改变每页行数；最后一页按剩余数量展示。
### 2026-09-02
- 漫剧视频阶段采用三栏预览布局：中央区域专注播放横屏成片，左右栏分别承载合成设置、状态反馈和成片信息；桌面端预览空间更高，窄屏自动按视频优先顺序排列。
- 漫剧工作台成为网站统一入口：桌面端和移动端导航聚焦漫剧、模型、动画、任务、资料与系统设置，旧小说、独立漫画、创作中枢和拆书地址继续兼容跳转但不再占用产品入口。
- 知识库收敛为漫剧脚本与资产设定的资料管理，移除已退役工作区的继续创作按钮和链接；模型库、动画库、任务记录与系统设置保持可直接进入。
- 漫剧成片支持一键下载：视频阶段的固定信息栏新增「下载视频」入口，点击即保存 mp4 成片，与下载字幕、新窗口打开并列。
- 女性动捕系列动画修复：女性行走、站立待机、坐姿待机等 5 条动捕动画此前一直保持静止站姿，现在正常播放源动作的完整姿态；动画库新增全库动作质量巡检，逐条比对源动画与库内动作的肢体轨迹。- 行走姿态整体修正：重定向管线补上胸腔指向校正（源骨架多三节脊柱导致的弓背）、手臂改锚胸腔并按臂长比例解算、肢体接近伸直时的肘膝抖动抑制——后退行走这类动作不再弓背缩手、四肢不协调。
...
- 漫剧 3D 草图的 AI 构图支持俯仰机位：镜头动作里写明俯拍、居高临下、上帝视角，或仰拍、低机位、高大压迫等语义时，构图会按对应机位取景——俯拍视线向下压、主体落画面上三分显弱势；仰拍视线向上抬、主体落画面下三分、体量更突出；没有明确俯仰语义的镜头保持平视。对已有分镜重新生成 3D 草图即可套用。
...
- 3D 分镜编辑器的取景窗口不再被后台数据刷新打断：AI 构图完成后到保存之前，窗口保持当前摆位不重置，手动调整的站位和机位同样得到保留。
...
### 2026-09-01
- 手指动作修正：导入动画的手指重定向加入阻尼——战斗待机等握拳动作不再把低模手的手指折进掌心穿模成一团，呈现为自然的半握姿势。
- 模型库第三批扩量：从虚幻资产池精选 53 个新模型入库，补齐酒吧娱乐（台球杆架、啤酒牌挂钟）、餐饮商业（快餐点餐牌、中餐菜单牌、饮料机）、家居细节（壁挂鹿头/鱼标本、壁镜、风铃、吊挂植物、吸顶空调、CRT 电脑）等场景道具，全库达到 261 个精选模型；全部经过尺寸、材质、视觉审核与前景门禁。
- 模型库的搜索框和搜索按钮移到顶部导航「AI 实况」左侧，与动画库共用同一位置；筛选区整行只保留分类，全部分类以胶囊形式随窗口宽度自动换行，宽屏一行放下、窄屏自动折行，模型卡片的展示空间更充裕。
- 全站操作提示统一改到屏幕右下角展示：成功、报错等所有模块提示共用同一位置和同一卡片样式，不再出现在右上角遮挡顶部内容。
- 手部接触类动作修正：重定向流程新增双手接触校正与逐条质量门禁——鼓掌、抱臂、祈祷、助威这类双手要碰到一起的动作，现在手会真实合拢到源动画的间距（实测与源差距 1 厘米内）；已发布的全部导入动画都用新管线重新生成，凡双手接触达不到的动作会被门禁拦下。
- 动画库筛选统一为一套分类：去掉与分类重复的「动作分类」下拉，全部、内置动画和待机、移动、坐姿、日常、互动、徒手战斗、拳击、剑术、武器战斗、魔法、受击 / 闪避、翻越 / 攀爬、睡眠、表演 / 手势、生物动作、潜行、配对互动等分类以胶囊形式多行铺开展示，点击即筛；搜索框和搜索按钮移到顶部导航「AI 实况」左侧，筛选区整行留给分类。
- 动画库聚焦质量优先的动作：页面现在只展示运动（移动）和生活表演两类导入动画，加上原有内置动画；存在穿模等质量问题的战斗、互动等其余分类暂时下架，待逐类跑通后再重新上架。
- 挠头等角色动作的手到位修正：重定向后的动作现在保留源动画里手与头部等关键接触点——挠头待机的手会真实挠到头顶，不再停在肩膀附近；动作导入管线对手部可达性加了自动校验，达不到接触位置的动作不会被放行动画库。
...
- 修复模型库与动画库卡片预览图一直转圈的问题：窗口被遮挡或切换到后台时缩略图生成不再卡死，回到页面后会继续补齐缺失的预览图；个别模型生成失败也不会拖住整个库的预览图。
- 模型库的批量资产继续统一为现代写实方向，新增模型必须通过来源、材质、缩略图和质量检查后才会进入目录，未经审核的其他风格模型不会直接展示。
- 模型详情页新增“删除模型”并提供二次确认；确认后模型会从目录隐藏，但模型文件和已有分镜引用会保留，误隐藏时可以恢复。
- 全站创作工作台统一为冷墨暗色的分层面板视觉：顶部导航、侧栏、移动端导航、卡片、按钮、输入框、页签和弹窗使用一致的材质、圆角、边框、焦点反馈与动效节奏。
- 首页、漫剧、小说、模型/动画相关入口和系统设置沿用同一套页面层级；桌面与移动端保持原有导航和工作流，内容区域更紧凑，窄屏滚动与安全区也保持可用。
- 动画库入口卡片收敛为缩略图、动作名称、分类信息和帧数，移除卡片内重复的播放图标与用途徽标；整张卡片仍可直接进入 3D 预览，顶部用途筛选继续保留。
- 修复模型和动画预览中 HDRI 画面与角色受光、地面阴影方向相反的问题；可见 HDRI、环境光照和方向光现在共享同一方位，离开预览后会恢复原场景方向，已有卡片缩略图会自动刷新。
- 动画、分镜和角色缩略图中的蓝色代理角色会让颈部保持与主体一致的蓝色，仅用淡蓝色区分关节区域，画面更简洁。
- 模型库扩充到 208 个可直接摆放的现代前景模型，重点补齐家具、收纳容器、厨房餐具与设备、办公电子、卫浴和灯具，并保留少量户外与日用模型。
- 新增模型沿用 Cine57 的统一现代写实风格，按真实尺寸、地面/墙面/台面/天花板摆放方式和中文用途说明进入模型库，便于直接用于住宅、办公室、商业空间和日常生活场景。
- 动画库的分镜可用范围改为原地播放目录：目前保留 277 段通过 In Place 来源和 GLB 根位移门禁的 Cine57 动作，明确 Root Motion 或整体位移过大的候选不会进入分镜主库；46 段旧 UAL2 动作继续放在兼容动画范围。
- 动画导出增加 3 厘米级全局根位移检查和手臂骨链姿态检查，向前慢跑等动作在预览和分镜中保持原地摆位，明显漂移、手部爆开或骨链断裂的产物不会发布。
...
- 动画预览新增“循环播放”开关；循环播放跨过动作边界时帧轴会回到第 0 帧并继续同步，关闭后播放到最后一帧会自动暂停。
...
- 模型库首屏会优先准备屏幕附近的卡片预览，滚动到后续区域后自动继续加载，减少打开页面时大量卡片同时等待的时间。
- 模型库按每页 24 个模型浏览，搜索或分类后自动回到第 1 页；当前页附近的预览优先准备，减少首次打开和切换结果时的等待。
- 深色主题下卡片、弹窗、下拉菜单和提示气泡的投影不再发白：阴影基色按亮暗主题分别取色，暗色界面恢复干净的描边面板质感。
- 提示条与状态标识的文字底色对比度优化：模型设置等页面的状态提示在深色主题下改用主题色文字配半透明底色，文字不再和底色糊在一起；全站同类浅色提示样式统一补齐深色适配。
- 漫剧列表改为纯方形卡片网格：项目卡片以方形展示，「创建漫剧」入口是网格首位同尺寸的方形卡片，点击直接打开创建弹窗。
- 全站暗色样式统一走查：提示条、章节编辑器标注、时间线、角色与封面弹窗等 50 余处浅色写法补齐深色适配，角色关系图主角高亮改用暗色形态，角色图片与封面弹窗回归统一深色面板，报错与成功提示条在深色下不再出现白底。
- 角色卡片的头像预览改为按图内容自适应取景：正面头部完整框住眉眼到下巴，不再把侧面视图、邻格内容或四视图之间的白色分隔线裁进画面。
- 漫剧列表卡片用场景图片做封面：卡片直接展示项目场景的画面（方形居中取景），没有生成过场景图的项目保持原文字样式；在漫剧工作台的「设定 · 通用」里可以把封面换成指定场景，默认用第一个有图的场景。
### 2026-08-31
- 模型进入素材库前增加真实 3D 预览和资源指纹校验；草、灌木、花叶等带透明材质的资产会保留透明通道，资源发生变化时必须重新复核预览。
- 移除出现贴图大色块的草模型，换入通过几何清洗、透明材质检查和详情页预览的草丛模型；质量不达标的候选会隔离，不会出现在模型卡片中。
- 模型库名称改为以模型实际外观为准：新增模型先经过标准预览图复核再进入目录，避免把文件名误当成物体名称；已将一项实际为烟灰缸的模型正确标注为“烟灰缸”。
- 模型卡片和详情页统一使用 45° 三分之四视角，主体在画面中约占 80%，不同尺寸和长宽比的模型都能更稳定地完整展示。
- 模型详情首次打开会按实际显示区域自动取景，卡片缩略图按模型真实网格的投影范围重新构图并自动刷新，避免薄小模型因保守包围盒或画布默认比例显得过远、偏出画面。
- 修复模型贴图在 3D 预览中近距离查看时被过度压缩的问题：岩石、木纹、砖石等细节恢复清晰，模型卡片也会自动刷新为贴图更新后的预览。
- 模型库 3D 预览的滚轮缩放改为按模型实际尺寸自适应：大模型可以继续拉远，小模型可以继续拉近；HDRI 半球直径不再限制相机取景范围，聚焦和复位视角也会同步适配模型尺度。
- 模型 3D 预览与模型卡片恢复完整 HDRI 环境补光，并继续使用更柔和的高质量投影阴影，避免模型暗部失去光照，同时保持材质细节与模型落地关系清晰。
- 模型和动画卡片缩略图现在会显示清晰的落地投影阴影，卡片与点开后的详情预览使用一致的光影方向；HDRI 背景构图保持不变，旧缩略图会自动刷新。
- 模型和动画卡片缩略图只保留 HDRI、模型/角色和落地投影阴影，不再把编辑器网格带入最终图片；旧缩略图会自动刷新。
- 模型库卡片缩略图改为轻量的 256px 级预览，并使用浏览器原生懒加载和异步解码；画面继续只保留 HDRI、模型和投影阴影，不显示编辑器网格。
- 通用资产中的中央广场 HDRI 预览默认将投射中心设为半球圆半径的 10%（15 米直径对应 0.75 米），3D 编辑页仍可调整投射中心、半球直径和分界线。
- 动画库统一收录 UAL2 内置动作与 Cine57 导入动作，共 150 段（46 段旧兼容动作与 104 段通过 root-motion 轨道校验的动作），入口优先按日常动作、日常互动、生活与表演、徒手战斗和武器战斗浏览，旧动画收进兼容范围。
- Cine57 新导入动作只进入通过 root-motion 源证据和最终 root 平移通道双重校验的目录，目录名称按实际移动语义显示，原地动作不会进入分镜 3D 草图可用动作。
- 修正导入动画的站立与行走姿势，手臂不再贴近错误的 T 形基准；动画预览、动画卡片和分镜 3D 草图继续使用同一个蓝色 UAL2 角色。
- 蓝色代理角色在分镜草图、动画预览和动画缩略图中使用浅蓝色关节区分动作结构，抬臂、弯腰和屈膝等姿态更容易辨认。
- 蓝色代理角色的外层脖子按完整外轮廓保持与身体一致的蓝色，关节继续使用浅蓝色区分动作结构，正面、侧面和背面查看时都能清楚看到颈部结构。
- 动画缩略图会复用同一份角色动画资源，并在资源更新后自动刷新，卡片画面与完整预览保持一致。
- 动画卡片会按每段动作总帧数的 50% 自动截取默认预览图；完整预览页使用整数帧浏览和设置卡片预览帧，打开时直接停在同一中间帧。
- 模型库扩充到 79 个可直接摆放和交互的前景模型，新增家具、容器、厨房餐具、书籍办公、玩具装饰、户外和卫浴道具；自然物细分为石头、灌木、树木、草、花和盆栽，查找更清楚。
- 模型库的「全部」视图现在按用途分类连续展示，分类数量、模型详情入口和现有模型资源保持不变，查找不同类型的道具更直观。
- 食材/纸箱只保留两个代表模型，重复变体不再挤占模型库；小摆件、雕像、奖杯和花瓶统一归入「玩具/装饰品」。
...
- 模型库隐藏角色模型预览，只展示可直接摆放和交互的前景模型；动画预览与分镜继续使用 UAL2 角色资源。
- 模型库和动画库新增按名称、文件名/片段名、分类、套装或动作类型搜索，搜索结果可与现有分类筛选组合。
- 模型详情页提供只读 3D 预览：显示顶点数量和包围盒的长宽高，画面中同步标出包围盒；环境与模型变换由系统统一管理，详情页只保留相机查看操作。
- 模型详情页的包围盒默认隐藏，查看尺寸时可勾选“显示包围盒”查看灰色线框，取消勾选即可恢复干净的模型预览。
- 模型库为每个模型补充可读取的使用说明：卡片直接标出地面、水平面、墙面或天花板摆放方式，详情页会显示定位基准、朝向要求和具体安装提示；时钟、宫灯等方向性模型可以按说明进入后续分镜摆放。
...
- 动画入口的第二行直接提供规范化细分类：武器可按剑、武士刀、刺剑、长枪与戟、双刃、弓箭、手枪、重锤、镰刀、匕首和法师武器筛选；徒手可按拳击、泰拳、咏春、忍者筛选。
- 怪物、幽灵、僵尸和可复用人形骨骼的生物动作按怪物动作、生物地面动作、生物攻击、爬行、坐姿、躺卧和空中动作归类；没有明确资产证据时不会虚构狼人等角色分类。
- 动画入口每页显示 24 段，只挂载当前页缩略图；来源与细分类合并为两行横向筛选，搜索或筛选后自动回到第一页，旧动画与新动作仍共用同一蓝色 UAL2 角色。
- 动画搜索框与来源筛选同排靠右显示，点击“搜索”或按回车后才提交搜索；搜索框旁不再显示单独的结果数量标签。
- 动画入口默认聚焦 104 段可直接用于分镜的 root-motion 动作；46 段旧动作单独放入“兼容动画”，也可切换到“全部”检查完整目录。套装、动作类型、姿态和武器都可以继续精确筛选。
- 分镜 3D 草图会优先使用动画目录中对应的真实移动片段，旧布局仍可继续使用兼容动作；待机、行走、跑动、蹲伏、互动、徒手和剑术的动作选择与动画库保持一致。
- 修复模型和动画预览中 HDRI 偶发不显示的问题：主预览与模型/动画卡片的默认预览图都会显示统一的中央广场 HDRI，避免出现黑色网格背景。
- 3D 预览与卡片缩略图会在 HDRI 和模型资源准备完成后再生成画面；切换页面、重复打开或生成失败重试时会释放旧渲染上下文，减少 HDRI 再次丢失的情况。
- 动画详情页打开时会直接停在动作的默认代表帧，进入页面即可看到对应姿态，不需要先点击播放；动画卡片缩略图会在统一 HDRI 场景中重新生成，避免继续显示黑色旧图。
- 模型、动画和漫剧预览会在显示 HDRI 穹顶前完成首帧渲染准备，首次打开时背景、环境光和高光方向保持稳定。

### 2026-08-30
- 编辑环境的预览区不再空白：还没有生成全景的状态会直接显示内置中央广场全景，生成后自动替换，和资产卡片、3D 预览看到的画面保持一致。

- 模型与动画的 3D 预览统一使用中央广场 HDRI 环境：模型编辑器不再提供环境选择，室内客厅与草地自然两套预览环境已下线；通用资产页只保留中央广场一张环境卡片，可以在里面继续维护状态、提示词和生成全景。

- 修复模型与动画预览切换到草地自然等高饱和 HDRI 环境时画面整体发白的问题：实时预览的色调映射与漫剧场景、HDRI 预览页统一，同一套环境在不同页面颜色一致。
- HDRI 环境按应用方向使用：室内场景用室内客厅、城市户外用中央广场、纯自然户外用草地自然，由使用场景选择对应环境，环境内不再有「设为当前全景」的切换；每套环境的生效全景就是它的默认状态生成图，其余状态（如夜晚、雨天）作为可生成的备选变体。
- 通用资产的 HDRI 环境改用与场景资产一致的卡片浏览：每套环境是一张资产卡片，卡片直接显示生效的全景（生成图优先，未生成时显示内置环境预览）和状态数，点卡片即进入编辑，不再有单独的「编辑环境」「3D 预览」按钮；状态图生成后编辑器内出现与场景资产相同的「3D编辑」入口，可跳整页 HDRI 预览调整投射中心与半球直径。
- 通用资产的「编辑环境」弹窗与场景资产编辑器完全同款：状态列表、参考图、时代风格、时间与天气、图片提示词和 AI 改写使用同一套交互；环境状态可以用 AI 把提示词改写成想要的氛围（例如夜晚下雨），生成的全景自动成为对应方向使用的 HDR 环境。
- 修复环境状态生成失败后「关闭失败提示」不生效的问题，并且只有当前显示的那次失败会被关闭。
- 通用资产的 HDRI 3D 预览使用整页编辑布局：3D 视口铺满整个工作区，与漫剧场景 3D 编辑保持一致的布局和操作，可以在更大的画面里切换环境、调整投射中心、半球直径和分界线。
- 通用资产的 HDRI 3D 预览修复黑屏：纯环境预览不再被空阴影接收器压黑，室内客厅、中央广场和草地自然三套环境都能正常显示；分镜、模型和动画预览仍保留角色阴影。
- 模型配置统一从「系统 → 模型设置」进入：配置一个文本模型并完成检测后，规划、正文、审校、修复等文字任务会自动使用该模型；创作过程中不再突然弹出全局快捷配置窗口，也不会拦截 AI 操作。
- 漫剧工作流的对话入口统一收敛到 Creative Hub，已移除不会参与当前创作链路的独立聊天和占星入口，漫剧、模型与动画工作台继续保持可用。
- 动画库修正站立待机、行走循环和坐姿循环的骨骼姿态与骨盆位移，角色动作不再保持 T 姿或出现异常深度偏移。
- 动画资源更新后会自动刷新动作缩略图，卡片预览与完整 3D 预览页使用同一套最新姿态。
- 场景 3D 编辑进入时会根据当前状态全景图自动估算圆半径、投射中心高度和地面分界；没有可靠尺度依据时使用圆半径 7.5 米、投射中心 2 米和 50% 分界的稳定默认值，已有场景按原有实际尺度继续显示。
- 场景 3D 环境统一使用 2.5–15 米圆半径，拖动半径时投射中心按比例跟随，场景编辑和分镜摆位使用同一套尺度。
- 角色和怪物状态的身高统一支持 0.5–10 米，血角兽等 5 米级怪物可以直接参与角色卡片、自动构图和 3D 分镜，保存后再次打开仍保持相对比例。
- 场景 3D 环境分析会记住对应的状态图，换图后才重新分析；用户手动保存的投射参数始终优先，不会被自动估算覆盖。
- 场景状态和资产卡片不再显示场景类型选项，时间、天气和状态图片提示词继续保留。
- 模型 3D 预览新增室内客厅、中央广场和草地自然三套 HDRI 环境，可直接切换；半球直径默认 15 米，并可在 5–30 米范围调节。
- 系统设置提供「通用资产」入口，旁白音色和模型/动画 HDRI 环境集中展示；每套 HDRI 都可以直接进入 3D 预览，调整后的直径会同步用于模型、动画和缩略图预览。
- 通用资产的 HDRI 3D 预览与漫剧场景编辑使用同一套场景编辑体验；可以在对象列表中查看环境并直接调整投射中心和半球直径，预览时不再出现无关的角色或摄像机辅助对象。
- 通用资产列表新增 2D 全景图放大预览，资源路径收起，只保留资产名称、全景预览、半球直径和 3D 预览入口。
...
- 分镜 3D 草图与动画库统一使用同一套 UAL2 角色骨架和动作资源，切换静态姿势时不会再因骨架不匹配而错乱或卡在 T 姿。
- 分镜姿势选择会依据统一动画文件的真实动作片段显示；历史布局遇到文件中不存在的姿势时会安全恢复为站立，不会让整个 3D 草图打不开。
- 点击动画卡片可进入完整的 HDR 3D 预览页，支持播放、暂停和拖动时间轴精确查看动作帧。
- 在时间轴上选定任意一帧即可截图设为动画卡片预览图；预览帧会保存在当前浏览器中，随时可以恢复默认预览图。
- 动画预览、动画卡片和分镜 3D 草图统一使用蓝色代理角色；换色后旧的黄色预览截图会自动刷新。
- 修复模型、动画和分镜 3D 预览的 HDRI 半圆穹顶投影黑影问题：全景环境不再把自己投射到地面形成黑块，同时角色阴影仍会正常落在地面上。

### 2026-08-29
- 分镜与场景 3D 预览现在会把代理角色的投射阴影叠加到 HDRI 下半部地面上，保留全景背景细节的同时让角色与环境的接触关系更清楚。
- 模型 3D 编辑的摄影棚背景修正为半圆球穹顶投射：全景不再整球包裹到视线下方，地平线以下回归编辑器地面，与漫剧 3D 场景的背景方案一致。
- 模型 3D 编辑的摄影棚穹顶调整到 10 米直径（与漫剧 3D 场景一致），全景图换成带细微环境细节的室内工作室（墙面拼缝、踢脚线、水泥地反光）；编辑器不再额外叠加地板，地面完全来自全景图，取景拉远时穹顶会按需扩大避免穿帮。
- 模型预览的棚拍环境换上 AI 生成的摄影棚全景图：沿用场景全景图的生成流程产出中性摄影棚 360° 全景，编辑器与缩略图的环境反射来自真实感摄影空间，背景直接呈现摄影棚弧形墙与顶部柔光。
- 模型预览引入真实 HDR 棚拍环境：3D 编辑与缩略图的环境反射换用内置中性摄影棚 HDRI，金属、釉面、漆面的高光有了真实的空间倒影，模型光影更立体；布光同步重新调配。
...
- 顶部导航新增「动画」页（位于「模型」与「系统」之间）：已导出的角色动画（站立待机、行走循环、坐姿循环）统一收录在独立页面里，与模型库同款卡片展示——每个动画带自己的动作预览图与名字，按待机、移动、坐姿分类并带数量统计，点页签筛选，点卡片即可在弹窗中播放查看角色动作效果。
...
### 2026-08-28
- 模型库全面扩充到 44 个模型并补齐真实外观：新增床、沙发、书桌、座椅、餐桌、茶几等近景家具，书堆、烛台、瓷瓶、摆件等装饰品，盆栽与棕榈等植物，以及山石等自然物；所有模型都带从源项目提取的漫反射贴图和颜色变体，不再是白色素模。
- 模型库页面更简洁：去掉大标题和描述，分类改为带数量统计的分类表置于页面顶部，模型网格一行显示 10 个，点卡片直接进入 3D 编辑。
...
- 顶部导航在「漫剧」旁新增「模型」入口：内置常用模型库；每个模型自动生成 3D 预览图，可按分类筛选查看。
...
### 2026-08-27
- 分镜 3D 草图的角色静态姿势改从动作片段的中段截取：开头未稳定的过渡帧不再被当成摆位姿势，站、坐、走等静态摆位更自然完整。
- 分镜 3D 草图的摄像机是可自由摆放的场景对象：拖拽机身、用移动/旋转手柄或在属性面板键入数值，都能把它放到任意位置、调好朝向；漫游视角不会再带着摄像机跑，也不会挡住布景。选中摄像机即可在视口右下角实时预览这台摄像机拍到的画面，机位随草图一起保存，下次打开保持原位。
- 3D 编辑器的属性面板顶部精简为图标加对象名称：去掉类型徽标、运镜时长等附加说明和卡片外框，两个 3D 编辑器观感一致。
- Transform 面板更紧凑：位置和旋转各是三个纯数字输入框（去掉轴标、单位后缀和上下步进箭头，数字完整可见），缩放改为单个数值的整体等比缩放——物体不能再被单独拉宽拉高；视口里的缩放手柄同步只保留整体等比，导出与保存行为不变。
- 3D 编辑器视口对齐 Unity 场景视图：左上角新增「移动 / 旋转 / 缩放」工具条，选中角色、空间标记后视口内出现彩色三轴手柄，直接拖拽即可调整位置、朝向和大小（导出的摆位草图不包含手柄）；分镜草图的角色支持像 Unity Transform 一样直接键入位置 / 旋转 / 缩放数值（原方向按钮网格改为数值与手柄），场景编辑器里的空间标记拖拽或改数值后都会自动保存。
- 场景 3D 编辑的属性面板卡片间距加大并与分镜 3D 草图属性面板保持一致：相邻卡片不再视觉上贴在一起。
...
- 场景与分镜的 3D 编辑器改用参考 Unity 的检查器交互：左侧「对象列表」带数量表头，右侧属性面板由可复用组件构成——顶部是 GameObject 风格的名称条（世界、空间标记可直接改名并保存），下方按「组件节」折叠组织各属性组；空间标记提供 Unity Transform 式的可编辑数值字段（位置 X/Y/Z、旋转 Y、缩放 X/Y/Z，改完即保存并同步 3D 视图），分镜摆位页里同样以 Transform 展示（只读，修改入口保留在场景编辑器）。
- 3D 草图新增 Unity 风格的场景摄像机：视口里常驻一个小摄像机机身，停在当前镜头机位并朝向拍摄焦点；可在对象列表点选「摄像机」或直接点机身选中，选中后按住机身拖动即可移动机位（始终看向焦点），属性面板可精确调位置、朝向与视野角，旋转按钮同步生效；右下角「镜头取景」画中画就是这台摄像机的实时画面。导出的摆位草图不包含摄像机机身等编辑器辅助对象。
...
- 弹窗提示更省心：报错弹窗超过 5 秒自动消失，不再永久堆叠占屏；所有报错弹窗都会自动记入日志，个别页面即使显示异常也能在日志里查到。「最近报错日志」就在「记录」页签顶部，支持按全部、弹窗报错、未捕获异常分类查看，本机保留最近 100 条，可随时回看、一键清空。
...
- 分镜 3D 摆位引入舞台余量：角色（含跑动等大幅动作落点）不允许进入场景半球边缘 1 米的范围，AI 自动构图会自动把站位限制在可用半径内，3D 编辑器拖拽角色时同样受限，并用琥珀色参考圈常驻标出这条边界。
- 分镜拍摄位统一锚定在场景投射中心：AI 自动构图生成的镜头位置固定在全景图原始取景点（高度与投射中心一致），构图通过视线方向、拍摄距离和焦段表达，成图与全景背景保持一致。
- 文本模型通道临时切换为 OpenCode Go：Grok Build 免费文本额度耗尽期间，大纲、正文、审校等文字任务改走 OpenCode Go 订阅额度，Grok 额度恢复后可切回。
- 模型通道错误提示更透明：OpenCode Go 上游错误（如余额不足）会直接显示真实原因，不再笼统提示“服务没有返回文本内容”；空间标记识别在纯文本通道上会明确提示需要视觉模型通道，不会在无图状态下生成无效标记。
...
- 空间标记识别会生成「可行走地面」范围：以最近墙面的深度为准，在 3D 预览里用薄板标出角色可以站立的地面区域，角色摆位和 AI 自动构图会把站位限制在这块地面内，不会站到墙面投影上；地面范围随门窗识别结果自动更新，不需要单独标记。
- 空间标记识别提速：识别前会自动把全景图压缩到适合识别的尺寸再交给视觉模型，大图场景的识别等待明显缩短，识别按钮也会显示预计等待时间。
...
- 分镜列表收敛为紧凑四行结构：每一镜按「分镜设计」「场景/角色」「旁白/对白」「配音」对齐展示，左侧固定标签加竖线分隔；动作、景别、运镜、场景、出场角色及状态可直接查看，画面提示词按需展开。场景与角色使用不同颜色的资产标签，语音行显示旁白/角色及语气；没有音频时只显示生成入口，生成后显示可拖动的真实时长试听条。
- 分镜的 AI 摆位统一放在「编辑3D」里：点击「AI 自动构图」后，角色位置、姿势、相机和景深会直接显示在当前镜头预览中，并在旁边展示镜头设计说明，确认前仍可继续手动调整。
- 场景 3D 编辑和分镜 3D 草图按浏览器高度铺满工作区：桌面端左侧集中显示返回入口、场景对象和属性内容，右侧保留完整 3D 视口。对象列表只显示图标和名称，HDRI 对象显示为「世界」、尺度校准人物显示为「参考角色」、已识别的空间标记直接列出；列表与属性内容各自滚动，选中可移动角色时属性区域会显示位置、旋转和大小。
- 3D 草图和场景 3D 编辑中的选中角色、怪物或参考角色改用贴合模型真实轮廓的外轮廓描边，不再显示矩形包围盒和脚下选择圆盘；导出的 3D 草图图片不会带上编辑器选中标记。分镜 3D 草图还支持调整选中角色的代理模型颜色，颜色实时更新并随布局保存。
- 场景 3D 编辑器和分镜 3D 草图调整投射中心高度、半球直径等参数时只更新当前预览，退出编辑器才一次性保存，连续调参不会反复触发保存请求。
- 漫剧 3D 场景 HDRI 改为连续的上下半球环境：天空和地面共用一圈边界与一次贴图绘制，半球与地面接缝为连续弧面过渡，场景图先转换为带边缘处理的立方体纹理再按世界方向采样，减少地平线裂缝、中心环状漩涡、上下画面断层和接缝拉伸。场景预览只显示设定直径范围内的有限 HDRI 半球，不再被引擎的无限天空盒覆盖。
- 3D 场景预览会从 HDRI 中识别窗户、太阳等高亮方向，为角色提供对应的基础方向光；环境反射与柔和环境光继续保留，切换或清除场景图时会同步更新光照。
- 场景空间识别会把当前状态图直接交给视觉模型，并保留每个标记的图像证据区域；单张全景图无法确定的绝对深度继续以构图用的近似距离处理。
- 场景空间标记按图像证据反算真实深度：门以框底落地线确定墙面距离，窗按可见跨度与典型窗高反算；同一面墙上的门窗共享一致墙距，门体整体落在地面上，角色摆位可以准确贴到门窗前。修改投射中心高度或半球直径后，旧标记会自动进入重新识别状态，不会把旧坐标继续带进 3D 预览和分镜摆位。
- 桌、椅、床等家具的深度由落地线、顶边高度与垂直跨度多路证据综合估算，识别框底部偏高时不再把家具推到场景边缘，家具也不会越过所在方位的墙面；修改投射中心高度、半球直径或全景地面分界后，旧标记会自动要求重新识别，避免家具和门窗坐标漂移。
...
- 场景全景图生成按固定构图分区：下半区（50% 以下）只保留连续干净的地面，远山、天际线和高大物体集中在地平线以上的远景带，再往上（70% 以上）只保留天空，中心过渡带保持空净，不在图片上绘制分割线；3D 场景编辑器提供 40%–65% 的全景地面分界调节，历史场景缺少该参数时按 50% 兼容读取。
...
- 角色进入分镜 3D 草图时会根据年龄、体型、外貌等已有设定推断近似身高，并把它保存为角色级比例基准；高大角色、儿童等在不同镜头里会保持稳定的相对大小，旧草图仍按原有缩放显示。
- 漫剧 3D 场景 HDRI 改为连续的上下半球环境：天空和地面共用一圈边界与一次贴图绘制，场景图先转换为带边缘处理的立方体纹理，再从投射中心按世界方向采样，减少地平线裂缝、中心环状漩涡和上下画面断层。
- 角色资产卡片会自动显示由角色资料估算出的分镜比例基准；从参考文本提取并应用角色时，默认状态中的年龄段、外貌和图片提示词会参与估算，资产页、提取结果和 3D 分镜使用同一份身高档案。
- 角色状态编辑器新增「身高（米）」输入（例如 1.75）；填写后当前状态优先使用该值参与分镜比例，留空则继续使用角色资料的 AI 估算，不同状态之间可以保持稳定的相对大小。
- 角色和怪物状态的身高范围统一为 0.50–10.00 米；血角兽等 5 米级怪物可以直接参与角色卡片、AI 估算和 3D 分镜比例。
- 角色状态打开后会先显示状态名、年龄段和身高参数，再显示状态图片，调整分镜比例时不需要先滚动大图。
- 角色、场景、道具状态编辑器统一先展示状态图片，再在下方紧凑排列状态名、时代风格及其他资料；角色状态仍可直接调整身高和年龄段。
- 3D 草图不再提供手工缩小或放大角色的操作；本镜角色列表和选中角色信息会显示近似身高，位置、旋转、姿势与相机视角仍可继续调整。
...
- 角色状态编辑器新增「身高（米）」输入（例如 1.75），填写后当前状态优先使用该值参与分镜比例，留空则沿用 AI 估算；角色和怪物状态的身高范围统一为 0.50–10.00 米，5 米级怪物也能保持稳定的 3D 分镜比例。3D 草图锁定角色身高比例，不再提供手工缩小或放大角色，本镜角色列表直接显示近似身高。
- 场景 3D 编辑器的比例参照角色统一为约 1.7 米并放在场景中心，使用橙色外轮廓高亮，代理角色使用当前 HDRI 环境光进行明暗表现。
...
- 场景资产新增「识别空间」能力：系统会从当前场景状态图识别床、桌、椅、门窗等固定物体，在场景 3D 编辑器和分镜 3D 草图中显示为可点击、可聚焦的半透明空间标记，帮助角色快速避开或贴近真实布景。
- 分镜 AI 自动构图会读取场景空间标记，结合固定物体安排角色位置；重新生成场景状态图后，旧图片对应的标记会自动失效，避免把上一张图片的家具位置带入新状态。
- 漫剧 3D 草图支持按当前镜头上下文自动规划所有出场角色的位置、姿势、相机参数和景深，并在 PlayCanvas 预览中直接呈现规划结果；角色清单不一致时会阻止应用，避免漏掉出场角色。
- 分镜还没有 AI 首帧时会强制显示已保存的 3D 草图；每一镜新增「AI摆位」入口，可让大模型规划角色位置、姿势、相机和景深后进入 3D 编辑器继续调整。
- 漫剧分镜预览采用左侧「3D图」、右侧「AI图」的水平切换；「编辑3D」「AI摆位」和「生成AI图 / 重新生图」集中在切换区域下方。
- 漫剧 3D 草图编辑器在旋转、缩放和摆位时只更新当前编辑状态；点击返回分镜时才一次性保存并自动确认，返回后立即刷新最新草图，避免操作过程被保存请求打断。
- AI 自动构图完成后保留未保存修改，返回入口只在最终保存期间锁定，确保规划结果和最新草图完成保存后再离开。
- 从场景资产进入 3D 场景编辑后返回，会保留“资产 / 场景”页签位置，不再回到当前脚本页。
- 分镜的 AI 图与场景参考图来源严格分开：AI 图缺失或加载失败时会明确提示重新生成，不再把场景图显示成 AI 图；生成服务也会阻止把参考图原样保存成 AI 首帧。
- 漫剧本地音频通道已恢复为 VoxCPM2：旁白、角色对白、状态音色和分镜配音统一使用本机 18761 语音服务，开发启动会先确认模型真正就绪；IndexTTS 2.5 暂时保留为显式兼容通道，不会自动启动或覆盖现有音色数据。
- 重新设计旁白音色时，只有用户明确选择的参考音频才会参与克隆；没有参考音频时会真正按新的音色描述重新设计，不会把上一轮试听结果带入导致男女性别或音色无法切换。
- 系统旁白的女性默认描述现在会明确声音年龄、音域、亲和度、语速、停顿和自然讲述感，减少偏平或播音腔；用户仍可编辑描述并生成自己的旁白风格。
- 画风管理的通用美术风格统一为写实影视化：角色、场景和道具的默认画风不再使用虚幻三维游戏渲染基线，分镜生成也会稳定回落到写实风格；现代、末世和年代等时代氛围选择继续保留。
- 漫剧分镜页将批量入口收敛为一个「合成」按钮：点击后会并发补齐缺失画面和配音，准备完成后自动进入视频合成；逐镜生成入口继续保留。
- 漫剧 3D 草图的 HDRI 面板收敛为投射中心高度和半球直径：高度默认 2、可调范围 1–10，直径默认 15、可调范围 10–50；水平旋转固定为 0，环境亮度固定为 1，不再占用面板空间。
- 3D 场景编辑器的右键水平旋转支持无缝跨过 0°/360° 接缝，拖动到边界不会停止。
- 场景资产新增独立的「3D 场景编辑」入口：可在场景资产里统一调整投射中心高度和半球直径，并用约 1.8 米的比例参照角色校准尺度；所有使用该场景的分镜都会继承同一组参数，分镜页不再单独维护 HDRI。
- 角色、场景和道具资产页签在桌面端一行展示五张卡片，主视图突出方形图片、资产名称和状态；大纲侧栏继续保留紧凑信息布局。
- 场景状态图片旁保留独立的「3D编辑」入口，会直接打开当前状态自己的 3D 预览；状态编辑器专注查看普通图片，不再承担全景拖拽交互。
- 角色、场景和道具状态图会按原图比例完整展示，避免固定容器造成两侧留白；场景状态的「3D编辑」与「生成图片」操作位于同一行。
- 场景状态编辑器的场景图片严格按 2:1 横向铺满显示区域；「3D编辑」移动到图片下方，避免操作按钮覆盖图片预览。
- 修复场景图片放大层遮住「3D编辑」入口的问题；只要当前状态已有图片地址，独立 3D 编辑器就会恢复对应 HDRI 背景，不受历史状态标记延迟影响。
- 2:1 全景场景图也会使用带投射中心的弧形地面，修改投射中心高度和半球直径会立即反映到预览；状态图片文件缺失时会明确提示重新生成，不会伪装成参数未生效。
- 3D 场景半球的地面投影修复经度接缝：首尾 UV 不再跨整张纹理，投射中心使用单一平底顶点并保留外圈过渡，避免状态图在地面出现竖向拉伸条。
- 3D 场景地面改为按投射中心进行片元级全景投影，避免顶点 UV 插值在中心形成环状漩涡；全景纹理经度采用可循环采样，旋转场景时不再出现人为拼接线。
- 3D 场景上下半球统一使用投影材质，并补齐标准材质的颜色空间输出；地面与上半球亮度保持一致，半球交界按同一投影连续采样，减少接缝和拉伸。
- 修复历史状态图在 3D 场景预览中因缺少制品指针而无法加载的问题；已有归属明确的图片会继续可用，不需要重新生成。
- 角色、场景和道具状态图重新生成失败时会保留最后可用预览，状态引用、分镜参考图和 3D 场景预览也会继续使用；当前制品异常时只从同一资产状态的历史图片恢复，不会再因同名状态串用其他资产图片。
- 状态图生成超时后，详情页可以用 X 关闭失败提示；已确认的图片、制品引用和重新生成入口都会保留，没有可用图片时回到普通缺图占位，即使新一轮生成出现相同错误文案，也不会被旧提示的关闭操作误清除。
- 普通场景图的半球地面改为连续的弧形投影，调整直径或投射高度时不会出现从中心向外扩散的圆形波纹；中键平移按屏幕方向工作，拖动时场景跟随鼠标移动。
- 场景 HDRI 全景图按 Unreal 式上下半区生成：地平线位于中线，上半部集中环境物体，下半部以连续地面为主，减少地面物体在等距柱状投影中的拉伸。
- 3D 草图使用当前 HDRI 生成环境光照，角色不再叠加独立方向光和补光；选中角色以绿色标记，场景编辑页的 1.8 米比例参照角色保持固定位置。
- 修复 3D 场景 HDRI 上半球消失的问题：上下半球统一使用投射中心投影材质，环境光照保持来自 HDRI，交界处不再因材质切换产生亮度断层。

### 2026-08-24

- IndexTTS 2.5 的全局旁白音色统一在「系统设置 → 旁白音色」中配置，模型音色、参考音频和试听都对整个应用生效；「分镜」页只保留分镜与配音操作，不再重复展示通用音色设置。
- 旁白音色设置现在只显示一个试听样本；参考音频仍可选择、上传和清除，IndexTTS 2.5 作为同一张旁白设置卡片中的语音合成来源，不再形成重复的本地模型页面感。
- 漫剧旁白和角色音色现在可以选择 IndexTTS 2.5 的基础/训练音色与参考音频，也可以直接上传新的参考音频并单独保存音色来源；生成的试听样本会自动用于后续分镜配音，换音色后旧配音会自动标记为需要重新生成。
- 漫剧分镜列表把每镜预览图放大并保留完整横屏比例：可在图片右侧切换「3D 草图 / AI 生图」，直接编辑 3D 草图或生成、重新生成 AI 画面；没有草图时仍可从编辑入口创建。
- 漫剧分镜操作统一为「3D图 / AI图、编辑3D、生成AI图 / 重新生图」，AI 操作只显示一个 AI 标识，避免重复图标干扰阅读。
- 从 3D 草图编辑器返回分镜列表时会稳定恢复当前章节，不会因轮询初始化时序导致页面空白。
- 本地配音通道切换为 IndexTTS 2.5：9000 网页工作台继续提供克隆配音、API 服务、多角色配音和字幕配音功能，项目开发环境会自动准备 9005 API，旁白、角色试听和漫剧配音统一使用新的本地语音服务。
- 分镜画面生成会真正带上本镜出场角色的当前状态图、地点场景图和相关道具图；同一场景连续镜头会沿用场景状态图的光照关系，减少角色外观漂移和镜头间忽明忽暗。
- 漫剧工作台启动页会区分“服务未响应”和 HTTP 错误，并持续自动重试；本地服务重启后，遗留的画面、配音和合成任务会自动恢复为可重新发起的状态。
- 服务启动会先完成遗留任务恢复，再接受新的漫剧任务；重复点击批量生成或合成时会复用当前任务，静音镜头也会按明确设置的镜头时长进入成片，不再伪造配音时长。
- 漫剧工作台的素材统计、批量配音、整集合成预检和实际取用的配音现在共用当前文本、音色和模型的就绪状态；脚本或音色变化后，旧配音不会再被误当成可用素材。
- 切换章节时，配音任务轮询和分镜批量选择会自动隔离当前章节；参考文本解析会等待本章脚本自动保存，避免旧草稿覆盖解析结果。
- 批量生成分镜画面会自动使用更高的安全并发，任务状态会显示当前并发路数，整集补图更快；失败镜头仍可单独重试。
- 漫剧视频页重新整理为「合成设置 → 合成进度 → 视频预览 → 视频信息」四个区域：字幕写入视频、片头片尾的作用更清楚，生成结果横向铺满预览区，并移除会跳到另一套流程的旧视频工作台入口。
- 漫剧分镜在同一项目内统一锁定写实影视化媒介：现代、末世和年代风格仍可按剧情改变氛围，但逐镜生成不会再切换到卡通或动画；画风在通用画风管理中设置，分镜页按当前设置继续生成。
- 漫剧工作室将整集合成结果统一放到「视频」页签；「分镜」页只保留镜头列表，生成分镜、生成配音和合成操作集中在页签上方，进入后能更快区分素材准备与视频输出。
- 系统旁白音色默认使用成年女声描述，旁白、角色和状态音色试听统一朗读标准普通话测试文本；来自小说正文的旧试听样本会被隔离，可直接重新生成。
- 旁白、角色和状态音色的试听与配音现在统一响度，避免同一项目中旁白过小或角色过响；重新生成的音频会自动控制峰值，降低爆音风险。
...
- 漫剧分镜统一从「3D 草图」进入角色与相机摆位：角色姿势只截取一个静态关键帧，保存的 3D 空间快照与 PNG 会继续作为分镜画面生成的构图参考。
- 3D 草图会把本镜场景状态图加载到内侧半球 HDRI 环境中，半球下半部同时提供弧形地面；环境固定在世界坐标，旋转或移动镜头只改变视点，不搬动地面，纯色地面只在没有可用环境图时作为兜底，角色和地面处于同一 3D 场景。
- 3D 草图会按场景图比例自动选择 HDRI 半球或 EnviroDome 式的带贴图弧形地面：普通场景图的天空/远景与下半球分层显示，避免地平线条带和地面拉伸；并提供投射中心高度（0.6–10）、半球直径（20–100）、水平旋转和环境亮度调节，参数会随草图保存，刷新后仍可继续调整。
- 3D 草图 PNG 按严格 16:9 的 1280×720 开发基准捕获，保存和确认期间会锁定编辑，避免空间快照与分镜参考图来自不同状态。
- 确认 3D 草图后，分镜画面生成会把它作为锁定的第一张参考图，以保持场景视角和角色相对位置；尚未确认的草图不会消耗图片额度或参与批量生成。
- 分镜配音区按真实音频状态显示：没有可播放音频时只保留「生成配音」入口，音频生成后才显示可拖动的试听进度条，部分完成时只展示已有音频。
- 漫剧旁白与角色配音现在按各自的语音语义走同一套本地音频链路：旁白使用系统旁白参考音频与旁白控制方式，历史上误用对白控制的旁白音频会自动进入重新生成状态。

### 2026-08-23

- 系统设置新增「旁白音色」：整个应用统一使用同一个旁白声音，可直接试听、保存描述并重新生成；已把旧项目生成过的旁白样本初始化为默认参考音色。
- 分镜工具栏按当前章节简化为「生成分镜」「生成配音/重新配音」「合成」三个入口，章节只在工作室右上方选择，避免重复选择和重复的项目级音色设置。
- 漫剧分镜现在直接生成静态横屏分镜画面，不再提供「首帧」、视频提示词或逐镜图生视频入口；画面会与旁白、角色对白和字幕一起进入成片。
- 成片工作台改为按镜头时长保持静态画面并合成横屏 MP4/SRT，缺少单镜画面或配音时会继续装配并明确显示可恢复提示。
- 脚本右侧资产列表按类型分组：角色统一排在上方，场景随后，道具最后；同类资产继续按脚本中的使用顺序排列。
- 脚本右侧设定列表改为图片优先：直接展示角色、场景和道具的默认状态图与资产名称，隐藏提示词摘要，并扩大图片预览区域，点开图片仍可进入编辑。
- 角色、场景和道具资产卡统一使用左侧方形预览：角色从四视图板最左栏的面部区域裁出 1:1 方形，场景与道具展示默认状态图的水平居中方形内容；没有图片时保留统一占位。
- 角色、场景和道具缺少默认状态图时会自动并行补图：进入设置或漫剧工作室后系统会自动发现缺图并开始生成，资产卡片即时高亮显示「生成中」，失败时保留「生成失败」状态并可从详情重试。
- 「提取」页的角色、场景和道具建议复用同一套左侧方形预览：已有资产按默认状态图展示，没有图片时保留统一占位；世界观条目以纯文字呈现，不显示无内容的图标。
- 修复资产预览裁切与串图：角色预览不再把整张四视图板垂直居中，而是固定取最左栏的面部窗口；场景和道具取水平居中的方形内容；不同资产即使使用同名「默认」状态，也会各自保存和读取图片，不再把一个角色的图片显示到另一个角色上。
- 角色、场景和道具的状态图片现在按资产、状态和生成批次分别保存，重新生成不会覆盖其他资产；生成失败时保留原图。无法安全判断旧图片归属的资产会明确标记为缺失，可直接重新生成。
- 脚本旁白行不再显示或编辑无效的「语气」占位；角色台词仍可填写语气，旧脚本中的旁白括注会自动清理，配音列表也按旁白处理。
- 脚本右侧列表点开资产就是编辑界面：和「资产」页签打开的是同一个弹窗——字段、外观状态（生图/音色）都能直接改并保存，弹窗底部可删除（有二次确认）。此前点开是另一个只读预览，要改内容得再去资产页签找。
- 漫剧工作室补齐脚本到分镜的主流程：脚本页点击「生成」只处理当前选中的章节，分镜页也保留同一个「生成」入口；生成前会使用已保存脚本并同步现有角色、场景、道具资产。
- 分镜列表右侧统一放置「生成缺失画面」「生成缺失配音」「全部重新配音」「合成」等操作：按当前章节把静态画面和配音合成为横屏成片，实时显示进度，完成后可直接预览成片并下载字幕。
- 批量生成缺失分镜画面现在会并发处理并立即显示状态：生成中的镜头缩略图会高亮并标出「生成中」，任务失败的镜头可以再次批量生成。
- 分镜配音支持逐镜操作：每一镜都可单独生成或重新生成本镜旁白/对白，已经生成的音频可直接在对应台词行试听，方便逐镜测试声音效果。
- 分镜行压缩为「镜头号/文字」和「音频控制」两层：不再单独占一行显示「音频 配音 1/1」；播放器支持拖动进度，直接显示真实音频的「当前秒数 / 总秒数」，生成/重新生成按钮与播放器同一行。未生成真实音频时不显示估算时长，整集合成也只使用实际测得的配音时长建立时间轴。
- 分镜合成按镜头时长保持静态分镜画面：整镜使用同一张横屏图片，不做推拉、平移或其它运镜；字幕和音频仍按配音时间轴同步。
- 修复长镜头整集合成可能停在渲染中的问题：横屏 16:9 的 720p 开发视频现在可以继续完成渲染、混流并生成字幕文件。
- 漫剧成片字幕统一改为无底板白字，并使用柔和的模糊阴影提升复杂画面上的可读性。
- 漫剧旁白字幕只显示白色正文，不再显示「旁白：」标签；角色对白继续显示说话人名称。
- 分镜画面和整集成片统一使用横屏构图；配音页面取消通道选择，直接使用系统设置里配置的音频模型，整站配音口径一致。
- 漫剧整集合成统一输出横屏 16:9：开发默认 1280×720，发布可切换 1920×1080；分镜画面、配音、字幕和成片共用同一条时间轴，缺少单镜素材时会保留可播放成片并显示告警。
- 系统设置总览新增「视频输出」：默认使用 720P（1280×720），需要正式产出时可切换为 1080P（1920×1080）；设置只影响后续生成的视频，已生成成片保持不变。
- 漫剧角色、道具和分镜图片统一使用严格 16:9 横版规格（1536×864），场景全景图保留严格 2:1 规格（2048×1024）；开发视频仍用 720P，最终发布再切换 1080P。
- 场景全景预览的水平拖拽方向与视角一致：向左拖视角向左转、向右拖视角向右转；上下拖动限制在 ±60°，避免视角翻到天空或地面极端位置。
...
- 脚本中的场景行与角色状态面板现在连成同一组，统一颜色和边界；一个场景下有多个角色状态时，也能一眼看出它们都属于当前场景。
...
- 画面时代风格跟着剧情走：生成角色/场景/道具的状态图和分镜首帧时，AI 会先读这一段剧情判断当下处于什么时代（比如故事开篇还是现代都市、章末才进入末世废土），再选对应的时代风格作画——不再全书一刀切用默认风格；剧情线索不明时才回落你选的默认风格。设定里的时代风格仍在，作为默认值与可选清单。
...
- 音色试听语速恢复正常：本地语音合成的结果此前按错误的采样率写出，播放时整体慢一倍、音调偏低像机械声；已按模型真实采样率输出，之前生成的旧试样重新生成一次即可恢复。
- 音色试听与配音播放器一打开就显示时长：改为只预读音频的媒体信息（不预下载整段音频），不再需要先点一次播放才看到时长。
- 资产卡片点开即编辑：在「资产」页签或设定中心点角色、场景、道具的卡片，直接进入编辑弹窗——外观状态、图片和音色当场就能看和改，不再先弹一层只读详情还要再点一次「编辑」；脚本页右侧资产栏仍保持原来的查看方式。
- 角色和道具的参考图升级为透明背景：角色四视图/状态图与道具图直接输出可叠加的透明底 PNG（无背景色、无地面阴影），能更干净地作为分镜画面的参考素材；这两类参考图统一改用 Codex 图片通道生成，走已登录的订阅额度，开发服务启动时会一并把该图片通道准备好。场景全景、分镜首帧等画面类图片保持原有背景不变。
- 状态编辑改成直接编辑并保持紧凑：角色/场景/道具的外观状态字段当场就能改（状态名与年龄段或场景类型/时间/天气同行，加图片提示词、音色提示词——状态名已能表达成因，不再单列「状态变化」，留空自动按状态名生成），改完点「保存状态」即可；点「生成图片/生成音色」也会先把未保存的修改自动存好再生成，不用来回点。「添加状态」挪到状态列表底部，区块标题去掉，图片/音色提示词支持多行书写。
- 状态图片提示词可以 AI 微调：图片提示词下面写一句要改的地方（如「去掉身上的伤」「换成黑色外套」），AI 只改这一处、其余描述原样保留——新增状态复用旧状态提示词时不用整段重写；提示词留空时也能按这句指令直接生成一条新的。
- 生成图片/音色时可以随时关掉弹窗：生成会继续完成并保存，重新打开弹窗直接显示最新结果——还在生成中会自动刷新到完成、按钮显示生成中；正在编辑的未保存修改不会被覆盖。
- 音色想用哪个用哪个：生成音色旁边新增「选取音色」——把任意一个状态已生成的音色直接拿过来用（列表里能预览各状态的音色提示词）；不再默认「沿用上一状态」。
- 生成图片前当场选参考图：参考图选择就在图片区——可以选任意一个状态的图当参考（保持同一形象只换装/加伤），也可以留空直接生成全新形象。
- 新增“画风管理”页面，角色、场景、道具各有独立画风卡片，可分别编辑正向画风、保存或恢复默认；固定规格与固定约束只读展示。
- 角色资产和状态图固定使用横向四视图，场景使用 360° 全景，道具使用 45° 三点透视，生成链按资产类别使用对应画风。
- 场景全景图真正按 360° 生成，并能环视预览：场景状态图按等距柱状全景出图（水平无缝环绕、整圈光线与材质一致），生成后在图片区直接拖拽环视、滚轮拉近拉远，像站在场景里转圈查看；也可一键切回平面图。之前生成的普通场景图重新生成一次即可变成全景。
- 道具参考图只画道具本身：提取与生成都会忽略道具周围的其它物品和环境（旧抹布、木板这类只属于场景），画面里只有这一件道具；道具的图片提示词也写得更具体——材质、颜色、造型比例、工艺细节、磨损痕迹、表面文字图案。
- 音色试听换成更清楚的参考文本：试听会完整朗读一句自然、清晰、稳定的普通话参考句，吐字与节奏更容易听清。
- 状态与资产统一保存：编辑弹窗只保留一个「保存」，姓名、身份字段和全部外观状态一次保存；状态区不再有单独的保存按钮，点「生成图片/生成音色」时仍会自动先保存再生成。
- 分镜首帧只注入当前镜头实际出现的角色、场景和道具画风，不把资产参考图的固定视图要求带入竖屏构图。
- 场景图片与角色一样按外观状态管理：编辑场景时直接从状态列表查看和生成当前状态图片，大纲详情与首帧参考也只使用场景状态图；旧版 360° 全景数据继续保留作兼容，不再与状态图并列展示。
- 角色、场景、道具的资产卡片与详情弹窗统一：脚本右侧和设定中心点击卡片都能看到相同的基础信息、图片/音色提示词与外观状态，编辑和删除仍从当前入口完成。
- 资产编辑弹窗统一放大，场景基础资料只填写场景名，类型、时间、天气、描述和图片提示词都跟随具体状态管理；道具的外观和图片提示词也跟随状态管理。新建角色、场景、道具都会自动带初始状态，状态图片可继续打开独立大图预览。
- 状态编辑界面进一步收敛：左侧只保留状态缩略图和状态名，首个默认状态统一显示为“初始”，列表不会被右侧详情内容撑出大块空白；右侧按图片、图片设定、音色分区，图片和试听内容直接表达是否存在，不再重复显示“已生成/待生成”标签。
- 漫剧「提取」建议与已有资产同名时，卡片直接显示该资产当前形象缩略图；点开弹窗就是编辑这个已有资产——弹窗顶部把「已存在」和资产类型显示为名字后的小标签，不另占行；姓名/性别和状态列表载入的都是已有数据，状态列表里能看到已生成的图片与音色（没生成过也能当场点「生成图片/生成音色」），改完点「保存」更新该资产，这条建议随即从列表移除。还没生成过图的资产卡片保持原样显示类型图标。资产重新生成状态图后，各处展示会立即显示最新图片，不再停留在旧图。
...
- 分镜画面跟随角色状态：生成分镜时 AI 会按剧情判断角色外观变化（换装、受伤、变身等），在变化后的镜头上标注角色当前状态；生成这些镜头的首帧图时，画面提示词自动带上该状态的外观描述，并优先用该状态的图片当参考图（没有状态图时用角色设计稿）——受伤后的镜头画的就是受伤后的形象，不再一直用初始形象。分镜列表里带状态的角色显示为「名字·状态」。
- 场景与道具可以生成参考图了：场景在资产编辑里一键生成 360° 全景（按场景的时间、天气与图片提示词渲染光线和氛围），道具一键生成 45° 透视视图；生成后分镜首帧图会自动把它们连同角色设计稿一起挂为参考图，画面与设定对得更齐。
- 角色设计稿改为四视图：头部正面、头部侧面、正面全身、背面全身——一张图看全角色长相与服装的前后细节，作首帧图参考更稳。
...
- 外观状态可以直接生成图片：角色/场景/道具编辑弹窗的外观状态列表里，每个状态都有「生成图」按钮——按你配置的生图参考（参考另一个状态的图保持同一形象只换装/加伤，或不参考直接生成新形象）产出该状态的图，生成后状态旁显示缩略图，漫剧资产详情里也能看；再点一次即重新生成。画风与整本书一致（通用画风+本书画风组合）。
- 首帧图、配音、视频批量任务不会再永久卡在「进行中」：服务重启或异常中断遗留的任务会自动标记为失败并提示可重新发起（已完成的部分自动跳过），程序出错时任务也会正常结束而不是一直转圈。
- 「重配此镜」只重配有变化的台词行：台词内容和语气没变的行自动复用已有配音，改一句台词不再把整镜全部重配；需要全部重来时用「全部重新配音」。
- 编辑更稳：切换章节或页签的瞬间正好在保存时，最后几笔输入不会再丢；脚本保存失败时页签行会显示「保存失败」并给「重试」，不再静默。
- 修复地图自动保存反复提交的问题：新建地点还没命名等情况下，地图会每几秒重复保存一次同样的内容，现在保存一次即停。
- 分镜侧的角色资产更安全：在分镜工作台生成过设计图、配过音色的角色，同步小说内容时保留全部成果；手动建的角色也不会被同步清掉，从小说设定里删掉的角色才会跟着移除。
- 漫剧列表与工作室打开更快：项目多、分镜多时列表卡片与阶段统计的加载不再随项目规模变慢。
- 角色音色描述自动同步：生成音色后，卡片里的描述会跟着更新为生成结果，不再停留在旧文案。
...
- 场景名更有辨识度：解析提取的场景会带上归属或特征（如「叶城大学宿舍」而不是「卧室」）——一本书里常有多个卧室、多条街道，具体名字才不会混、画面才有辨识度；同一地点始终用同一个名字，换场标记跟着它走。
- 场景表单更聚焦：新增时间（早上/中午/晚上）与天气（晴天/阴天/雨天）——生成场景图时按它们定光线与氛围，解析提取场景时会按原文自动推断；「氛围」「故事作用」不再出现，环境提示词改叫「图片提示词」，角色、场景、道具三类资产的提示词统一叫法。
...
- 美术风格拆成两层：系统「通用画风」定所有画面共用的渲染质感（默认虚幻引擎级 3D 写实，到 设置·通用画风 调整）；每本书再选具体题材画风——内置 现代都市、末世废土、东方玄幻、现代诡异、古代年代、民国年代，也可以自定义——两层组合后生成角色、场景与画面。同一本书在现代与末世、现代与玄幻之间切换时，画质与质感保持一致，只有题材氛围变。
- 角色开场自带起始状态：登记过外观状态的角色，解析脚本时会在首次出场处标出起始状态（如「【角色状态：叶晨：正常】」），后续受伤、变身、换装等切换从它开始，第一章开头不再没有基准。
...
- 「初稿」和「正文」合并为「脚本」页签，改成线性列表：视频按什么顺序发生，列表就按什么顺序排——场景切换一行、角色形象切换一行、一格分镜一行、分镜下的旁白与台词逐行跟着。点任意一行就能改（画面、说话人、语气、内容、景别都能改），悬停可上移/下移/删除，分镜下可以补台词，底部可以直接添加分镜、台词、场景切换、角色状态；正文里的角色/场景/道具名字按类别高亮。解析产出的脚本质量已经够当正文用，不再需要单独的正文生成步骤。
- 提取的角色信息补全结构化字段：性别、年龄段、体型在提取时直接推断并分别填好（外貌只写发型、五官、穿着这些样貌描述，不再把性别年龄挤成一句话）；点开建议卡片时表单里的性别下拉、年龄段、体型都已预填，确认即用。此前解析的结果里这些字段是空的、信息全堆在外貌一句话里。
- 提取建议改为逐条核对应用：点开建议卡片即可查看并修改全部信息（与资产页签同一套表单——外貌、性格、画面提示词、音色提示词等），改好点「应用」创建这一条；不再多选批量创建。同名资产会标「已存在」并拦截重复创建；提取时也不再自动生成外观状态，状态由你自己手动管理。
- 美术风格独立成页签：整本画面有了明确的画风体系。默认风格点选即存——新增「3D写实电影」（虚幻引擎写实质感，类似《黑神话：悟空》）作为全类型默认，另有动漫、3D玄幻国漫、写实现代、写实末日、写实古装、民国年代共 7 种内置；还可以给这本书自定义多套画风（例如「现代诡异」「末世爆发后」），想换画风时改选默认风格即可，整本统一。
- 脚本列表里一眼分层：角色台词行淡蓝底、旁白行素底，场景切换行淡绿、角色状态切换行淡黄，换场点与形象变化点都醒目。
- 初稿支持角色形象切换：角色受伤、变身、换装时，解析自动插入「【角色状态：角色：状态】」（优先用设定里登记过的状态名），切换后一直生效到下一个同名标记，后续画面生成按它切换角色形象。
- 世界观的「基本设定」只留世界前提与时代背景：时代改为下拉选择（古代/民国/现代/末世等，可选自定义），基调与世界规则统一放到「关键设定」里一条一条写。
- AI 实况里的每个模型调用有明确标题：参考「解析」的两个调用分别标注「把参考小说本章改编成分镜初稿」「从参考小说本章提取角色、场景、道具」，漫剧各环节（改编策略/分集大纲/剧本/分镜/审校等）也都按环节命名，不再笼统显示「大纲与章节规划」。
- 初稿支持场景切换标注：解析自动按剧情在换场处插入「【场景：地点】」行（地点名优先对齐设定中心的场景），后续生成分镜与视频时按它切换到对应场景。手写一行「【场景：客厅】」也一样生效。
- 「解析」一键完成两件事：生成本章分镜初稿的同时，把这一章里的角色、场景、道具、世界观设定都提取出来，提取结果随章节保存（不用也不会丢）；「提取」页签不再有单独按钮，改为查看和勾选创建，页签上显示待处理数量。
- 「解析」的提取结果一解析完就保存：解析还没结束时切到别的页面也不会丢，回到「提取」页签仍然能看到这次的结果。
- 「解析」合并为一次模型调用：同一份参考原文一次解析同时产出分镜初稿和设定建议（AI 实况里只显示一条「参考解析」），等待与开销更省。
- 修复漫剧项目点「解析」报「简易模式项目当前仅供阅读」：漫剧项目的章节、解析、设定现在全部可编辑，不再被简易模式只读拦截（普通简易小说的只读保护不变）。
- 脚本与设定按名字对应：右侧设定面板只显示本章脚本用到的角色/场景/道具（按脚本里出现的顺序排在上面，没用到的不再显示）；脚本里用到了但还没生成的名字，以橙红警告卡提示并附一键创建（名字已预填），建好后警告自动消失；脚本行里未对应的场景/角色名字文字直接显示为橙色（有对应的恢复正常颜色），不占额外空间。
- 重新点「解析」直接覆盖本章脚本，不再弹「替换」确认——重新解析就是对现有结果不满意，结果一到就整章重写。
- 外观状态自己管理：角色/场景/道具的编辑弹窗里可直接添加、修改、删除状态（换装、受伤、昼夜、损坏…），每个状态可配置生图参考——参考同一资产的另一个状态（如新状态参考上一状态，长相不变只换装）或不参考直接生成全新形象；角色状态还能单独设置音色。
- 提取资产自动补全设定：角色带外貌（性别、年龄、体型、样貌合并成一句）、性格、画面提示词、音色提示词；场景带环境画面提示词，道具带实物画面提示词——创建后可直接用于生图与配音。
- 资产外观状态：随剧情换装、受伤、昼夜切换、道具损坏这类大变化，解析时 AI 会对照资产当前样子识别出来，勾选创建后记为一个新的外观状态（画面与音色提示词跟着新状态走，资产卡上以标签展示全部状态），生图与配音自动用最新状态。
- 角色、场景、道具资产都可以删除（角色若已被写作数据引用会明确提示不能删）。
- 初稿右侧的设定卡片只留类型和名字，点开即看完整信息（外貌、性格、生图与音色提示词、外观状态），弹窗里可直接删除该资产；「提取」的建议卡片同样加了「详情」，确认创建前能先看全貌（含画面提示词）。
- 漫剧「参考」页签新增「引用」按钮（「解析」左侧）：一键把参考小说里与本章同序号的章节带入本章参考文本。参考小说按「第一章/第二章…」标题自动切分，第 1 章只引用第 1 章，不会把后面章节的剧情带进来；解析与提取只用编辑器里的内容。
- 每章的参考正文随章节自动保存，重开页面后仍在；「参考」页签只保留编辑器本身。
- 「提取」「解析」按钮不能点时在按钮旁说明原因；「参考」编辑时可见自动保存状态（保存中/还有未保存的修改）。
- 重复上传同一本参考小说不再堆出多份一模一样的存档版本。
- 配音带上情绪：分镜台词按「角色（语气）：台词」生成（如「林小雨（冷声质问）」），配音时逐句把语气传给语音服务（VoxCPM 情绪提示）——台词有了愤怒、惊喜、沙哑低语这些听得出的情绪，而不是每个角色一个固定腔调；分镜列表每句台词旁能看到它的语气，改语气会提示重新配音。
- 世界地图升级为画布：可亲手圈出地形范围（平地、山地、水域），把城市摆到陆地上；两城连线后自动算直线距离，按地图跨度换算公里并给出步行/骑马/车船大约要几天。
- 世界地图支持两级下钻：选中城市点「内部地图」，进入这座城/村镇自己的地图画城区分布，面包屑随时回大地图；AI 起草只负责地点名单，你画的地形与内部地图不会被覆盖。
- 参考小说不再丢失：每章的参考正文随章节自动保存，重开页面后仍在；新建章节的标题也会按参考小说对应章节预填。
- 初稿分镜画面带上景别（特写/近景/中景等），并写清画面里谁在做什么动作、什么神态、正在发生什么——一格分镜看一眼就知道这一镜拍什么。
- 漫剧「设定」页签分成三块：「世界观」把关键概念一条一条写清楚（比如丧尸是什么、异能怎么运作），每条概念名加一段解释，可增可删；「世界地图」是新的一页；「通用」收着参考小说、画面风格与分镜项目状态。
- 世界地图：点「AI 生成地图」按世界观规划地点、方位与通路，先预览再应用；地图是可编辑的——拖动圆点摆位、增删地点、连线标注通路（如「南下商路」），再写一段地图总述讲清整体格局。已有人工整理的地点会在 AI 补充时保留。

### 2026-08-19

- 初稿解析升级为分镜式：一格分镜两行——分镜画面一行、旁白或角色台词一行（台词带表情神态），格子之间空行；初稿编辑器里对话格整组淡蓝显示、旁白格白色，一眼区分。
- 「提取」建议更全：原文里有名字或有台词的角色、出现的地点、核心设定概念都会列入建议，勾选确认后创建。
- 分镜与配音合并为一个「分镜」页签：一行就是一个分镜——左侧首帧图（未生成可就地点生成）、右侧镜头信息与台词，同一行直接列出这一镜的配音段（谁说的、状态、可播放），行内一键重配该镜；顶部保留集选择、批量生成缺失首帧/缺失配音、音色设置。圈选批量与视频提示词等深度操作仍在完整分镜工作台。
- 章节管理更顺手：一行排 5 个章节、卡片只留章节号与标题；列表固定高度不再跳变，打开时自动定位到正在编辑的章节。
- 创建漫剧可拖入现成小说（txt，自动识别中文编码）：作品名自动按文件名填好；小说正文随项目保存，并进入工作室「参考」页签直接解析成本章初稿。创建弹窗同步精简，只保留作品名、故事想法与上传。
- 漫剧工作室新增「提取」页签：从参考小说一键提取角色（带身份定位）、场景与世界观条目，勾选确认后创建进项目设定；解析初稿时同时忽略书名、章节标题、作者感言等非正文内容，并优先保留动作、神态、场景等有画面感的描写。
- 漫剧工作室新增「参考」页签：粘贴一段参考小说原文（两三千字即可），点「解析」AI 会压缩成约 20 行的本章初稿——每行单独标注旁白或说话角色，直接写入初稿；初稿已有内容时会先让你确认并预览再替换；参考文本按章自动保留在本机，刷新或切换章节后仍在。
- 漫剧工作室界面更清爽：空状态、按钮提示、弹窗说明只保留事实信息（如「第 3 章还没有节拍」），去掉「先去某页签」「点某按钮」一类操作指引文案与多余的「去写初稿」入口。
- 漫剧工作室「资产」与「设定」页签整理：「资产」分为角色 / 场景 / 道具三张子页签（带数量），「设定」分为世界观与项目两张子页签，世界观设定归入「设定」；两张子页签条与「当前」一样居中显示、样式统一。
- 漫剧工作室顶栏返回改为轻量样式：图标加项目名即可点击返回漫剧列表，不再有按钮底色与悬停高亮。
- 漫剧「分镜」页签直接就是分镜工作台：分集选择、分镜板、首帧图生成、视频任务都在本页签内完成，不再跳转其它页面；切换章节或进入分镜页签时自动把小说最新内容同步进分镜，无需手动点「同步」。
- 漫剧工作室顶栏返回更顺手：返回按钮和项目名合并为一个整体，点哪里都能回到漫剧列表；顶栏右侧当前章按钮去掉图标更简洁，旁边新增「＋」按钮，一键打开新建下一章。
- 漫剧工作室页签重组：顶部页签改为项目级的「当前 / 资产 / 设定」；「当前」下的子页签（初稿 / 正文 / 分镜 / 配音 / 视频）都作用于当前章——初稿写本章故事（代码编辑器式、行号只读、默认铺 20 行编号空行、自动保存），正文查看与修改 AI 解析的节拍（3～10 拍逐拍可改可增删）、AI 写出正文后直接阅读本章成稿，分镜/配音/视频按当前章推进。「设定」页签集中放世界观与画面风格等项目配置；「资产」页签查看角色、场景、道具；顶栏右侧是当前章按钮（点开章节管理切换或新建）与 AI 写作进度。
- 章节管理简化：工具栏只留新建、搜索框与搜索按钮，卡片一行显示「序号 · 章节名」与字数，点卡片即切换当前章；弹窗不再放底部按钮，右上角 × 即可关闭。
- 修复漫剧工作室写作被「简易模式项目当前仅供阅读」拦住的问题：写本章初稿（自动保存）、点「解析」生成本章细纲、保存细纲、手动新建章节、参考解析现在都正常可用，新建漫剧项目（还没生成分镜）同样不受影响；小说本身仍是简易模式，普通简易小说的只读保护不受影响。
- 修复漫剧列表删除不可用：此前点删除会报「项目 ID 不能为空」，项目删不掉；现在可以正常删除，并连同小说正文与已生成的分镜、配音、视频数据一并清理。
- 漫剧主流程新增「资产」页签：集中查看和维护本项目的角色、场景、道具与世界观设定；大纲右侧面板负责写作时的快速查找与创建。
- 漫剧新增本地视频合成通道：不需要申请外部视频服务，用本机 ffmpeg 就能把镜头首帧图与配音直接合成真实视频片段（缓慢推拉动效、竖屏 1080x1920），在视频工作台选择「本地合成通道」即可使用。
- 漫剧项目可选画面风格：创建分镜项目或之后随时切换（3D玄幻国漫、动漫、写实现代、写实古装剧、民国年代剧、写实末日），首帧图与角色设计稿会按所选风格渲染；风格只影响画面媒介与质感，不改动角色与场景设定。
- 漫剧全链路已可从一本现成小说直达成片：导入小说 → 分集台本 → 影视分镜 → 首帧图 → 逐镜配音 → 视频合成（配合整集合成可直接出片）；本地语音（VoxCPM2）执行 VoxCPM 目录下 `openai_speech_server.py` 即可启动。
- 生成的图片点击即可放大查看：漫画格子、角色设计稿、角色资产图、漫剧分镜首帧都支持大图模式（Esc 关闭、一键下载），横竖图自适应完整显示。
- 角色资产库支持搜索与整理：按名称/描述搜索、按类型筛选、按最近更新或名称排序，资产较多时快速定位。
- 漫剧分镜镜头可手动编辑：镜头弹窗新增「编辑」，可修改台词、动作、景别、运镜、场景与时长；修改台词后配音工作台会自动把对应段落标记为需重配。
- 漫剧新增「整集合成」：在视频工作台一键把整集镜头拼接成一支竖屏成片（1080x1920），自动带片头片尾卡；有视频片段的镜头用片段（画面与配音时长自动对齐），没有的用首帧图加配音做动态镜头，缺素材只降级不阻断。合成进度实时可见，完成后可直接播放、下载字幕（SRT）、重新合成。
- 整集合成支持字幕：台词字幕按配音逐行生成、时间轴与画面严格同步，可一键烧进画面（导出即可发布）或选外挂 SRT；断句与换行适配竖屏阅读。
- 移除「创作向导」页：首页同样展示创作进度和下一步建议，配置引导弹窗保留；侧边栏和移动端菜单相应减少一个入口，导航更聚焦创作本身。
- 漫画与短剧的图像资产统一为固定规格：角色设计稿统一为四视图（面部特写 + 正面、侧面、背面全身），场景参考图改为 360° 全景横图，武器道具等资产改为单张透视的横版设计稿（服装多视图横向并排）；各生图场景画幅统一为「设计参考类横版、分镜首帧/封面竖版」并集中管理。已生成的旧图不受影响，重新生成即按新规范输出。
- 漫剧工作室支持章节管理：顶栏「章节管理」打开，章节卡片列表支持按序号/标题搜索、手动新建章节（只填标题，创建漫剧时上传过参考小说的会自动预填对应章节标题）；点开章节可编辑本章大纲，并用 AI 把大纲展开成 3～10 个细纲节拍（逐拍可改可增删，保存后常驻）；AI 写作进行中仍可浏览章节与细纲。
- 漫剧「配音」阶段升级为完整配音工作台：分镜台词逐行展示，有说话人的行标角色、没有说话人的行标旁白，每行三种状态——已配音可直接播放、已过期（台词或音色改过）提示重配、未生成；支持按集批量「生成缺失配音 / 全部重新配音」，也可对单独一镜重配，未变化的行自动复用已有音频。
- 配音新增音色设置：每个角色用一句话描述声音（年龄、性别、语气、节奏），生成试听后该角色全部台词都用这个声音；项目可单独设置旁白音色，无说话人的台词行自动用旁白配音。修改音色或台词后相关行会标「已过期」，补配即可。
- 漫剧分镜焕新为卡片式分镜板：每个镜头一张竖版卡片，首帧图、景别、运镜、时长、角色与台词一目了然；支持勾选镜头批量生成首帧、一键选中未生成或失败的镜头、宫格预览全部已生成画面、大图模式与单镜放大查看。首帧生成中会自动刷新进度，失败镜头直接在卡片上显示原因并一键重试。
- 图片生成不再被服务重启卡死：三视图、表情稿、格子图等生成过程中遇到服务重启，重新打开后会自动提示「上一次生成被中断，可重新生成」，不会再一直停在“生成中”；漫画项目页在有图片生成任务时会自动刷新，完成或中断后界面立即恢复。
- 漫剧列表卡片更紧凑：宽度收窄一半（一行最多 6 张）、高度增加；卡片显示章数与全文字数，右上角新增删除按钮，确认后连同小说正文与已生成的分镜、配音、视频数据一并清理。
- 漫剧列表卡片精简：去掉封面预览图，与小说书架一致的紧凑卡片；点击卡片任意位置即可打开漫剧工作室。
- 新增「画面风格」：小说封面、角色立绘、拆书角色形象图和漫画画风可选用统一的画面风格预设（3D玄幻国漫、动漫、写实现代、写实古装剧、民国年代剧、写实末日），同一次生成的多张图保持同一画面质感；风格只决定媒介与质感，不会改动你的角色、服装与场景设定。
- 支持自定义画面风格：上传一张参考图，AI 自动提炼画面质感生成风格草稿，确认保存后即可在各生图入口选用；也可以手动填写渲染说明与禁止项。漫画项目选中画面风格后，整本漫画的角色、场景与格子图都会使用该风格。
- 短篇「编辑正文」支持查找高亮：输入关键词后全部匹配实时标黄，Enter 或按钮可逐个跳转（Ctrl+F 唤起、Esc 清除），当前跳转到的匹配用醒目颜色区分，定位人名、地名、情节更直接。
- 短篇「编辑正文」新增行号与统计：编辑区左侧显示全篇连续行号（软换行自动对齐，窄屏自动收起），底部实时显示字数、行数、段落数和光标所在行列。
- 新增「漫剧」完整工作流：侧边栏新增「漫剧列表」，创建漫剧只需要一个作品名。每个漫剧项目都是从写小说到动态漫视频的流水线——先写小说，小说有成稿后就能生成分镜（每个镜头有画面描述、台词、时长与运镜，并绘制首帧图），再做逐镜配音，最后交给视频通道合成动态漫视频；支持边写小说边做后期，新章节随时同步进分镜。
- 漫剧项目的小说阶段提供「我的大纲」工作台：写下简略大纲，AI 会推理出分章细纲（每章标题、剧情梗概、关键事件、出场角色与场景），逐章可修改、删除、调序，确认后作为剧情契约交给 AI 写作——AI 只做扩写与节奏补充，不会推翻你的剧情走向；也可以先在「设定」里自建角色、场景、道具和世界观再开写。
- 漫剧项目与小说分开管理：漫剧项目只出现在「漫剧列表」，小说列表和首页不会再显示这些作品。
- 简易模式作品的设定编辑恢复可用：设定中心的角色/场景/道具/世界观修改可以正常保存，不会再被“仅供阅读”提示拦截。
- 视频阶段会在未接入外部视频服务时明确提示：可先完成首帧图与配音并导出素材，接入视频服务地址后即可生成动态漫视频。
- 产品定位为纯网页应用：设置里的「桌面与维护」页面移除，桌面更新提示等桌面版专属入口一并去掉，设置导航更聚焦创作配置；网页端所有创作功能不受影响。
- 侧边栏更聚焦创作：题材基底库和推进模式库移入“系统”分组——开书时在小说基础信息里选择、或用一句话灵感推荐即可，需要自定义分类时再从这里进。
- 小说导入漫画时角色设定完整带入：性别、年龄段与「面部锚点」会随角色一起进入漫画项目，漫画角色设定图的面部一致性直接使用小说设定中心维护的面部特征，整体外貌与着装作为视觉提示，减少两边角色形象不一致。
- 小说列表简化为单一书架视图：作品以紧凑的横版卡片展示（创作进度、常用入口），不再区分内容几乎相同的“书架/工作台”两种视图；打开 AI 驾驶舱的入口直接放在作品卡片上。
- 书架作品卡片去掉封面并收窄一半：卡片只保留书名、创作进度和常用操作，一屏能同时看到更多作品。
- 书架与创建页暂时收起「AI 自动导演开书」入口：新建小说从「创建小说」直接填写书名和基础信息开始，已在创作中的作品不受影响。
- 写法引擎焕新：正在使用的写法固定放大置顶展示，其他写法统一尺寸排列、点击即切换；切换后的选择会被记住，下次进入仍然生效；页面配色跟随应用主题。
- 拆书支持导入外部小说：新建拆书时可直接选择本机 txt 文件（自动识别常见中文编码），导入后自动出现在文档列表并选中；选中文件后会显示文件名与导入状态。温度、最大 Tokens、预算上限等专家参数从新建弹窗移除，分析统一使用系统设置中的默认文本模型。
- 移除“视觉资源库”：它只是聚合展示各处已有图片的只读页面，没有实际使用场景；世界设定中的“地图与图谱”占位入口一并移除。
- 标题生成更快：默认一次给出 5 个精选标题候选；AI 会一次多产出几个候选再自动挑出风格和句式最有差异的一组，不再整批重新生成，等待时间大幅缩短。
- 设定中心支持按提示现场生成实体：添加角色/场景/道具时写一句提示（如「男大学生」），AI 会生成完整草稿——角色包含随机姓名、性别、年龄段、体型、性格、外貌、着装与背景；生成后可随意修改再保存，提示里写的约束会被保留。
- 角色新增「面部锚点」、场景新增「环境提示词」、道具新增「视觉提示词」：这些字段按图片生成所需的格式生成与编辑，为后续一键生成角色立绘、场景图、道具图做准备。
- 首次自动生成的设定同样携带这些图片提示词字段。
- 生成任务更省时：短篇整体规划采用更紧凑的输出格式，规划阶段的等待时间明显缩短；正文逐段续写与全篇审校保持不变。
- 模型设置的文本模型卡片提供“思考模式”开关：开启后模型先推理再输出、结果更稳，追求速度可以关闭；不提供思考模式的模型和本机订阅通道不受影响。
- 任务详情新增“模型调用耗时”：逐步列出这次任务每次模型调用的时间、使用的模型、耗时、token 用量和失败原因，慢在哪一步一眼可见。
- 短篇与简易创作新增「设定中心」：作品页可以切换查看和管理角色、场景（地点）、关键道具和世界观（含世界地图与关键设定），全部支持手动编辑与 AI 重新生成。
- 动笔前先立设定：AI 会根据小说想法自动生成角色、场景、道具与世界观草稿；短篇在开始写作前会显示「设定已就绪」，可一键采纳开始写作，也可先调整设定再写。
- 正文生成会遵守设定：角色言行、场景氛围、道具功能、世界观规则都会作为写作约束注入，减少内容“漂浮”和凭空冒出的设定。
- 简易模式在继续创作前会自动补全缺失的场景与道具设定，让后续章节有据可依。
- 提示词管理页面跟随应用主题显示：深色、夜航、宣纸等皮肤下，提示词目录、槽位编辑、预览与上下文面板的配色与全站保持一致。
- 模型设置按能力分类：文本模型、图片模型、音频模型。配置好文本模型后，大纲、正文、审校、修复等全部文字任务自动使用该模型，各任务保留各自的创作参数。
- 音频模型可直接配置：默认连接本机 VoxCPM2 语音服务，用于角色配音与朗读；也可以把服务地址改成其他兼容 OpenAI /v1/audio/speech 接口的语音服务。本地语音生成不消耗云端额度，无需填写 API Key。
- 音频模型的“测试连接”会合成一句固定短语，验证服务地址、密钥与模型整体可用。
- 更换模型供应商时，直接修改对应类别的服务地址、API Key 和模型名称即可，全部任务随即切换到新模型。
- 各创作页面的模型选择简化为直接选择文本模型；新手引导只需连接一次文本模型，检测通过后即可开始创作。
- 小说封面、角色立绘、场景插图等图片任务统一使用图片模型类别的配置。
- 模型设置会识别本机订阅通道（OpenCode、Codex 订阅）：这类通道直接显示“已连接本机订阅通道，不需要填写 API Key”，不会再出现空白的密钥输入框；把服务地址改为其他供应商后，会自动恢复密钥填写方式。
- 文本模型的“测试连接”会真实试写一段文字并检查结构化输出；图片模型的“测试连接”会检查图片通道是否可用并列出可用模型。
- AI 创作实况的每条调用记录会显示发生时间、用途和耗时，展开后可查看内部标识与使用的模型；测试连接类调用会明确标注“测试连接”用途。
- 拆书、创作工坊、对话等功能在没有单独指定模型时，会统一使用模型设置中的文本模型，不会再回退到未配置的默认厂商而报“未配置 API Key”或服务器错误；界面上的服务器错误提示会附带状态码，便于定位。
- 全部创作功能完成模型默认值排查：世界观、写法引擎、标题工坊、题材、写作公式、章节编辑、小说核心生产链等文字任务统一使用文本模型；封面、角色立绘、漫画分格、剧照等图片任务统一使用图片模型，不再出现“用错模型”或要求配置未使用厂商的 Key。
- 拆书结果工具栏新增“删除”：确认后删除该次拆书结果与角色档案，来源文档保持不变，可随时重新拆书；重新解析历史拆书任务时会自动改用当前文本模型，旧任务不再因历史模型配置失败。
- OpenCode Go 本地文本通道：使用本机已登录的 OpenCode 订阅，不需要额外 API Key；执行 `pnpm opencode:bridge` 即可启动本地模型服务，已经在运行的服务会自动复用。本地通道同时支持普通调用和结构化输出调用，JSON 类任务会自动附带输出协议约束；长篇章节生成的等待上限放宽到 15 分钟。
- Codex 本地图片通道：直接选用 Codex 订阅的内置图片能力，不需要额外 API Key；执行 `pnpm codex:image` 即可启动本地图片通道，封面竖版尺寸、参考图融合等常用能力均可直接使用。
...
- 新增 Codex 图片本地模型：小说封面、角色立绘等图片生成可以直接选用 Codex 订阅的内置图片能力，不需要额外 API Key。
- 执行 `pnpm codex:image` 即可启动本地图片通道；封面竖版尺寸、参考图融合等常用能力均可直接使用。
- 漫画工作台支持删除项目：项目卡片提供删除入口，确认弹窗会标明将一并删除的分集、角色、分格与已生成图片。
- 书架与首页的作品封面改为程序化生成：未生成封面的作品按书名自动配色，直接把书名渲染成封面，不再显示通用占位图；已生成或已上传的封面仍会优先展示。
- 漫画的角色设计稿与格子图生成直接使用模型设置中配置的图片模型：配置 OpenAI 或本机 Codex 订阅通道的 gpt-image-2 即可直接生成，无需在漫画页单独选择图片服务。
- 拆书失败时会明确显示失败状态和具体原因（例如模型通道未配置），可以一键重试；不再出现“任务完成但所有栏目都是空的”的假成功。
