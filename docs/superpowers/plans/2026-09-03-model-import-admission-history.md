...
- [x] **任务 4：实现 preflight/stage/publish 工作流边界**
  - 创建仓库内导入工作流门面；`preflight` 只输出计划，先跳过历史拒绝项，再安排转换。
  - 暂存报告必须包含浏览器方形预览审计、当前资源哈希、GLB/贴图/尺寸门禁结果；`--check-staged` 和 `curate-cine57-library.mjs --staged-report` 在目录写入前拒绝不完整报告。
  - 对失败候选写 rejected 台账事件和可恢复隔离记录；外部历史构建器只负责候选转换，不能绕过仓库门禁决定发布。
  - CLI 检查无副作用；缺预览、非方形截图或哈希变化时不会通过 staged 发布检查。
...
```text
pnpm exec node --experimental-strip-types --test scripts/models/model-library-import-history.test.mjs scripts/models/model-library-import-admission.test.mjs scripts/models/model-library-expansion-candidates.test.mjs scripts/models/model-library-import-history-generation.test.mjs scripts/models/model-library-import-workflow.test.mjs
