...
- 在 blocking 姿势模块集中声明每个业务姿势的渲染方式：真实 UAL2 片段优先；`lying`/`prone` 缺少专用片段时使用真实 standing 片段加明确的贴地局部旋转，返回的业务 pose 仍为请求值；其它缺失片段直接报资源能力错误。
...
- UAL2 没有真实躺姿片段，贴地旋转是有限的代理展示，不声称替代真实动作；业务 pose 会保留，后续替换资源时可自动优先使用真实片段。
