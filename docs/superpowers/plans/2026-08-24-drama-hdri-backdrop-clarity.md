...
**Architecture:** PlayCanvas 环境层按纹理宽高比选择标准等距 DomeGeometry，或普通场景图的上半球 + 带贴图的下半球网格，模拟 Unreal `EnviroDome` 的弧形地面。环境参数由 viewer 持有、由右侧控件调整、由 `layout3d.environment` 保存；普通场景图的下半球按世界投射中心 `(0, projectionCenterHeight, 0)` 计算单次 UV，X/Z 不可偏移，旧布局缺少参数时使用默认值，地面中心永远保持在世界 Y=0。
...
- [ ] **Step 3: 实现纹理和环境参数**：设置线性采样、关闭 mipmap、设备最大各向异性过滤、U/V 边缘寻址；以 `domeRadius`、`yawDeg`、`intensity` 更新天空和地面网格，以投射中心 `(0, projectionCenterHeight, 0)` 重建下半球 UV；保持环境世界坐标固定，地面中心位于 Y=0。
