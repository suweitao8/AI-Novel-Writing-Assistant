...
**Architecture:** PlayCanvas 环境层按纹理宽高比选择标准等距 DomeGeometry，或普通场景图的上半球 + 带贴图的下半球网格，模拟 Unreal `EnviroDome` 的弧形地面。环境参数由 viewer 持有、由右侧控件调整、由 `layout3d.environment` 保存；旧布局缺少参数时使用默认值，地面永远保持在世界 Y=0。
