...
**Architecture:** PlayCanvas 环境层按纹理宽高比选择标准等距 DomeGeometry 或普通场景图地面投影 UV。环境参数由 viewer 持有、由右侧控件调整、由 `layout3d.environment` 保存；旧布局缺少参数时使用默认值，地面永远保持在世界 Y=0。
...
- [ ] **Step 2: 实现最小几何修复**：根据 `texture.width / texture.height` 判断 2:1 等距 HDRI；非 2:1 时，在 `DomeGeometry` 下半球把 UV 改为方位角 + 地面半径采样，避免底部像素集中拉伸。
