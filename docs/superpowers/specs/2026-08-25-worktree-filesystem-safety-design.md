...
4. 先执行 `git worktree remove --force <validated-path>`，随后确认登记已消失、路径已消失，最后才删除本地分支。
5. Git 删除失败时保留目录和分支，输出原始错误与人工处理建议；不使用 `Remove-Item -Recurse`、`rmdir /s` 或未解析的 glob 作为兜底。

`--force` 只在目标已通过路径边界和 reparse-point 检查后使用，且只作用于 Git 登记的精确工作树路径。这样可以清理正常工作树中的本地 `node_modules`，同时避免跨工作树 Junction 被 Git 删除流程跟随。
