...
- **仍断链（后续工作）**：状态图已可生成（2026-08-19，见 story-settings-hub.md 状态图片生成节），但**首帧图还不消费状态**——【角色状态】标记的逐镜解析未实现、keyframe 的角色参考图仍只取 DramaCharacter.portraitData（后续可扩展为「按当前状态取对应状态图」）；分镜的道具没有结构化引用（靠文本匹配，道具名与镜头描述写法不一致时静默不生效）。
