...
- 「参考」子页签：**预览/编辑两态 + 按章回落**。本章有参考文本时是 50 行 `LineNumberedTextarea` 编辑器（头部标注字数与「解析/提取使用这份文本」）；本章还没有参考文本时**只读展示参考源**——参考小说先按章节标题行**确定性切分**（`referenceChapters.ts` 的 `splitReferenceChapters`：「第N章/回/节 标题」优先，中文数字到 9999；全篇没有时退「N、/N. 标题」编号式且要求 ≥2 处；重复章号以首次出现为准、其后内容不并入前章），**工作室第 N 章回落到参考小说第 N 章**（含标题行，截前 2 万字，`getKnowledgeDocument` 拉取、`drama-reference-doc` 查询键缓存）；切不出章节结构的文件才按整本回落；本章序号超出参考小说章节数时无回落内容、只留「粘贴新文本」入口并说明原因。「解析」「提取」直接用回落源文本；「复制为本章参考」一键把该章节写入本章字段并走自动保存，「粘贴新文本」进入空白编辑态（首个字符才落本章字段）。**为什么按章对应**：整本包含后面章节的内容，第 1 章的初稿/提取拿到第 2、3 章的剧情是错误输入（2026-08-20 用户反馈「6000 多字把第二章也带进来了」）；章节标题行是小说文本的强约定，程序切分属确定性解析非 AI 路径。**禁止把回落内容当可编辑框展示**——同日用户在整本回落的编辑框里粘贴同一本小说叠出多份重复文本，分态后该路径不存在。**参考文本服务端持久化**：本章正文存 `Chapter.referenceText`（`PUT /chapters/:chapterId`，1.2s 防抖静默保存，切章/卸载冲保存，与 expectation 同链路同守卫白名单；子页签行右侧同款「自动保存中…/还有未保存的修改…」指示）。**不使用浏览器 localStorage**——用户内嵌浏览器本地存储不可靠（写入静默失败/重载即丢），曾导致参考文本与提取建议凭空消失（见 debugging/drama-studio-local-storage-loss.md）。「解析」按钮在该子页签行右侧（与初稿的解析同位；mutation 与参考文本状态收敛在 `useReferenceDraftStage`，替换确认弹窗挂页级、切页签不丢），走 `POST /novels/:id/chapters/:chapterId/reference-draft/preview`（Prompt 资产 `novel.chapter.reference_draft@v5`：**先剔除非正文内容**——书名、章节标题行、作者感言、求票求收藏等元信息不得进入初稿。输出是**分镜式初稿**：整章拆成 10～16 个分镜单元（schema min 8 max 18），每单元 `shot/storyboard/speaker/kind/mood/text`——shot 是必选景别（大远景/远景/全景/中景/近景/特写），storyboard 写这个画面里正在发生什么（谁在画面中、动作/姿势/神态、环境，≤40 字，初步分镜不用太细），第二行是旁白（speaker 固定「旁白」）或台词（保留原文角色名、mood 标表情神态，作为后续配音的情绪提示）；服务端把每单元拼成两行 `分镜：景别，画面` + `说话人（神态）：内容`，单元间空行，即 draftText。postValidate 强制单元去重与说话人规则（narration 强制 speaker=旁白并清空 mood）。v3→v4 强制景别，v4→v5 mood 明确为配音情绪提示），服务端只做纯预览不落库。前端拿到 `draftText` 后经 `useNovelChapterWorkspace.applyExpectationText` 替换初稿并**立即静默落库**（不等初稿页签的 1.2s 防抖）；初稿已有内容时先弹「替换本章初稿」确认框（弹窗内可预览全文）。该端点已加入简易模式写守卫的漫剧工作台白名单（仅 POST preview 形状）。新建章节的标题预填（`CreateChapterDialog`）同样从切分结果取「第N章 标题」（`collectReferenceChapterTitles`，由 `splitReferenceChapters` 派生；文档加载完成前用户已手动输入时不覆盖）。
- 「提取」子页签（参考与初稿之间）：从当前参考文本提取设定建议——角色（name/role/description）、场景、世界观条目，走 `POST /novels/:id/chapters/:chapterId/reference-extract/preview`（Prompt 资产 `novel.chapter.reference_extract@v2`，忽略非正文内容、只提取原文明确出现的不虚构；**宁多勿漏**——建议由用户逐条筛选确认，漏提比多提更影响使用，凡有名字或有台词的角色、每个独立地点、每类核心设定概念都要提，每类上限 16 条；postValidate 命中「示例文本」等占位内容直接判无效触发修复重试（弱模型可能照抄结构化骨架的占位示例，已实测发生过）；服务端纯预览不落库，写守卫按 `reference-(draft|extract)` 形状放行 POST）。提取结果是页级内存态（重开页面点一次「提取」即可恢复；参考文本已在服务端，不会因重载丢输入），用户勾选确认后创建进设定中心：角色走 `createStorySettingsCharacter`（role=定位、personality=描述）、场景走 `createStorySettingsScene`（summary=描述）、世界观条目读取现有 world 文档后按标题去重合并进 `keySettings`（PUT 为部分更新，只传 keySettings 不动 premise）。创建成功或已存在同名的条目从建议列表移除，失败的保留供重试；创建后失效 settings 相关缓存。「提取」按钮在该子页签行右侧；**禁用原因必须内联可见**（按钮旁灰字 + 空态卡里说明本次提取用的参考来源与「十几秒」耗时预期）——只放 title 悬浮提示在内嵌浏览器里等于没有提示，曾让用户以为按钮坏了。
