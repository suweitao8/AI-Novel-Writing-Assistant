...
**逐镜配音操作（2026-08-23）**：配音列表是“每一镜一个操作单元”，正文区只展示一次旁白/对白；下方音频控制行提供可拖动试听控件，时长只显示媒体实际加载出的「当前 / 总」值，未生成或过期的段不显示伪时长。生成/重新生成按钮与控制行同排靠右。本镜存在缺失或过期段时按钮为「生成配音」，调用单镜接口且只补需要生成的段；本镜所有段都可播放时按钮变为「重新生成」，以 `force=true` 重做本镜全部段。顶部批量生成仍保留，用于整集操作，但不能替代逐镜入口。生成中按钮只锁定当前镜并显示进行中状态，完成后失效查询刷新该章的音频段状态。
...
- **语音通道**：IndexTTS 2.5 整合包 `D:\Tools\yzy-index-tts-2.5-260824\启动.bat` 提供 9000 网页工作台，网页的「API 服务」页启动 `app_api.py`（默认 9005）。**台词情绪链路**：分镜台词行约定「角色名（语气）：台词」（`drama.storyboard@v3` 生成时写入，初稿解析 `novel.chapter.reference_parse@v4` 的 mood 同源语义；v3 起两处都要求角色用本名、禁「妹妹」等称谓，drama.storyboard 的 action 另需写明位置姿态且同地点相邻镜头位置连贯，characterRefs 列画面可见角色全名——首帧图按 characterRefs 名字挂角色参考图）；`parseDialogueLines` 把（语气）拆成独立 `emotion` 字段、角色名保持干净用于匹配角色音色；配音时逐行 emotion 交给 IndexTTS 适配层，服务支持 QwenEmotion 时使用 `emo_text`，否则保留参考音色模式；行内语气优先于角色默认情绪（voice.emotion/voicePrompt），旁白行（含「旁白：」前缀行）用旁白音色描述；`buildDialogueVoiceKey` 把行内语气纳入音色指纹——语气变化会使已有音频判 stale 需重配。
...
- **音色（2026-08-19 接线）**：`Character.voiceTexture` → `SourceCharacter.voicePrompt`（契约新增字段）→ `DramaCharacter.voiceProfile` 初始 `{voicePrompt}`（与 `readCharacterVoice` 的解析键对齐），配音链路（IndexTTS 2.5/HTTP TTS）直接可用；用户在音色面板设计过的 voiceProfile 优先于初始值。
