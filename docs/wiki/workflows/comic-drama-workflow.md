...
- 当前实现约束：分镜工具行的操作按钮统一靠右；静态分镜画面与成片使用横屏规格（画面 `IMAGE_SPECS.dramaKeyframe=1536x864`，整集输出 `1920x1080` / `16:9`）。配音不由页面选择 provider，批量与单镜头入口都解析系统设置里的音频模型槽位；配音段只有在当前系统 provider 返回真实音频时才投影为 ready。浏览器播放器从媒体元数据读取当前/总时长，整集合成用 ffprobe 测量每个音频文件建立时间轴，不以镜头计划秒数、字数或缺省秒数替代；旧模拟配音记录必须重新生成。
