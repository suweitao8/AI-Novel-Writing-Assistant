...
- 章节级的「参考初稿解析」（`novel.chapter.reference_draft@v4`，粘贴原文解析分镜式初稿）与本字段是两回事：前者是章节工作台的即时解析输入，后者是项目级参考资料库。

### 「当前」阶段的章节管理与单章节拍

- 「参考」子页签：粘贴参考小说原文（50 行 `LineNumberedTextarea`，无 placeholder 等附加文案）。**参考文本服务端持久化**：本章正文存 `Chapter.referenceText`（`PUT /chapters/:chapterId`，1.2s 防抖静默保存，切章/卸载冲保存，与 expectation 同链路同守卫白名单）；某章还没有自己的参考文本时回落展示整本参考小说开头（创建漫剧时上传的知识库文档 `versions[isActive].content` 截前 20000 字，`getKnowledgeDocument` 拉取、`drama-reference-doc` 查询键缓存），编辑即落到本章字段。**不使用浏览器 localStorage**——用户内嵌浏览器本地存储不可靠（写入静默失败/重载即丢），曾导致参考文本与提取建议凭空消失（见 debugging/drama-studio-local-storage-loss.md）。「解析」按钮在该子页签行右侧（与初稿的解析同位；mutation 与参考文本状态收敛在 `useReferenceDraftStage`，替换确认弹窗挂页级、切页签不丢），走 `POST /novels/:id/chapters/:chapterId/reference-draft/preview`（Prompt 资产 `novel.chapter.reference_draft@v4`：**先剔除非正文内容**——书名、章节标题行、作者感言、求票求收藏等元信息不得进入初稿。输出是**分镜式初稿**：整章拆成 10～16 个分镜单元（schema min 8 max 18），每单元 `shot/storyboard/speaker/kind/mood/text`——shot 是必选景别（大远景/远景/全景/中景/近景/特写），storyboard 写这个画面里正在发生什么（谁在画面中、动作/姿势/神态、环境，≤40 字，初步分镜不用太细），第二行是旁白（speaker 固定「旁白」）或台词（保留原文角色名、mood 标表情神态）；服务端把每单元拼成两行 `分镜：景别，画面` + `说话人（神态）：内容`，单元间空行，即 draftText。postValidate 强制单元去重与说话人规则（narration 强制 speaker=旁白并清空 mood）。v3→v4 变更即强制景别字段），服务端只做纯预览不落库。前端拿到 `draftText` 后经 `useNovelChapterWorkspace.applyExpectationText` 替换初稿并**立即静默落库**（不等初稿页签的 1.2s 防抖）；初稿已有内容时先弹「替换本章初稿」确认框（弹窗内可预览全文）。该端点已加入简易模式写守卫的漫剧工作台白名单（仅 POST preview 形状）。新建章节的标题预填（`CreateChapterDialog`）同样改为从知识库文档解析「第N章 标题」行（`collectReferenceChapterTitles`，确定性解析非 AI 路径；文档加载完成前用户已手动输入时不覆盖）。
- 「提取」子页签（参考与初稿之间）：从当前参考文本提取设定建议——角色（name/role/description）、场景、世界观条目，走 `POST /novels/:id/chapters/:chapterId/reference-extract/preview`（Prompt 资产 `novel.chapter.reference_extract@v2`，忽略非正文内容、只提取原文明确出现的不虚构；**宁多勿漏**——建议由用户逐条筛选确认，漏提比多提更影响使用，凡有名字或有台词的角色、每个独立地点、每类核心设定概念都要提，每类上限 16 条；postValidate 命中「示例文本」等占位内容直接判无效触发修复重试（弱模型可能照抄结构化骨架的占位示例，已实测发生过）；服务端纯预览不落库，写守卫按 `reference-(draft|extract)` 形状放行 POST）。提取结果是页级内存态（重开页面点一次「提取」即可恢复；参考文本已在服务端，不会因重载丢输入），用户勾选确认后创建进设定中心：角色走 `createStorySettingsCharacter`（role=定位、personality=描述）、场景走 `createStorySettingsScene`（summary=描述）、世界观条目读取现有 world 文档后按标题去重合并进 `keySettings`（PUT 为部分更新，只传 keySettings 不动 premise）。创建成功或已存在同名的条目从建议列表移除，失败的保留供重试；创建后失效 settings 相关缓存。「提取」按钮在该子页签行右侧。
