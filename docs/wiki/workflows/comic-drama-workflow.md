...
- 分镜采用**影视分镜镜头**（drama shots：静态分镜画面 + 台词 + 时长）而不是漫画分格：镜头数据直接衔接配音、字幕与成片时间轴，画面本身不做图生视频或运镜动画。**漫剧只出静态分镜（2026-08-28 用户决定）**：`drama.storyboard` schema、手动编辑路由、3D 草图上下文、自动构图输入与导出时间轴均不再包含 `cameraMove`（运镜）字段，UI 也不展示；`durationSec` 仅作为成片时间轴/静音镜头时长保留，DB 旧列保留只读兼容。
...
- 工作台按钮契约：脚本页和没有项目时的分镜空状态都只显示「生成分镜」，按钮使用右上角当前章节序号；脚本仍在保存、保存失败、或为空时禁用。已有分镜的工具栏只保留「生成分镜」「生成配音/重新配音」「合成」，调用当前章节对应的批量任务与 `GET/POST /projects/:id/episodes/:order/assembly`，不在列表内重复提供章节选择或项目级音色设置。
- 当前实现约束：分镜工具行的操作按钮统一靠右；静态分镜画面与成片使用横屏规格（画面 `IMAGE_SPECS.dramaKeyframe=1536x864`，整集输出 `1920x1080` / `16:9`）。配音不由页面选择 provider，批量与单镜头入口都解析系统设置里的音频模型槽位和系统旁白参考音频；配音段只有在当前系统 provider 返回真实音频时才投影为 ready。浏览器播放器从媒体元数据读取当前/总时长，整集合成用 ffprobe 测量每个音频文件建立时间轴，不以镜头计划秒数、字数或缺省秒数替代；旧模拟配音记录必须重新生成。

### 系统旁白音色边界

- 整个网站的旁白使用同一个系统级音色，权威配置是 `AppSetting` 的 `drama.globalNarratorVoice`，不是 `DramaProject.narratorVoiceData`。系统设置页面提供描述保存、固定样句试听和重新生成；旁白合成与音频段 stale 判断必须读取同一份状态。
- 参考音频和描述共同决定旁白音色指纹。合成请求把全局 `sampleAudioUrl` 传给音频 provider；`voiceKey` 保存 `sampleSha256`，避免把完整 data URL 重复写进每条台词，同时确保换样本后旧音频变为 stale。
- 为兼容已有数据，首次读取全局 key 且为空时才从旧项目字段迁移一个有效旁白；显式导入旧项目样本使用 `server/scripts/import-drama-narrator-voice.cjs`，运行时不依赖旧项目目录，也不删除旧音频。
...
**逐镜配音操作（2026-08-23）**：配音列表是“每一镜一个操作单元”，正文区只展示一次旁白/对白；下方音频控制行提供可拖动试听控件，时长只显示媒体实际加载出的「当前 / 总」值，未生成或过期的段不显示伪时长。生成/重新生成按钮与控制行同排靠右。本镜存在缺失或过期段时按钮为「生成配音」，调用单镜接口且只补需要生成的段；本镜所有段都可播放时按钮变为「重新生成」，以 `force=true` 重做本镜全部段。顶部批量生成仍保留，用于整集操作，但不能替代逐镜入口。多镜配音可以同时进行：每个镜头独立显示自己的进行中状态、互不覆盖；同镜重复触发在服务端合并为一次执行，全局合成闸门统一限制并发，不会中断已开始的任务（详见 comic-drama-voice-overdub 的「并发与队列」）。完成后失效查询刷新该章的音频段状态。
...
漫剧生成链同时消费三类参考资产和横屏首帧。参考资产的固定视图是为了让后续模型识别角色、空间和道具，首帧则必须服务于镜头构图；把参考图版式原样塞进首帧会造成画幅冲突和错误构图。
...
- **音色（2026-08-19 接线）**：`Character.voiceTexture` → `SourceCharacter.voicePrompt`（契约新增字段）→ `DramaCharacter.voiceProfile` 初始 `{voicePrompt}`（与 `readCharacterVoice` 的解析键对齐），配音链路（VoxCPM2/HTTP TTS）直接可用；用户在音色面板设计过的 voiceProfile 优先于初始值。
...
- 供应商出现大量超时或限流：先检查 provider 侧配额，并将任务并发调整到 1–4 路的安全范围内；不要通过无限扩大前端请求数量规避问题。
