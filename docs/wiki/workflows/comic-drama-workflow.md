...
- 工作室顶栏统一承载：返回按钮（图标）+项目名、居中的五阶段主 tab（小说/资产/分镜/配音/视频；「资产」页签内嵌 `StorySettingsTabs` 全量设定中心）与小说阶段子 tab（大纲/细纲），右侧放当前阶段的操作按钮（小说=当前章按钮（如「1 · 重生」，点开章节管理弹窗）+AI 写作进度，分镜=同步最新章节/打开分镜工作台，视频=打开视频工作台）。小说阶段按「当前章」创作：`useNovelChapterWorkspace`（comicDrama/hooks）持有当前章（默认第一个无正文章，否则最后一章）、本章大纲 expectation（1.2s 防抖静默自动保存、失焦/切页签/切章即冲保存、空白铺 50 行）与本章细纲 beats 草稿；切章时先落库上一章再重置。子页签行右侧的「解析」按钮=按本章大纲推理本章细纲（`previewChapterDetailOutline`，成功后切到细纲页签）；自动导演接管（让 AI 开始/继续创作）的入口在章节管理弹窗页脚，不再占用顶栏。大纲编辑区是 `LineNumberedTextarea`，基于开源 CodeMirror 6（`@uiw/react-codemirror` + `@codemirror/view`，client 直接依赖）：行号固定编辑器最左侧且只读、软换行、当前行/行号高亮、默认 50 行起随内容自动增高；颜色全部走语义 token（CSS 变量）随明暗主题自适应；minRows 换算 minHeight，maxLength 在 onChange 截断。不要再用「透明 textarea + 排版镜像」方案重写该组件——对齐维护成本高，已废弃。highlight prop 接收设定名单（角色/场景/道具名）做实体高亮：MatchDecorator + lookbehind/lookahead 断词（\p{L}\p{N}），长名优先、≥2 字才匹配，三类各用一种色调（角色 primary token、场景 emerald、道具 amber，后两者是组件内色调映射允许的调色板原色）。大纲页签右侧的 `OutlineSettingsAside` 复用 createStorySettingsCharacter/Scene/Prop 与 queryKeys.novels.storySettings*（与设定页签共享缓存，创建后互相同步）。面板 lg:sticky 固定右栏、max-h 视口高度内部滚动，卡片按 updatedAt 倒序（最近使用优先），搜索为提交制（输入 + 回车/搜索按钮提交 appliedKeyword），新增走弹窗（类型/角色定位/名称/一句话说明）。空白大纲会在前端一次性铺 50 行换行（NovelOutlineTab 的 seededRef，trim 后判空所以纯换行不会触发自动保存），修改走静默自动保存（`saveOutlineMutation` 的 `silent` 参数，1.2s 防抖、失焦与切页签即冲保存，成功不弹 toast、失败仍报错）。
...
- 工作室小说阶段常驻两个子 tab（大纲/细纲），都作用于当前章：大纲页签=本章 expectation 的 CodeMirror 编辑区 + 右侧 `OutlineSettingsAside`（快速查找与创建）；细纲页签=本章 beats 编辑器（3～10 拍，逐拍可改可增删，保存走 `saveChapterDetailOutline`）。章节管理弹窗（`ChapterManageDialog` 承载 `ChapterManagePanel`）点卡片=切换当前章（父级 `switchChapter` 落库上一章并重置编辑态，弹窗关闭），支持搜索与手动新建；单章详情编辑不再放弹窗里。全书级大纲（`Novel.outline`）不再在工作室编辑，只保留空白小说书架的 `BlankStartPanel` 三步流（`useNovelOutlineWorkspace`）。
