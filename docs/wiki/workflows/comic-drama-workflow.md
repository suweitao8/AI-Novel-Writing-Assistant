...
- 分镜采用**影视分镜镜头**（drama shots：静态分镜画面 + 台词 + 时长/运镜）而不是漫画分格：镜头数据直接衔接配音与成片合成，画面本身不做图生视频或运镜动画。
...
- 当前实现约束：分镜工具行的操作按钮统一靠右；静态分镜画面与成片使用横屏规格（画面 `IMAGE_SPECS.dramaKeyframe=1536x864`，整集输出 `1920x1080` / `16:9`）。配音不由页面选择 provider，批量与单镜头入口都解析系统设置里的音频模型槽位。
...
**逐镜配音操作（2026-08-23）**：配音列表是“每一镜一个操作单元”，行内的音频段分别提供原生试听控件；本镜存在缺失或过期段时按钮为「生成配音」，调用单镜接口且只补需要生成的段；本镜所有段都可播放时按钮变为「重新生成」，以 `force=true` 重做本镜全部段。试听、状态和生成入口统一放在行内「音频」区域，标题区只保留镜头信息；当音频段存在时不再重复渲染镜头的对白/旁白摘要。顶部批量生成仍保留，用于整集操作，但不能替代逐镜入口。生成中按钮只锁定当前镜并显示进行中状态，完成后失效查询刷新该章的音频段状态。
