...
- 「参考」子页签：粘贴参考小说原文（50 行 `LineNumberedTextarea`，无 placeholder 等附加文案；参考文本按「小说+章」键存浏览器 localStorage、粘贴即写穿自动保存、切章载入对应章文本，刷新不丢——不落服务端，正式产物是初稿。某章还没有自己的参考文本时回落载入创建时上传的整本参考小说（source 槽位），用户截取本章相关部分再解析；一旦编辑过即写入该章自己的槽位），「解析」按钮在该子页签行右侧（与初稿的解析同位；mutation 与参考文本状态收敛在 `useReferenceDraftStage`，替换确认弹窗挂页级、切页签不丢），走 `POST /novels/:id/chapters/:chapterId/reference-draft/preview`（Prompt 资产 `novel.chapter.reference_draft@v2`：**先剔除非正文内容**——书名、章节标题行、作者感言、求票求收藏等元信息不得进入初稿；旁白行优先保留有画面感的内容——动作、神态、场景与环境描写（漫剧分镜的画面素材），其次情节推进；15～25 行、目标 20 行，每行 `speaker/kind/text`——旁白行 speaker 固定「旁白」，台词行保留原文角色名；postValidate 强制行去重与说话人规则。v1→v2 变更即上述忽略非正文 + 画面感两条），服务端只做纯预览不落库。前端拿到 `draftText` 后经 `useNovelChapterWorkspace.applyExpectationText` 替换初稿并**立即静默落库**（不等初稿页签的 1.2s 防抖）；初稿已有内容时先弹「替换本章初稿」确认框（弹窗内可预览全文）。该端点已加入简易模式写守卫的漫剧工作台白名单（仅 POST preview 形状）。
- 「提取」子页签（参考与初稿之间）：从当前参考文本提取设定建议——角色（name/role/description）、场景、世界观条目，走 `POST /novels/:id/chapters/:chapterId/reference-extract/preview`（Prompt 资产 `novel.chapter.reference_extract@v1`，忽略非正文内容、只提取原文明确出现的不虚构、每类上限约 12 条；服务端纯预览不落库，写守卫按 `reference-(draft|extract)` 形状放行 POST）。结果按小说存 localStorage（`drama-studio-extract:${novelId}`），用户勾选确认后创建进设定中心：角色走 `createStorySettingsCharacter`（role=定位、personality=描述）、场景走 `createStorySettingsScene`（summary=描述）、世界观条目读取现有 world 文档后按标题去重合并进 `keySettings`（PUT 为部分更新，只传 keySettings 不动 premise）。创建成功或已存在同名的条目从建议列表移除，失败的保留供重试；创建后失效 settings 相关缓存。「提取」按钮在该子页签行右侧。
