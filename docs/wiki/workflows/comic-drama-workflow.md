...
**逐镜配音操作（2026-08-23）**：配音列表是“每一镜一个操作单元”，正文区只展示一次旁白/对白；下方音频控制行分别提供原生、可拖动的试听控件和已知秒数，生成/重新生成按钮与控制行同排靠右。本镜存在缺失或过期段时按钮为「生成配音」，调用单镜接口且只补需要生成的段；本镜所有段都可播放时按钮变为「重新生成」，以 `force=true` 重做本镜全部段。顶部批量生成仍保留，用于整集操作，但不能替代逐镜入口。生成中按钮只锁定当前镜并显示进行中状态，完成后失效查询刷新该章的音频段状态。
