...
资产状态编辑器只负责查看和生成这张 2:1 状态图，不承载可拖拽的全景空间预览；需要相机、投射中心或角色摆位时，必须从状态图旁的「3D编辑」进入独立的场景 3D 编辑器。这个入口必须处于图片放大控件之上并保持可点击，不能被 `LightboxImage` 的交互层遮住。这样状态信息编辑与空间构图编辑保持单一职责，生成的等距柱状源图仍完整保留给 3D 环境加载链路。
