...
- 右侧固定拆成上下两区：上方是场景对象列表，约占右侧面板三分之一；下方是当前对象的属性面板并使用剩余空间。长对象列表只在对象区内部滚动，属性内容只在属性面板内部滚动，不能让整个编辑器页面被单一长侧栏撑高。
- 对象列表是选择入口，不承担状态摘要或删除按钮：每行只显示对象类型图标和对象名称；固定高度的「属性面板」承载当前对象的全部属性与操作，内容超出时只在面板内部滚动。可移动角色至少展示位置、旋转和大小，移除等对象操作也放在属性面板中，避免对象树出现嵌套操作或信息密度失控。
- 对象树必须包含一个前端语义上的“世界”根节点，并把角色、空间标记、参考角色等对象统一纳入选择状态；根节点用于查看场景级属性，不新增数据库实体。列表选择、视口拾取和下方操作面板必须同步当前对象。HDRI 世界对象显示为「世界」，固定约 1.7 米的尺度校准人物显示为「参考角色」，当前状态识别出的空间标记直接逐项显示在对象列表中。
- 场景级环境参数继续保存到场景资产，角色摆位、姿势、相机和 PNG 继续遵循退出时一次性保存；工作台布局重构不能改变原有保存边界。

### 视口交互

- 分镜摆位页左键拖动角色调整地面位置；场景资产页的比例参照角色固定在世界原点，左键只负责选中；两种页面都用右键拖动旋转相机、中键拖动平移相机和滚轮调整距离。选中角色使用半透明的绿色空心圆环，中心不填充且不写入深度，避免遮住 HDRI 地面。
- 选中角色或比例参照时，视口使用 PlayCanvas `OutlineRenderer` 根据模型真实可见轮廓生成绿色外描边；轮廓采样放在独立临时渲染层，避免主相机重复绘制角色。角色原有的地面空心圆环和空间标记线框继续保留，外描边只属于运行时选择反馈，不写入 `layout3d` 或导出的 PNG；捕获 PNG 前必须暂时移除并在捕获后恢复。
...
- 「识别空间」使用注册的多模态结构化 Prompt `drama.scene.state.3d_markers@v4`，输入当前状态的真实图片制品和场景环境参数，输出床、桌、椅、门窗等固定物体的类别、近似米制位置、长方体尺寸、朝向、置信度和必填图像证据区域。人物、动物和临时物品不进入标记集合；室外/自然场景可以返回空集合。Prompt 固定要求 `v=0.5`、中心安全带 `v=0.48–0.52`，中心线附近被切开的家具不进入标记集合。
- 世界坐标合同固定为地面 `y=0`、`+Z` 指向全景水平中心、`+X` 指向右侧；`position` 是长方体中心，地面锚点由服务端归一化到 `size.y / 2`。`imageRegion` 是实际图片证据，服务端以其水平经度确定 X/Z 方向，以其垂直射线结合投射中心高度、半球直径和固定 `v=0.5` 反算当前 3D 空间；模型 `position` 不得把图片像素或固定坐标直接当成世界坐标。墙面标记的 yaw 同步朝向径向方向，地面物体保留自身朝向。
- 单张全景图不能可靠测量绝对深度，因此带有效图片证据的 wall/ceiling 标记统一使用当前半球可用的外侧墙体参考半径，不能继续采信模型返回的近中心径向提示；floor 标记优先使用框底向下射线与地面的交点，并在固定 50% 合同使家具框底位于地平线之上、射线无法落地时，改用当前半球可用的地面外圈半径，避免桌椅集中在球体中心。标记尺寸由图片框的角度跨度结合结构化 `kind` 的米制范围校准，模型尺寸只作为近似输入。空间标记仍是构图参照，不能当作精确测绘或碰撞几何。没有 `imageRegion` 的旧 AI 数据继续保留原位置，`source: "manual"` 的标记优先级最高，归一化必须幂等。
...
- 家具标记都挤在半球中心时，先检查其 `imageRegion` 底部是否位于固定地平线 `v=0.5` 以上；这种情况下不能再用模型的径向深度提示，floor 投影必须落到当前地面外圈，且仍保留图片区域给出的水平经度。
...
`NovelScene.scene3dEnvironmentJson` 是场景资产的唯一 3D 环境参数源，只保存投射中心高度和半球直径；未自定义时按场景默认状态（第一个状态）的类型取默认值：`interior`（室内）为高度 `1`、直径 `8`，`exterior`（室外）为高度 `1.7`、直径 `10`，`nature`（自然）为高度 `1`、直径 `20`。状态类型是权威来源，场景表的 `sceneType` 仅作为旧数据兼容回退，类型仍缺失时按室外 `1.7 / 10` 处理。同一场景的环境参数是唯一的一份；切换到其他状态或针对其他状态做空间标记分析时，仍复用默认状态类型解析出的环境，避免同一场景出现不同半球直径。用户明确保存过的自定义参数不因类型默认变化而重置。历史未标记默认快照会按当前类型迁移到这组新值。全景图的垂直投影分界固定为 `v=0.5`，不属于环境 JSON，也没有 `0.4–0.65` 的可调范围。`yawDeg` 与 `intensity` 只保留在共享运行时合同中，始终归一化为 `0` 与 `1`，不提供编辑控件。数据字段仍使用 `domeRadius` 以兼容旧快照，但由于基础半球网格半径为 `0.5`，其用户可见语义是半球直径；半球直径的当前可调范围统一为 `5–30`。投射中心严格位于世界 X/Z 原点，只有世界 Y 高度可调。旧版本曾允许保存的高度 `0.6–1` 和直径 `50–100` 会在读取时裁剪到新范围，不会让整张 3D 摆位失效；其他超出当前范围的历史直径也统一裁剪到 `5–30`。旧环境 JSON 中的 `panoramaHorizonV` 读取时被忽略，重新保存时不会再写回。
