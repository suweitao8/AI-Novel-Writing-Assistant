...
- 自动构图 Prompt `drama.shot.blocking.autoPlan@v6` 不再引用 floor 标记：角色站位只受"以投射中心为圆心的可用站位半径、靠边约 1 米运动缓冲"约束；固定物体标记仍是必须避让的障碍和叙事参照。多角色结果还必须输出有向角色关系，关系由服务端落实为接地/上方位置与相对体量。
