...
环境纹理使用线性采样、关闭 mipmap 和各向异性过滤；经度方向使用循环寻址，纬度方向使用边缘夹取。上半球必须保留原始等距 UV 并使用独立的自发光贴图材质显示完整上半幅，地面必须单独使用按世界坐标片元投影的材质采样下半幅，不能把地面投射材质复用到上半球，否则上半球会退化为单一地平线采样，看起来像半球消失。两种显示材质都不应依赖场景直射光来保持环境细节。状态图 URL 返回 404 时属于资产文件缺失，不是环境参数保存失败；编辑器应保留明确的加载失败提示，修复方式是重新生成当前状态图。
