...
- **Projection Rule：** 生成图契约把 2:1 全景图下半部视为俯视地面材质样片，而不是一张要再次从投射中心重投影的透视地面。平坦地面在世界 `X/Z` 平面按样片坐标展开，中心到平坦边界保持有限、连续的采样；上半球天空/远景和外圈弧面继续按投射方向采样，并在平坦边界渐变退出。禁止把已经合成透视线的地面重新按“投射中心 → 地面片元方向”采样，否则会把地面线条再次汇聚成半圆漩涡。
