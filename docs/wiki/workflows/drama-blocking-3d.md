...
- 右侧固定拆成上下两区：上方是场景对象列表，下方是当前对象的属性面板。长对象列表只在对象区内部滚动，属性内容只在属性面板内部滚动，不能让整个编辑器页面被单一长侧栏撑高。
...
- 对象树必须包含一个前端语义上的“场景对象”根节点，并把角色、空间标记、比例参照等对象统一纳入选择状态；根节点用于查看场景级属性，不新增数据库实体。列表选择、视口拾取和下方操作面板必须同步当前对象。
