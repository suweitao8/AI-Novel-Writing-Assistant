...
`layout3d.environment` 是可选的向后兼容字段，保存投影高度、半球尺寸、地面贴图密度、水平旋转和环境亮度。投影高度通过半球竖直比例调整地平线构图，地面仍固定在世界 Y=0；`groundTextureScale` 控制普通场景图下半球的 UV 重复次数，范围为 1–20，默认值为 10，用于直接缩小地面纹理细节而不改变半球几何尺寸。旧布局没有该字段时使用 viewer 默认值。右侧 HDRI 环境面板是这些参数的唯一编辑入口，保存草图时与角色、相机快照一起写回。
