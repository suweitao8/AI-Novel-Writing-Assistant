...
- 场景状态图作为半球 HDRI 环境贴图，角色代理放在带网格的地面上；环境跟随相机移动，避免把场景图铺成后置平面。相机、角色位置、朝向、缩放和静态姿势一起保存为 `layout3d` 快照。
...
场景状态图加载为 `DomeGeometry` 半球，并使用内侧剔除显示球面内部。环境实体跟随相机位置，角色和地面始终处于同一场景中；没有状态图时保留纯色环境，不创建平面背景。
