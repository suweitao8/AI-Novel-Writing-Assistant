...
`layout3d.environment` 是可选的向后兼容字段，保存投射中心高度、半球直径、水平旋转和环境亮度。投射中心高度范围为 `1–10`、默认 `3`，半球直径范围为 `10–50`、默认 `20`；服务端、viewer 和页面控件使用同一组边界。数据字段仍使用 `domeRadius` 以兼容旧快照，但由于基础半球网格半径为 `0.5`，其用户可见语义是半球直径。投射中心严格位于世界 X/Z 原点，只有世界 Y 高度可调；水平旋转和环境亮度不属于用户可调状态，运行时始终归一化为 `0` 和 `1`。普通场景图的下半球使用连续浅碗形曲面，边缘高度随投射中心和实际半球尺寸计算并保持在投射中心下方；投影 UV 按实际半球世界坐标从投射中心计算，不通过中心圆形硬阈值、UV repeat 或把一圈点截断到同一高度制造密度变化。旧快照中的 `groundTextureScale` 作为未知字段被忽略，新的导出数据不会再写回。右侧 HDRI 环境面板只提供高度和直径，保存草图时与角色、相机快照一起写回。
