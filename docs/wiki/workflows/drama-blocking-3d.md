...
场景状态图加载到内侧剔除的 EnviroDome 式环境网格中。所有状态图都拆成上半球和带投射中心的下半球：源图上半幅（`v=0–0.5`）用于天空/远景，下半幅（`v=0.5–1`）用于弧形地面。2:1 等距柱状图也必须走这条拆分路径，不能因为画幅接近 2:1 而改用不受投射中心高度影响的完整穹顶；否则直径或高度修改只会保存成功、视口却没有视觉变化。地面仍然是带贴图的弧面而不是后置平面。半球生成器必须把 `sin(0)`/`sin(π)` 的浮点残差归零，并给极点使用稳定的单一 U，避免同一几何点携带一组不同 UV 而形成退化三角形拉伸。加载成功后隐藏仅用于无环境时兜底的纯色地面平面，定位网格仍作为辅助线绘制在地面上。环境实体固定在世界坐标，Y 轴固定在世界地面；相机旋转或移动只改变视点，不搬动环境地面。没有状态图或环境加载失败时恢复纯色地面。
