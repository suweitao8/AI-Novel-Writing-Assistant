...
- 「识别空间」使用注册的多模态结构化 Prompt `drama.scene.state.3d_markers@v6`，输入当前状态的真实图片制品和场景环境参数，输出床、桌、椅、门窗等固定物体的类别、近似米制位置、长方体尺寸、朝向、置信度和必填图像证据区域。人物、动物和临时物品不进入标记集合；室外/自然场景可以返回空集合。生成图默认要求 `v=0.5` 中心安全带 `v=0.48–0.52` 保持空净，中心线附近被切开的家具不进入标记集合；3D 投射时使用当前场景保存的 `panoramaHorizonV`。Prompt 要求框紧贴物体可见边缘：门框底部贴住落地线、窗框紧贴窗框边缘、家具框包含腿脚或落地部分，框的贴合度直接决定深度反算精度。
- 世界坐标合同固定为地面 `y=0`、`+Z` 指向全景水平中心、`+X` 指向右侧；`position` 是长方体中心，地面锚点由服务端归一化到 `size.y / 2`。`imageRegion` 是实际图片证据，服务端以其水平经度确定 X/Z 方向，以其垂直射线结合投射中心高度、半球直径和当前 `panoramaHorizonV` 反算当前 3D 空间；模型 `position` 不得把图片像素或固定坐标直接当成世界坐标。墙面标记的 yaw 同步朝向径向方向，地面物体保留自身朝向。
- 深度反算走集合级投影 `projectStoryScene3dMarkerSetFromImageRegions`（`shared/utils/scene3dProjection.ts`），因为门窗的深度证据要在同一方位的标记之间共享。深度估计只用三个图像几何量取中位数组合：框底落地线交地面（仅 floor 锚点与门，窗台不是落地线）、框顶边位于类别典型总高度（同样仅落地物体，窗顶浮在窗台上方不可用）、框垂直跨度等于类别典型高度（对门窗家具通用）。高度先验取 `STORY_SCENE_3D_MARKER_SIZE_POLICIES` 的类别中值而不是模型 size，保证估计只依赖 imageRegion 与环境参数，重复归一化幂等。
- 墙面深度合同：门以落地线为最强证据，窗以跨度反算；同一方位（默认 45° 容差）聚类后整簇取加权中位数（门落地证据权重加倍）统一墙距，因此同面墙的门窗深度一致。门的 `position.y` 固定为 `size.y/2` 整体落地，窗保持中心射线高度。家具深度除三路估计取中位数外，还以同方位墙距（默认 60° 容度内最近墙簇）为上限，不能越过墙面。所有估计都无证据时才回退 `STORY_SCENE_3D_MARKER_FALLBACK_WALL_RADIUS_RATIO × domeRadius`（0.45）参考半径。标记尺寸由图片框的角度跨度结合结构化 `kind` 的米制范围校准，模型尺寸只作为近似输入。空间标记仍是构图参照，不能当作精确测绘或碰撞几何。没有 `imageRegion` 的旧 AI 数据继续保留原位置，`source: "manual"` 的标记优先级最高，归一化必须幂等。
- 可行走地面薄板（`kind: "floor"`，id 固定 `scene-floor-walkable`）不是视觉模型的输出：Prompt v6 的类别白名单排除了 floor，schema 拒绝该类别，它是 `normalizeStoryScene3dMarkerSet` 在集合级投影之后从墙面标记深度推导的确定性后处理。全景投射时地平线以下的墙面像素会落在地面几何上，角色若站在这些区域会显得踩在墙上；薄板取所有已投影 wall 标记的最近半径生成内接方形（0.95 边距、厚度 0.06m、y=size.y/2），没有墙面证据时回退空间标记参考半径，空标记集合不生成。每次归一化先丢弃旧薄板再重新推导，幂等且随门窗识别结果自动更新；`adoptLegacyStoryScene3dMarkerEnvironment` 忽略 floor 标记的 imageRegion 缺失。
- 3D 视图端把 floor 薄板渲染为参考层：透明度远低于家具标记（未选 0.09/选中 0.22），不参与射线拾取（否则会挡住其覆盖范围内的所有家具标记），仅选中时绘制描边。自动构图 Prompt `drama.shot.blocking.autoPlan@v2` 明确 floor 是站立范围而不是障碍物：角色站位必须落在薄板范围内，不能站到边界之外或墙面上。
