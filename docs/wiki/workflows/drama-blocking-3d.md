...
- 场景状态图作为半球 HDRI 环境贴图，角色代理和模型库前景都放在半球底部的真实水平地面上；环境固定在世界坐标，避免把场景图铺成后置平面或在相机旋转时搬动地面。相机、角色位置、朝向、缩放和静态姿势一起保存为 `layout3d` 快照；投射中心高度、半球直径和全景地面分界属于场景资产，不属于镜头快照。
...
- **Projection Rule：** 生成提示仍要求 2:1 全景图下半部尽量输出连续、干净的俯视地面材质，但渲染器不能假设模型一定遵守这个契约：普通生成图的下半部常常仍是透视地面。天空和外圈圆墙按投射方向采样，地面使用覆盖完整半径的水平平面与稳定材质色；禁止把已经合成透视线的地面重新按“投射中心 → 地面片元方向”采样，也禁止把下半图直接展开到世界 `X/Z` 平面，否则会把地面线条汇聚成同心漩涡。桌椅、角色等需要碰撞、摆位或交互的对象必须作为独立模型库前景加入，不得烘焙进 HDRI 地面。
...
- `drama.shot.blocking.autoPlan` prompt（当前 v8）承载导演工艺基线：景别→distance/focalPoint 高度基准（特写 1.5–2 至远景 ≥10）、三分法与 headroom/lead room、双人对话 180° 轴线相向站位并把 DoF 焦点锁在说话者、elev 正负对应仰拍/俯拍的叙事语义，以及先识别有向角色关系再规划坐标。结构化 `relations` 明确 `on_top_of` 的上下方向和 `sizeRelation` 的体量方向；上方 subject 只能使用 `crouching` 或 `kneeling`，不使用当前 UAL 无法正确呈现的 `prone` / `lying`；这些语义由 Prompt 判断，服务端只落实已经结构化的关系并对不兼容姿势做确定性归一化，`compositionNote` 继续让模型自述构图依据。
...
场景状态图加载到内侧剔除的 EnviroDome 式环境网格中。所有状态图都使用一份连续的上下表面：生成源图默认以 `v=0.5` 作为安全地平线，3D 投影地平线由场景的 `panoramaHorizonV` 参数决定；其上方用于天空和远景，下方是完整的水平地面，最外圈用垂直圆墙接回地平线。上下表面必须共享唯一的地平线顶点圈，并由同一个 `MeshInstance` 和材质一次绘制，不能用两个独立网格在同一位置叠边，否则光栅化会留下细缝。交界圈的世界高度必须与投射中心高度一致，使天空方向落在当前全景地面分界；2:1 等距柱状图也必须走这条带投射中心的 EnviroDome 路径，不能因为画幅接近 2:1 而改用不受投射中心高度影响的完整穹顶；否则直径或高度修改只会保存成功、视口却没有视觉变化。地面几何必须是真平面，不能用向中心收缩或向上卷起的碗状曲面。半球生成器必须把 `sin(0)`/`sin(π)` 的浮点残差归零；极点的空间坐标要精确收敛，投影材质在水平分量接近零时必须直接使用固定经度，不能先执行未定义的 `atan(0,0)`。加载成功后隐藏仅用于无环境时兜底的纯色地面平面，定位网格仍作为辅助线绘制在地面上。环境实体固定在世界坐标，Y 轴固定在世界地面；相机旋转或移动只改变视点，不搬动环境地面。没有状态图或环境加载失败时恢复纯色地面。

地面全景贴图不能把 `atan2` 得出的经度直接写入地面顶点 UV。中心平底的三角扇会对角度 UV 做线性插值，产生环状漩涡；更重要的是，普通生成的 2:1 场景图下半部通常是透视画面，不是可标定的俯视地面样片，把它重新按世界平面展开或按投射中心重投影都会制造同心弯曲。当前材质只对天空和外圈圆墙按投射中心方向采样原始 2:1 源图；地面片元不再重投影下半图，而是从下部若干安全采样点计算一个低频稳定材质色。这样既避免漩涡，也不会把桌椅、角色等前景错误地烘焙进地面；`.hdr` 的 RGBE 源仍按 RGBE 解码，普通图片源沿用 gamma 解码，源图同时继续作为环境光照 atlas 的输入。地面几何只保留常量占位 UV 以满足 PlayCanvas 顶点流要求，不得把它重新当作全景投影 UV。CPU 投影数学仍保留用于天空方向和回归测试。地面几何只负责完整平面与外圈垂直圆墙的连续拓扑，不能恢复会形成碗状过渡的曲面或地面顶点 UV 投影。

投射中心的南极奇点只由天空/圆墙的方向投影材质处理：水平分量接近零时使用固定经度。地面直接走稳定材质色，不建立以投射中心为圆心的地面投影环，也不能通过重新引入立方体重投影来替代平面地面。
...
源环境纹理使用线性采样、关闭 mipmap 和各向异性过滤，经度方向循环寻址、纬度方向边缘夹取；可见 EnviroDome 与环境光照 atlas 共用原始环境纹理，天空/圆墙投影材质在片元阶段按世界方向直接采样，`.hdr` 与普通图片分别走 RGBE/gamma 解码。连续环境网格共用一份材质：上半部和外圈圆墙显示天空/远景，下方完整平面使用从源图下部安全区域提取的稳定材质色。自定义着色器必须沿用 PlayCanvas 标准材质的 sRGB 解码、tone mapping 和 gamma 输出，否则环境图会绕过标准颜色空间流程而明显变亮。天空、圆墙和地面必须属于同一份连续几何和同一个 `MeshInstance`，不能用两套显示材质或重复边界制造亮度、颜色或光栅断层；角色阴影由独立的 StandardMaterial shadow catcher 叠加。状态图 URL 返回 404 时属于资产文件缺失，不是环境参数保存失败；编辑器应保留明确的加载失败提示，修复方式是重新生成当前状态图。
...
普通场景图的下半部由一块覆盖完整半径的水平圆形平面和外圈垂直圆墙组成；圆墙从投射中心高度线性落到地面高度，不能向中心收缩或把平面弯成碗状曲面。平面中心只保留一个几何顶点，不能继续用下半球极点复制一整圈经度，否则地面三角扇会把纹理压成尖刺。地面顶点只使用常量占位 UV，投影材质对地面不计算 `u/v`，只从源图下部安全区域生成稳定材质色；天空和圆墙才按投射中心方向采样。旧快照中的 `groundTextureScale` 作为未知字段被忽略，新的导出数据不会再写回。
...
HDRI 下方地面上的角色阴影不能直接由可见投影材质承担：可见环境是按世界坐标采样天空/圆墙的自定义 shader，不包含 PlayCanvas 标准材质的阴影片元块。正确边界是保留可见 HDRI 网格，再用同一套完整平面与外圈圆墙几何创建独立的 `StandardMaterial.shadowCatcher` 网格：材质使用 `BLEND_MULTIPLICATIVE`、关闭 skybox 与深度写入，只把方向光阴影乘到下半部，不给天空加黑影；由于接收层与可见层共用拓扑，使用双面剔除。HDRI 派生方向光、代理角色和场景阴影开关必须同时启用，接收层设为 `castShadow=false`、`receiveShadow=true`，并用透明通道的 `drawBucket=250` 让它在可见背景之后叠加。投射中心高度或圆半径变化时必须同时重建可见网格和 shadow catcher，切换环境、加载失败和 viewer 销毁时同时释放 catcher 的实体、网格和材质。该能力只属于 PlayCanvas 分镜/场景 3D 预览；Remotion 最终视频仍使用自己的 2D 合成链路。
