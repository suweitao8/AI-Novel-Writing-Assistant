...
- 选中角色或比例参照时，视口使用 PlayCanvas `OutlineRenderer` 根据模型真实可见轮廓生成绿色外描边；轮廓采样放在独立临时渲染层，避免主相机重复绘制角色。角色原有的空间标记线框继续保留，外描边只属于运行时选择反馈，不写入 `layout3d` 或导出的 PNG；捕获 PNG 前必须暂时移除并在捕获后恢复。
...
`NovelScene.scene3dEnvironmentJson` 是场景资产的唯一 3D 环境参数源，保存投射中心高度、半球直径和 `panoramaHorizonV`；未自定义时按场景默认状态（第一个状态）的类型取默认值：`interior`（室内）为高度 `1`、直径 `8`，`exterior`（室外）为高度 `1.7`、直径 `10`，`nature`（自然）为高度 `1`、直径 `20`，分界默认 `0.5`。状态类型是权威来源，场景表的 `sceneType` 仅作为旧数据兼容回退，类型仍缺失时按室外 `1.7 / 10` 处理。同一场景的环境参数是唯一的一份；切换到其他状态或针对其他状态做空间标记分析时，仍复用默认状态类型解析出的环境，避免同一场景出现不同半球直径或分界。用户明确保存过的自定义参数不因类型默认变化而重置。历史未标记默认快照会按当前类型迁移到这组新值。`panoramaHorizonV` 的可调范围为 `0.40–0.65`，旧环境 JSON 缺少该字段时读取为 `0.5`；`yawDeg` 与 `intensity` 只保留在共享运行时合同中，始终归一化为 `0` 与 `1`，不提供编辑控件。数据字段仍使用 `domeRadius` 以兼容旧快照，但由于基础半球网格半径为 `0.5`，其用户可见语义是半球直径；半球直径的当前可调范围统一为 `5–30`。投射中心严格位于世界 X/Z 原点，只有世界 Y 高度可调。旧版本曾允许保存的高度 `0.6–1` 和直径 `50–100` 会在读取时裁剪到新范围，不会让整张 3D 摆位失效；其他超出当前范围的历史直径也统一裁剪到 `5–30`。
