...
普通场景图的下半球在投射中心附近使用有限平底，半径 `0.95` 以内保持可用地面，最外侧窄环连续过渡到随投射中心和实际半球尺寸计算的地平线高度；这是网格形状约束，不是纹理 UV 的硬裁切。平底中心只保留一个几何顶点，不能继续用下半球极点复制一整圈经度，否则地面三角扇会把纹理压成尖刺。投影 UV 按实际半球世界坐标从投射中心计算，并把地面边缘的投影角归一到上半球的 `v=0.5` 接缝；首尾经度的重复顶点必须分别使用 `U=1` 与 `U=0`，不能从相同的 X/Z 坐标重新计算出同一个 U，否则闭合四边形会跨越整张纹理并出现竖向拉伸条。不使用 UV repeat 或把纹理缩放成重复图案。旧快照中的 `groundTextureScale` 作为未知字段被忽略，新的导出数据不会再写回。
