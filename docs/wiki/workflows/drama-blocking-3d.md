...
普通场景图的下半球在投射中心附近使用有限平底，半径 `0.95` 以内保持可用地面，最外侧窄环连续过渡到随投射中心和实际半球尺寸计算的地平线高度；这是网格形状约束，不是纹理 UV 的硬裁切。投影 UV 按实际半球世界坐标从投射中心计算，并把地面边缘的投影角归一到上半球的 `v=0.5` 接缝，不使用 UV repeat 或把纹理缩放成重复图案。旧快照中的 `groundTextureScale` 作为未知字段被忽略，新的导出数据不会再写回。普通场景图加载完成后，viewer 会读取上方区域的高亮像素并估算主光方向与颜色，只用于角色代理的基础方向光，不写入场景参数。
