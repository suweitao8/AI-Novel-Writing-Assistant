...
场景状态图加载为 `DomeGeometry` 半球，并使用内侧剔除显示球面内部。`DomeGeometry` 的下半部承担 HDRI 地面弧面；加载成功后隐藏仅用于无环境时兜底的纯色地面平面，定位网格仍作为辅助线绘制在地面上。环境实体跟随相机位置，角色和地面始终处于同一场景中；没有状态图或环境加载失败时恢复纯色地面。
