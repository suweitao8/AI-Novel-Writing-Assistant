...
- **音色描述生成（design 模式）**：不克隆音频，用一句文字描述（年龄/性别/语气/节奏）作为声音设计提示，让 VoxCPM2 用固定样句合成一段参考音；若已有有效参考音频则同时用于音色克隆。角色继续按角色资产维护；旁白统一由系统设置维护，所有漫剧项目共用一份旁白参考样本。
...
- `AppSetting` 的 `drama.globalNarratorVoice`（JSON）是系统旁白唯一权威来源：`{description, sampleAudioUrl, sampleText, sampleSha256, source, updatedAt}`。合成与 stale 投影都读取它，旁白请求把有效的 `sampleAudioUrl` 作为 VoxCPM2 的参考音频候选；历史 IndexTTS 文件名会被过滤，不会覆盖新样本。
...
- `server/src/services/drama/audio/VoxCPM2TTSProvider.ts`（当前默认 VoxCPM2 配音适配器）
- `client/src/components/audio/IndexTTS25VoiceControls.tsx`（暂存的 IndexTTS 兼容控件，当前页面不挂载）
