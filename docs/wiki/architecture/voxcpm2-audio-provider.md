...
- 根目录 `pnpm dev` / `pnpm dev:log` 会先执行 `pnpm voxcpm2:bridge`；直接运行服务端开发脚本时，`server/package.json` 的 `dev:api` 也会先执行同一个幂等启动器：复用已通过 `/health` 与 `/v1/models` 双重校验的正式桥，未运行时按 `VOXCPM2_ROOT`、`VOXCPM2_BRIDGE_PYTHON` 启动它，并等待模型加载完成。这样无论使用整栈启动还是单独启动 API，服务都不会在音频桥尚未就绪时先对外提供“可生成”页面。
...
- 响度统一也放在这个公共出口：对桥接返回的 PCM16 WAV 只统计高于 -40 dBFS 的有效语音样本，将 RMS 归一到约 -18 dBFS，并把峰值限制在 -1 dBFS 以内；如果少量尖峰阻止安全增益，会先做自适应的轻度峰值压缩再归一化；非 WAV 音频保持原样。VoxCPM2 的 `normalize=true` 只做文字规范化，不能替代这层音频响度处理。
...
- 桥接未启动：18761 端口无响应或 404，设置页“测试连接”/音色生成报「上游模型服务连接失败」；根目录整栈启动和服务端 `dev:api` 都会自动拉起并等待正式桥。手动排查时仍可执行 `pnpm voxcpm2:bridge`。启动器只接受正式 FastAPI 桥的 `/health` + `/v1/models` 响应，不会把旧 Gradio 桥误判为可用。
