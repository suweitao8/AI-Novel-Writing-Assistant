...
- 角色级 AI 身高档案与角色状态编辑器的 `身高（米）` 输入统一限制在 0.50–10.00 米，覆盖儿童、普通人类和血角兽等巨型怪物。当前状态按“人工 `heightMeters` > 角色级 AI/兼容档案 > 1.8 米兼容基准”解析；留空状态身高不会修改角色级档案，而是继续使用 AI 档案。
- 状态编辑器先呈现状态名、年龄段和人工身高，再呈现状态图片与生成操作；身高是状态级基础参数，不应被大图或图片操作区推到内层滚动区域之后。
...
- 分镜保存接口如果只校验位置和缩放、却漏掉 `heightMeters`，5 米级怪物会在保存往返后回退到默认身高；布局请求与归一化必须显式保留并校验 `heightMeters` 的 0.50–10.00 米范围。
