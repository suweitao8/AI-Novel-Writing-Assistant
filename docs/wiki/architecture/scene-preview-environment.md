...
- 离屏缩略图必须在开始异步加载 HDRI/模型/动画前借用 `app.start()` 完成 PlayCanvas 系统初始化，随后取消其持续 RAF，并在抓图前显式调用 `app.update()`；这样投影材质在有效生命周期内完成装配，动作状态仍会推进，销毁缩略图应用时也不会留下访问空 `renderer` 的异步帧。
- 任何会显示可见 HDRI 穹顶的预览都必须把当前相机传给环境运行时。环境实体加入世界后先保持禁用，运行时用该相机、世界层灯光 hash 和 view uniform 预创建精确的 forward shader variant，并轮询 PlayCanvas 的异步链接完成状态；只有完成后才 finalize 并显示穹顶。等待超时或当前请求失效时，必须释放当前请求已经创建的环境资源，不能让旧请求清理新请求。
...
- 离屏缩略图在 `app.start()` 之后才加载环境，或让 `autoRender=false` 的应用持续运行，或让没有投影者的 shadow catcher 参与合成：前者会让可见穹顶在预启动上下文中错过有效装配，后两者会分别在销毁竞态中访问已清空的 renderer、把缩略图地面压黑；应在异步加载前启动并立即取消 RAF，手动推进动作帧，并关闭无投影者缩略图的 shadow catcher。
- 可见穹顶加入世界后立即参与首帧渲染，或直接调用 shader finalize 而不等待 `KHR_parallel_shader_compile` 完成，会把仍在链接中的有效 GLSL 误报为 `Failed to compile ... null`；必须先预热当前相机的 forward variant，确认异步链接完成后再显示穹顶。
