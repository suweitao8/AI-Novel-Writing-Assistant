...
- 全局旁白的 IndexTTS 2.5 配置唯一从「系统设置 → 旁白音色」维护，模型 speaker、参考音频和旁白描述对所有项目生效；分镜页只负责查看镜头和执行配音，不承载通用音色配置。角色音色数据仍保留在角色资产和服务端兼容接口中，不能用全局旁白设置覆盖角色对白。
- 系统旁白页由同一张设置卡片统一承载语音合成字段和唯一音频播放器：参考音频是合成来源，生成试听是最近一次结果；两者在数据层保留不同语义，但页面不重复渲染同一份音频。页面读取 `referenceAudioUrl` 时不得回退到 `sampleAudioUrl`。
