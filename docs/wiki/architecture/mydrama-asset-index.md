...
| 配音分段显示模型 | `frontend/src/components/episode/voice-stage.tsx` | `ShotVoiceListPanel.tsx` + `NarratorVoiceSettingsPage.tsx`（见 `docs/wiki/workflows/comic-drama-voice-overdub.md`） | 分镜页只显示旁白/对白分段三态；全局旁白音色在系统设置维护，设置页统一展示一个试听样本 |
