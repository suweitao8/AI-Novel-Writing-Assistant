...
- 无参考图封面默认使用 `grok_build` 图片通道，模型为 `grok-build-image`，服务地址为 `http://127.0.0.1:18767/v1`。**资产参考图（角色/道具/场景）2026-08-22/23 起不再走 Grok Build**：角色与道具统一透明底、场景全景要 2:1 等距柱状比例，都只有 Codex 通道支持（见 codex-image-provider.md）。
- Grok Build 基础图片统一生成并归一化为 1280×720 PNG。图片提示词只允许一次 `image_gen`，并明确禁止 shell、代码、文件编辑和网页工具，避免订阅 CLI 把图片任务变成普通代理任务。
- 角色状态图与 `DramaCharacterImageService` 的漫剧角色设计稿必须共用 `buildCharacterStateSheetPrompt`：单张 16:9 板固定为“面部正面 → 面部 90°侧面 → 全身正面 → 全身背面”，每个栏位填满整高，面部栏延伸到肩胸，禁止悬浮裁切和大块空白。这样不同页面生成的参考图才能进入同一条身份一致性链路。
- Grok Build 图片桥只接收文字提示词，不接收 PSD/模板文件，也不能执行 `/images/edits`。角色状态图因此不声称“把 PSD 传给了 Grok”：无参考图时由 `StoryAssetStateImageService` 用一次完整的四格参考板提示词生成原生 1280×720 图片，要求四个等宽栏位按“正面头像 → 90°侧面头像 → 正面全身 → 背面全身”排列；代码中的模板只作为板式契约和兼容旧合成器的几何定义，不能替代模型的图片输入能力。
...
- `grok-cli` 默认承担文本，`grok_build` 默认承担无参考图封面；资产参考图（角色透明底、道具透明底、场景 2:1 全景）与一切带参考图的任务由路由选择 Codex 兼容通道。
- `StoryAssetImageService` 的场景全景走 Grok Build；角色四视图（`StoryAssetStateImageService` 状态图与 `DramaCharacterImageService` 漫剧角色设计稿）与道具图（状态图 + 45° 透视图）统一走 Codex 透明底生成。场景提示词必须是空环境：人物、动物、怪物和其他活体只能被转译为爪痕、血迹、破坏植被等环境痕迹。
...
- 角色四视图在统一渲染媒介之外还必须满足漫剧主角的商业审美：提示词明确要求对称五官、清爽健康的面部、利落发型、挺拔的精瘦体型和可亲近的主角吸引力；末世氛围只能作用于表情、服装磨损和材质细节，不能把角色基础脸型推向憔悴、病态、低颜值或不讨喜。
