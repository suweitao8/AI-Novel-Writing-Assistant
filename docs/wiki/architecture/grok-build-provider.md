...
- 场景基础图与无参考图封面默认使用 `grok_build` 图片通道，模型为 `grok-build-image`，服务地址为 `http://127.0.0.1:18767/v1`。**2026-08-22 起角色与道具的资产参考图不再走 Grok Build**：这两类参考图统一透明底，只有 Codex 通道稳定支持（见 codex-image-provider.md）。
...
- `grok-cli` 默认承担文本，`grok_build` 默认承担场景全景与无参考图封面；角色/道具资产参考图（无论有无参考图）与一切带参考图的任务由路由选择 Codex 兼容通道。
