...
- **文本/视觉通道**：桥新增 `POST /v1/chat/completions`，经 `codex exec --json`（read-only 沙箱、隔离 CODEX_HOME、`-i` 图片附件）转发文本与图片理解请求；默认 `gpt-5.6-luna` + `model_reasoning_effort=low`（官方描述 fast responses，即 fast 模式；`CODEX_TEXT_MODEL`/`CODEX_TEXT_REASONING_EFFORT` 可覆盖）。账号可用模型为 gpt-5.6-luna / gpt-5.5 / gpt-5.4 / gpt-5.4-mini（均支持图片输入）。JSON 能力声明为 true/true：`response_format` 由桥翻译成输出协议注入 agent prompt（工具调用走 `__codex_tool_call__` 信封协议），Zod 校验兜底。文本槽/视觉槽/图片槽已统一指向 codex。
