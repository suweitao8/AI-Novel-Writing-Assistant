...
- **角色状态是外观与音色的唯一新入口（2026-08-21）**：`Character.statesJson` 里的 `StoryAssetState` 额外包含 `ageGroup`；性别仍属于角色基础身份。年龄段、外貌描述、图片提示词和音色提示词只在状态里编辑。`normalizeStoryCharacterStates` 会把没有状态的旧角色确定性归并成 `initial/初始状态`，把旧 `ageGroup/appearance/physique/attireStyle/facePrompt/voiceTexture` 作为一次性初始状态来源，并保留旧列作为兼容回退，不做删除式迁移。已有状态的人工提示词、图片、音频和显式 `referenceStateId: null` 不被覆盖。新角色默认带一个不可删除的初始状态；新增状态默认继承上一状态的年龄段，生图参考默认不指定（2026-08-27 起参考图是显式选择，未选即全新生成），状态描述可自动生成生图提示词，音色留空时沿状态链向前继承。服务端更新角色时必须保留首个状态，不能通过直接调用 API 删除或移动初始状态。
...
### 故事资产卡片的预览裁切契约（2026-08-23）

资产卡片的图片是展示投影，不是新的持久化媒体。`storyAssetPresentation.ts` 先从状态列表中查找标签为「默认」的状态；历史数据没有该标签时回落到数组首状态，再把带 `generatedAt` 版本参数的状态图 URL 和裁切模式交给 `StoryAssetPreview`。因此卡片、脚本右侧和设定页签不应各自读取 `states[0].imageUrl` 或保存派生缩略图。

- 角色使用 `character-left-square`：当前角色源图是 1536×864 的严格 16:9 横向四栏板，先取最左 1/4（x=0..384），再取 y=240..624 的 384×384 面部窗口，保持等比例显示，不把整张图垂直居中或拉伸成四列两行。
- 场景和道具使用 `center-square`：以默认状态图的水平中心为主体，裁切为方形；原图仍由编辑弹窗负责查看和编辑。
- 没有图片或图片加载失败时，统一保留方形占位，不能因为缺图改变卡片左右布局。
- 预览组件只负责裁切、加载失败状态和可访问名称；编辑、生成、删除、缓存更新继续由各自业务入口管理。
- **资产卡片信息层级（2026-08-25）**：角色、场景、道具设定页的资产货架是用于快速比较图片的展示面，`xl` 及以上使用五列响应式栅格，竖向主卡片只呈现方形默认状态图、资产名称和状态信息；摘要、属性徽标和详细操作进入编辑弹窗。大纲右侧空间受限，继续通过 `StoryAssetCard compact` 使用横向摘要卡片，不能把两种展示场景强行合并成同一套密度。
- **状态详情预览比例（2026-08-25）**：资产编辑弹窗中的已生成状态图使用 `LightboxImage` 的自然比例模式，容器高度跟随原图，避免角色/场景/道具历史图片与固定 `aspect-video` 或 `2:1` 容器不一致时在左右产生留白；没有图片时仍使用该资产的目标画幅占位。场景的「3D编辑」与生成/重新生成操作属于同一状态图片操作组，保持同排，3D 预览仍通过独立入口打开。
- 状态图的 `stateId` 只在所属资产内唯一，文件与 URL 必须同时包含小说、资产类型、资产 ID 和状态 ID；这样不同角色都使用 `initial` 时不会互相覆盖。旧的无归属 URL 只保留兼容入口，资产列表投影与分镜参考图都必须改写为资产归属明确的新 URL。
...
- **画风解析链**：时代风格完整优先级 = **状态自选 eraStyle（pinnedStyle，仅状态图；未选兜底内置「现代都市」预设）** > **剧情判定（scriptJudge，仅分镜首帧）** > 全局链（本条）。全局链 = `DramaProject.visualStyle`（手动选择/创建时写入）> 小说 `defaultArtStyle` > 内置默认（预设列表首位）。**章节脚本【画风：名】标记层已于 2026-08-23 移除**（用户决定：时代风格由资产状态自带——角色/场景/道具的状态各自选风格，脚本不定义章节画风；「脚本」页签的画风下拉、【画风】标记行编辑、resolver 的脚本标记解析与 GET /drama/era-style 的 source=script 一并移除，历史【画风：…】行在脚本文档里按未知标记保留为文本、不参与解析）。GET /api/drama/era-style/:novelId 返回当前生效风格与来源（novel-default/builtin），提取应用预填新状态时代风格仍用它。小说默认→项目的同步推送入口随项目内画风面板移除（2026-08-22）；项目自己的画风用 POST /api/drama/projects/:id/visual-style 设置，校验放行内置预设 id 与全局自定义风格名。**生成侧（首帧图/立绘）**统一走 `resolveDramaArtStyleContext({ visualStyle, sourceRef })`：visualStyle/defaultArtStyle 都没有则只用资产层。
...
- **提取建议的应用态与道具门槛（2026-08-23 用户要求）**：「提取」页签的应用成功后**建议保留在列表**并亮「已存在」徽标（旧版把已应用条目从 `Chapter.referenceExtractionJson` 里删掉——用户应用后回看不到提取结果、不知道哪些建过；徽标靠 `existingNames`（资产名单缓存）驱动，应用成功 `invalidateStorySettingsCaches` 刷新后自动亮，再点开走已有资产更新路径，同名兜底拦重复创建；重新「解析」仍整份重写建议）。**世界观条目同样亮「已存在」（同日晚些补）**：世界观没有独立资产，existingNames.worldview 按 `NovelSettingsWorld.keySettings` 条目标题比对；同名条目重复应用改为**更新该条内容**（旧版直接抛「已有同名世界观条目」拦人——用户应用后看不到标记会再点一次，报错体验差）。**道具提取按复用度过滤（reference_parse@v14）**：只收反复出现或直接推动关键情节的贯穿性道具；行李箱/背包/糖果零食/银行卡/手机/普通衣物餐具这类只在个别分镜用一下的通用日常物品一律不收（用户实测第一章提出一堆低复用道具——换个故事也一样用，单独建形象没有价值，本章没有就返回空数组）。契约锁定在 server/tests/chapterReferenceParse.test.js 与 server/tests/dramaExtractApplyContract.test.js。
- **提取应用创建的新状态预填本书时代风格（2026-08-23 用户实测血角兽问题）**：提取应用创建角色/场景/道具时，新状态的 `eraStyle` 预填**本书当前生效时代风格**（`getDramaEraStyle` 同一条链：脚本【画风】标记 > 小说默认 > 内置默认）——此前创建路径不写 eraStyle，生成时兜底内置「现代都市」，末世书新建的怪兽落成现代都市画风。落点是 `useReferenceExtractStage`（查询 + 创建路径 `withDefaultEraStyle` 兜底）与 `ExtractApplyDialog`（新建议初始状态预填，弹窗里可见可改）；`assetForms` 的三个 `createInitial*State` 工厂已补 `eraStyle` 透传（旧工厂是固定字段对象，多传字段会被静默丢掉）。**只影响创建路径**：更新已有资产（existingId 分支）不碰状态原值。注意范围：设定中心/资产页签手动「添加状态」仍不带默认风格（空=生成时兜底现代都市，2026-08-22 决定），如需同规则要另改。契约锁定在 server/tests/dramaExtractApplyContract.test.js。
