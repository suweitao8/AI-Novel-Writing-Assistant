...
**状态图归属隔离（2026-08-23）**：`stateId` 只在所属资产内唯一，状态图文件与 URL 必须同时包含小说、资产类型、资产 ID 和状态 ID；新生成写入 `generated-images/story-state-images/<novelId>/<kind>/<assetId>/<stateId>/image.<ext>`，列表投影和分镜参考图也统一改写为资产归属明确的 URL。旧的无归属 URL 只作为兼容入口，不能作为新的资产 DTO 或参考图来源，避免多个资产都叫 `initial` 时互相覆盖。
