...
- **状态编辑器的表达层级**：左侧状态列表只用于选择，展示状态缩略图和状态名，编辑/删除作为必要操作保留；图片或音色是否存在不再用“已生成/待生成”标签重复说明。右侧固定按「图片 → 图片设定 → 音色」排列，图片为空时保留空白生成区和生成按钮；角色音色提示词、处理方式、试听与生成操作集中在底部，场景和道具复用同一套图片区与图片设定区。
