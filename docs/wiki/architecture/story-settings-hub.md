...
- **状态资产引用与音色契约**：`referenceStateId` 未提供时由 `normalizeStoryAssetStates` 按数组顺序补为紧邻上一状态；首状态补 `null`，显式 `null` 永远代表用户主动取消参考。状态图生成只读取这个有效状态的 `image.status=done` 图片，避免把“有配置但未生成”的状态误当成参考图。角色状态额外保存 `voice: { status, mode, sourceStateId?, sampleAudioUrl?, prompt? }`：上一状态有已完成试听时默认 `reuse_previous`，复用只复制上一状态的试听，不调用模型；选择 `generate_new` 才通过 `synthesizeAudioSpeech` 使用当前状态音色提示词/角色基础音色合成固定试听短句。状态音色路由为 `POST /novels/:id/settings/characters/:characterId/states/:stateId/generate-voice`，读改写时只替换目标状态并保留其他状态的图片与音频。
