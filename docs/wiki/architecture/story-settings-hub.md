...
- **状态迁移的安全边界**：`parseStoryAssetStatesJson` 同时返回归一后的可读状态和 `canSafelyRewrite` 标记。只有 JSON 可解析、顶层为数组且每个条目都有稳定 `id/label` 时，列表读取才允许把旧角色补成初始状态并回写；损坏 JSON 或含非法条目的原始字符串保持不动，图片/音色生成也会停止并提示先在设定中心显式保存，避免读时迁移覆盖手工数据。
