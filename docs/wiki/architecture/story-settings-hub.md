...
- **下游消费边界**：漫剧分镜的 `characterStates` 是镜头级状态选择，`DramaDialogueAudioService` 和 `DramaAudioSegmentsService` 必须用它覆盖角色基础音色；完成状态试听作为 `TTSGenerationRequest.referenceAudioUrl` 传给 `indextts25`，适配层会把音频缓存到 IndexTTS `voices/` 并作为 `audio` 文件名传给 `/tts`，同时纳入 `voiceKey`，这样状态变化会把旧对白标为过期。配音默认 provider 为 `indextts25`，`mock` 只作为显式联调选项。首帧图参考默认开启，前端必须显式发送 `false` 才表示用户取消参考。
