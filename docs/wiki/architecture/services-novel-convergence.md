...
- ~~`client/src/pages/novels` 根层（19 文件）与 `novels/hooks`（18 项）~~ 已收敛（2026-08-20）：4 个 novelEdit* 页面支撑文件并入既有 `edit/` 模块，6 个章节/卷规划与结构化大纲工具进入 `planning/`（根层 19→9：6 个路由页 + novelBasicInfo.shared/novelWorkflow.client/novelWorkspaceNavigation），6 个 `useNovelVolumePlanning*` 同前缀文件进入 `hooks/volumePlanning/`（hooks 根层 .ts 降至 12 以内）。门禁：`pnpm --filter @ai-novel/client typecheck` 零错误。教训：批量改写脚本的「移动文件审计」只覆盖被移动文件，引用方行的改写错误要靠 typecheck 兜底（本批别名分支漏了收尾引号，由 typecheck 定位到唯一行后修复）。
