...
用户在漫剧工作室上传/生成图片报「上游模型服务连接失败（ECONNREFUSED）」。`netstat -ano | grep 1876` 显示 18761（voxcpm2）/18762（OpenCode Go）/18763（opencode serve）/18764（Grok Build 文本）/18767（Grok Build 图片）全部 LISTENING，唯独 **18766 没有进程**。根因：根目录 `package.json` 的 `dev`/`dev:log` 启动链只串了 `pnpm grok:bridge`（同时拉起 18764+18767），`codex:image`（18766）只有独立脚本、不在启动链里——开发环境重启后 Codex 桥一直是停的。
...
1. `netstat -ano | grep -E "1876[0-9]"` 对照端口表：18761 voxcpm2、18762 OpenCode Go 桥、18763 opencode serve、18764 Grok Build 文本桥、18766 Codex 图片桥、18767 Grok Build 图片桥。
