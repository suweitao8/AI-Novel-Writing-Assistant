...
- 进入模型或动画详情页时，必须先取消并等待仍在初始化/渲染的卡片缩略图工作室；详情页默认预览图直接从当前可见画布抓取，不能让两套 HDRI WebGL 应用并行销毁或编译资源。
...
- 推荐：详情页先等待缩略图队列结束，再创建可见查看器；缩略图工作室在 Promise 尚未 resolve 时也必须持有可立即执行的销毁句柄。
