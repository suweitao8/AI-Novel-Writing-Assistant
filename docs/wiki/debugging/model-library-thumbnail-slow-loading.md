...
- 模型库先完成搜索与分类筛选，再按当前网格的列数、可用高度、卡片高度和行间距计算页大小；页大小始终是列数的整数倍，只挂载完整行，最多展示 8 行，测量不可用时回退到 24 条。分类或搜索结果变化后回到第 1 页，越界页收敛到最后一页。
- 页面、滚动容器、网格尺寸或窗口大小变化时重新测量分页尺寸；每次测量都从当前 DOM 读取第一张卡片，避免翻页卸载旧卡片后沿用失效尺寸而意外退回小页。
...
- 只做分页但取消视口门控：当前页仍可能一次性启动整页串行 3D 任务，首屏会继续等待复杂模型；分页和视口门控必须同时保留。
- 把 256px 继续压小：只能减少 data URL 和解码成本，无法消除 GLB、贴图和材质解析成本。
- 翻页/筛选时只清理观察器和广播订阅、不释放未开始队列项：旧页任务会继续占用单工作室，导致新页等待。
- 在测量函数外捕获第一张卡片：翻页后旧卡片已经卸载，读取到 0 高度会让自适应页大小退回默认值，宽屏网格又会只显示两三行。

## Verification

- 源码与行为测试锁定自适应整行分页、不可测量时的 24 条回退、页码边界、筛选重置、`IntersectionObserver`、`rootMargin: "320px 0px"`、离页队列释放和合并缓存写入。
