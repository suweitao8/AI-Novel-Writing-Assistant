...
- 动画入口默认用途范围为“全部”。当前导入流程用精简验收清单重建活动库：保留 46 条 UAL2 内置基础动作与 Anim57 `/Game/Characters/Mannequins/Anims/Unarmed/Attack` 下通过门禁的 4 条攻击动作；其它历史 Cine57 条目从活动 GLB/前端目录移除，源导出、中间产物与旧发布包保存在外部 `Cine57-exported/runs/<run-id>/` 备份中，后续按独立清单逐批恢复。Cine57 每条 clip 必须显式声明 `motionMode`：`in-place` 继续通过 root 全局位移不超过 0.03 米门禁，`root-motion` 则必须保留可观察的 root translation 轨道，不能在重定向时静默清零。用途范围切换时保留来源选择，只把动作分类恢复为“全部”，让来源和用途可以继续组合筛选。
