...
- **静态目录 + 可见性覆盖，不做用户资产 CRUD**：模型清单是 `client/src/config/modelLibrary.ts` 里的纯数据数组（当前 446 个条目：445 个精选静态模型与 1 个角色模型、17 类）；模型文件放 `client/public/models/` 由前端静态服务。模型库是"策展型"资产（由开发流程提取入库），不是用户上传型资产。用户在详情页执行的删除只写入按模型 ID 独立保存的隐藏状态，目录读取服务端可见性覆盖后再展示；模型文件、材质、缩略图和已有分镜引用都不删除。若将来需要用户上传，再单独引入服务端资产存储与生命周期接口，不把用户操作混入静态目录生成流程。
- **数量决策（2026-08-29、2026-08-31、2026-09-01 批A）**：曾一次性扩到 509 个，因质量参差先回退到 44 个，再按前景交互边界收敛；当前按可追踪白名单发布 445 个静态前景模型，另有 1 个非静态角色资源供动画/分镜使用但不计入静态前景目录展示。另有 39 个已经确认不适合前景的候选保留在可恢复隔离区并写入导入历史，不再进入模型卡片。batch3 池子的可策展净余已全部消化——重跑选择脚本只能得到零件/灰模/超尺寸/重复变体，后续扩量必须回到 UE 做定向导出（建筑构件、门窗、窗帘等缺口）。格式确认用 **GLB**（浏览器通用标准；FBX 浏览器不能直接加载，管线本来就 UE→FBX→GLB）。
...
- **前景准入与历史台账**：`scripts/models/model-library-selection.json` 的 `foregroundAdmission` 是模型进入前景库的统一规则：世界空间最大轴必须在 `0.1m` 到 `5m`（含边界）之间；碎屑堆、单片草丛、缺少真实颜色贴图等已确认不可用的资产，只能按显式 ID/Mesh 记录拒绝，不能用宽泛关键词误杀。`scripts/models/model-library-import-history.json` 为每个源模型保存规范化包路径 + Mesh 资产键、源行 SHA-256、最新状态、失败阶段、原因、证据和事件序列。源指纹未变化的 rejected 记录在转换前返回 `previously-rejected` 并跳过；源指纹变化或人工重开审查才重新进入流程。当前台账覆盖 445 个 approved 和 39 个 rejected 结论。
- **暂存、预览、发布边界**：`scripts/models/modelLibraryImportWorkflow.mjs --preflight` 先读取 manifest 和默认历史台账，输出 candidates/rejected/skipped 计划，不触碰正式目录；转换器只负责产生候选暂存产物。每个候选必须在 `/models/<id>` 真实详情页生成方形截图，并通过当前 GLB/贴图指纹、几何、材质、请求、控制台和视觉审核检查；`--check-staged` 或 `curate-cine57-library.mjs --check` 失败时不得发布。外部 `build-library-v3.cjs` 不是发布入口，避免未经预览的候选覆盖正式目录。
...
- **真实三维预览证据是发布前硬门禁**：新增或替换的资产必须先在产品模型详情页按统一环境和相机生成预览，再进入发布目录。复核记录使用 `model-preview-audit-YYYY-MM-DD`，绑定 `/models/<id>`、GLB 与所有目录贴图的 SHA-256、渲染器标识、日期和 `textureStatus`；`modelLibraryVisualReview.mjs` 会拒绝缺失/伪造/过期证据，资源哈希变化后不能复用旧截图。灌木、花、树等透明材质资产还必须确认叶片边缘和透明背景没有不透明大块。
...
- **分类与容器配额**：页面继续使用平面分类页签；自然资产固定细分为「石头 / 灌木 / 树木 / 花 / 盆栽」，小摆件、雕像、奖杯和花瓶归入「玩具/装饰品」，箱子、纸盒、木桶和篮筐归入「容器与箱子」。食材/纸箱同族最多发布两个代表模型，当前保留一组食材纸箱和一个纸盒；后续扩容必须先更新策略并通过质量门禁。
- **透明贴图导入故障的诊断与恢复**：源 RGBA 贴图的 alpha 探测、最终 PNG/opacity 映射和真实详情页预览必须作为一条可重复验证的链路；构建器的 execFile stdout 解析失败、FFmpeg YMIN 格式变化和 GLB OPAQUE 占位材质都不能被当作“不透明”结论。旧有损输出只能按逐文件 SHA-256 清单移到仓库外可恢复隔离目录，不能由详情页运行时裁黑块或静默隐藏。完整诊断顺序见 docs/wiki/debugging/model-preview-alpha-and-import-gate.md。
- **透明镂空材质统一双面渲染**：目录材质存在非空 `opacity` 映射时，共享材质入口将其设为 `pc.CULLFACE_NONE`，保证草、花、树叶从正反两面完整显示；普通无透明贴图材质继续使用 `pc.CULLFACE_BACK`。详情页、模型缩略图和分镜前景模型都必须复用这条规则，反向视角属于透明植被的预览验收内容。
...
- **材质回填（modelMaterials.ts）**：目录 `materials` 字段按「UE 材质资产名 → 贴图/颜色/标量」声明真实外观，运行时按材质名匹配（忽略大小写与符号）回填。带贴图参数的槽位回填 baseColor/normal/rma；**纯材质图槽位**（UE 里无贴图参数的玻璃/铬金属/墙漆，共 106 个槽）从 introspection 合并出 tint/metallic/roughness/opacityValue/emissive，复合材质图不可解时兜底中性灰。`MESH_OPACITY` 表可按 mesh 名强制半透明（当前为空：白壳是碰撞体，不是玻璃）。
 - **GLB 材质绑定与占位图是同一条发布门禁**：只要 GLB 材质声明 `baseColorTexture`，就必须在目录 `materials` 中按规范化材质名提供回填；内嵌 1×1 图片按导出占位图处理，不能把“文件可加载”当成“材质可用”。解析结果没有真实贴图/颜色且无法补回填的候选，必须记录到 `foregroundAdmission.rejectedAssets` 或 `quarantinedAssets` 精确清单并从发布目录剔除；隔离文件保留以便补齐材质后恢复。模型详情预览只消费通过该门禁的目录条目，视觉审核不能替代材质检查。
 - **缺失颜色贴图的灰模诊断**：FBX2GLTF 生成的内嵌 `1×1` Base Color 是占位证据，不等同于真实颜色来源。统一准入返回 `texture/missing-base-color-texture`；有同名真实 `baseColor` 目录绑定的材质继续接受贴图路径和预览检查，没有绑定的模型不进入目录。完整诊断路径见 `docs/wiki/debugging/model-import-texture-appearance-gate.md`。
 - **角色资源与模型展示边界**：模型目录可以保留动画库已有的 UAL2 角色 GLB 作为内部资源，但它不是 Cine57 静态道具入库清单的一部分，也不在模型库页面提供角色卡片或 3D 预览；分镜、动画预览和动画缩略图继续复用同一套蓝色代理材质，并以主体色 + 同色系浅色关节和下颚线区分动作结构。正式角色纹理必须换成带有效 UV 的角色资产，再接入真实纹理生成/保存链路。
  - **UE5 原生角色 profile 是分镜与动画的共同骨架边界（2026-09-03）**：Anim57 的徒手攻击从 `/Game/Characters/Mannequins/Anims/Unarmed/Attack` 直接通过 UE5 GLTFExporter 导出到 `UE5_Manny_Animations.glb` 与 `UE5_Quinn_Animations.glb`；两套资源都绑定同一个 `SK_Mannequin`，不经过跨骨架重定向，因此 3D 草图和动画预览不会因为重定向轴向或 IK 产生姿势漂移。两套文件包含相同的 4 条攻击轨道，并额外内置一条不进入动画目录的 `standing` 基础姿势，供 3D 草图初始化时直接显示站立状态。`resolveCharacterModelProfile` 只消费结构化的 `gender`、`actorKind`、`bodyBuild`：女性或纤细体型选择 Quinn，男性、壮体型和未知值选择 Manny；怪物/其他生物同样按体型选择，分镜可用显式覆盖固定某一套 profile。所有 UE 导出、组装、校验和备份都落在单次 `Cine57-exported/runs/<run-id>/` 目录中，不能在 Unreal 工程目录散落临时目录。
...
- **环境与缩略图规则**：模型编辑器、HDRI 预览、模型缩略图和动画缩略图都通过统一运行时创建可见穹顶与 `scene.envAtlas`；模型和动画详情、卡片都使用中央广场 `model-preview` 光照，模型/动画缩略图实例必须开启 `castShadows`，运行时默认创建 shadow catcher。卡片最终图只保留 HDRI、模型/角色和真实投影阴影，不绘制编辑器网格；详情编辑器仍可显示网格辅助线。模型缩略图缓存键为 `model-library:thumbnails:v28`，动画缩略图键为 `animation-library:thumbnails:v21`，改动环境、投影、材质或动画资源逻辑必须升版本；模型取景优先按 GLB 实际顶点的屏幕投影拟合并回正投影中心，AABB 仅作为安全回退；动画缩略图工作室在队列开始时加载一次统一 GLB，逐条实例化角色，不能为每张卡片重复解析同一文件。
...
- **动画库是独立一级页面（/animations），不寄生在模型页里**：顶部导航在「模型」与「系统」之间提供「动画」入口；入口页保留模型库同构的分类页签 + 卡片网格，点击卡片进入 `/animations/:animationId` 完整 3D 预览页，不在入口页打开弹窗。动画清单是 `client/src/config/animationLibrary.ts`，GLB 放 `client/public/anims/`；当前活动目录由 46 条 UAL2 兼容内置动作和 4 条 UE5 原生 Anim57 徒手攻击组成，目录条目用 `fileUrl + clipName` 指向对应 profile 的动画轨道。入口页搜索匹配动画名称、片段名、套装、动作类型和目录标识，并与来源组、套装、动作类型筛选取交集；同一骨架的新动作优先追加到对应 profile 资源包，而不是重新引入重定向链路。
...
- **分镜姿势必须以实际 profile 片段为准**：分镜运行时从当前 Manny/Quinn profile 的 `resource.animations` 计算可用姿势，姿势选择器不展示没有对应片段的旧选项；历史布局若保存了当前 profile 未提供的蹲伏、跪姿、趴姿或奔跑等姿势，加载时统一安全回退到站立，不得把不同语义的动作冒充成目标姿势。
- **动画缩略图与模型库同一套离屏生成方案**：`pages/animations/animationThumbnailStudio.ts` 复用模型缩略图的「离屏画布 + localStorage 缓存（`animation-library:thumbnails:v21`）+ 队列闲置销毁」结构，创建工作室时先启动并初始化 PlayCanvas、再按目录条目的 `fileUrl` 加载对应 profile GLB 与 HDRI，之后逐个把 `clipName` 装配到开启投影阴影的独立角色实例，读取轨道真实帧率并将 `activeStateCurrentTime` 定位到最后一帧的 50% 后暂停抓 JPEG——卡片的预览图反映动作中点姿态，且与详情页默认帧、光照 profile 一致。卡片最终图只保留 HDRI、角色和 shadow catcher 的真实投影阴影，不绘制编辑器网格；详情页仍可显示网格辅助线。动画预览和分镜草图共用同一个蓝色代理材质，主体与 `M_Joints` 关节槽使用同色系浅色区分动作结构；动作评估依赖应用帧循环，所以画布必须 `app.start()`（`autoRender=false` 只关自动出图，离屏 RAF 随后取消、抓图前用 update 手动推进）；新增动画无需手工出图，进目录即自动生成缩略图；资源、材质或生成逻辑变化时必须升缓存版本。
...
- **历史 UAL2 兼容动画入库管线**：旧的跨骨架动作仍遵循 UE 动画序列 → `AnimSequenceExporterFBX` 导出 FBX → FBX2glTF 转 GLB → `scripts/animation/retarget_ual2.py` 离线重定向到 UAL2 骨架 → 链式合并进兼容 GLB；源片段必须是绝对姿态，加法层、分层轨道和未烘焙的控制器结果要在 UE 导出前烘焙。Anim57 徒手攻击不走这条历史链路，而是按上面的 UE5 原生 profile 规则直接导出。
  - 重定向先把源动画相对源绑定姿态的世界旋转增量应用到 UAL2 `Idle_No_Loop` 的固定 40% 站立基准：`W_t(b) := W_s(b) · inv(W_s0(b)) · W_t_standing_base(b)`，再按目标父节点解局部四元数；由于跨骨架绑定姿态的局部骨骼轴可能不同，随后必须逐帧将躯干、颈部、锁骨、上臂、前臂和腿的源子骨方向对齐到目标解剖骨段。Anim57 源骨架若含额外的 `spine_04/spine_05/neck_02`，必须把目标 `spine_01/02/03/neck_01` 分别折叠映射到源 `spine_03/04/05/neck_02`，不能按同名编号硬匹配，也不能用通用胸腔瞄准补偿缺失节点；移动动作不无条件套末端 IK。根/骨盆平移使用目标站立基准加源绑定姿态相对增量 `T_t := T_t_standing_base + s · (T_s - T_s0)`；目标侧只从 `skins[].joints` 建立骨骼映射，避免把 `Mannequin` 网格包装节点当作骨骼。
...
- **策展剪枝必须和发布 GLB 同步（2026-09-01）**：导入动画曾一次性上架 277 条，出现明显穿模（如向后慢跑），历史批量清单和旧发布包需要保留在可恢复备份中；本次四条徒手攻击流程采用独立的精简验收清单。无论是用 `published: false` 暂时下架，还是替换为精简清单，都必须同步生成并通过与清单严格一致的 GLB 门禁，不能只改前端目录而继续引用旧 GLB。恢复历史动作时，先从运行备份恢复对应清单和产物，再重新执行门禁，不直接删除唯一副本。
...
- 动画入口默认用途范围为“全部”。当前导入流程用精简验收清单重建活动库：保留 46 条 UAL2 内置基础动作与 Anim57 `/Game/Characters/Mannequins/Anims/Unarmed/Attack` 下通过门禁的 4 条 UE5 原生攻击动作；46 条内置动作仍在 UAL2 兼容包中，4 条攻击动作发布在 Manny profile 包中，Quinn profile 包保持同一套原生轨道。其它历史 Cine57 条目从活动目录移除，源导出、中间产物与旧发布包保存在外部 `Cine57-exported/runs/<run-id>/` 备份中，后续按独立清单逐批恢复。保留的 root-motion clip 必须有可观察的 root translation 轨道，不能在导入时静默清零；原地动作仍通过 root 位移门禁。用途范围切换时保留来源选择，只把动作分类恢复为“全部”，让来源和用途可以继续组合筛选。
- 分镜动作解析按策选目录 ID 维护片段映射：原生 UE5 条目固定使用对应 profile 的 `fileUrl + clipName`，历史 UAL2 条目才回退到兼容包中的旧片段名；不得通过猜测字符串生成别名。稳定的条目 ID、GLB 片段名和 profile 路由必须保留，以兼容已有分镜布局。
...
- 策选阶段只接受真实 Asset Registry 路径、`AnimSequence` 和可匹配的 Mannequin 骨架；UE5 原生 profile 必须绑定 `SK_Mannequin`，机器人骨架、GhostSamurai 专用骨架和无法加载的资产不得混入标准角色动作目录。
- 活动条目不再强制合并进单一 GLB：46 条历史内置动作继续使用 `/anims/cine57/UAL2_UE_Anims.glb`，4 条 Anim57 徒手攻击使用 Manny/Quinn 原生 profile 包；分镜草图和动画预览按 profile 共享对应的蓝色角色与动作文件。
...
- 发布门禁同时检查动作语义（待机手臂下垂、行走双脚有轨迹、代表慢跑的手臂可达、坐姿骨盆不跳离角色）与 GLB 结构（旋转为 VEC4 单位四元数、平移为 VEC3、通道目标属于 skin joints）。Cine57 片段按清单中的 `motionMode` 检查：`in-place` 的 root 全局位移范围和首尾净位移均须 `<= 0.03m`；`root-motion` 必须保留 root translation 轨道并且有可观察位移。骨盆局部运动始终保留，不能用骨盆轨道代替 root 语义。
- 修改重定向、骨架别名、IK 或导出/合并逻辑后，必须通过单次运行目录执行可追溯导入：`D:/UnrealWorkspace/Cine57-exported/runs/<run-id>/` 统一保存 FBX、GLB、重定向中间文件、日志、备份和 `run-manifest.json`，不能把临时目录或 `.uproject` 备份写入 UE 工程目录。四条徒手攻击验收使用 `run_animation_import.cjs`，只有同一源工程、同一源骨架、四条资源都导出成功且最终 GLB 门禁通过后，才替换仓库中的统一发布文件；失败时保留失败运行目录和原发布 GLB 备份，禁止用旧中间产物拼接发布包。
...
- **跨骨架重定向必须按解剖骨段对齐**（2026-09-02 向前奔跑与 2026-09-03 徒手攻击案例）：源 UE Mannequin 与 UAL2 的核心骨骼名称和父子层级可以相同，但绑定姿态的局部骨骼轴不同；只套 `W_source_animation * inverse(W_source_bind) * W_target_standing_base`，再叠加通用胸腔瞄准或无条件末端 IK，会让 `spine_03→neck_01`、颈部、肩链和腿链逐渐偏离源动作，表现为低头、肩膀不对称、攻击方向侧扭和脚步变形。重定向器先用世界空间姿态差建立初始旋转，再逐帧把躯干、颈部、锁骨、上臂、前臂和腿的源子骨方向对齐到目标解剖骨段；当 Anim57 源骨架含 `spine_04/spine_05/neck_02` 而 UAL2 没有这些节点时，必须按链条折叠为目标 `spine_01/02/03/neck_01` 对应源 `spine_03/04/05/neck_02`，不能按同名编号硬匹配。末端 IK 只在源双手互相接近（最小腕间距不超过 `0.15m`）时两侧同时启用，或在单侧手腕接近头部（距离不超过 `0.20m`）时只启用接触侧；手-头接触帧由专用接触门禁校验，不能用锁骨到手的普通伸展距离误判。普通移动帧和腿链不受手部接触影响；`RETARGET_USE_LIMB_IK=1` 可对特殊动作显式强制全帧双臂与双腿，`RETARGET_NO_ARM_IK=1` 可关闭。
- **动画接地门禁必须检查蒙皮几何**：脚关节世界高度为正并不能证明可见脚底没有穿地；重定向后的发布 GLB 要按实际 `POSITION + JOINTS_0 + WEIGHTS_0 + inverseBindMatrices` 计算所有选中片段的蒙皮顶点最低点，再根据最坏片段的负值加有限 root ground lift 和安全余量。四条徒手攻击的回归测试必须覆盖整段帧范围，且最低点不得低于地面；root lift 只能写入 `root` 平移，不能通过篡改 `pelvis` 平移掩盖接地问题。
