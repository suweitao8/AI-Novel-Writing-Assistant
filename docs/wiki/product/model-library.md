...
- **静态目录 + 可见性覆盖，不做用户资产 CRUD**：模型清单是 `client/src/config/modelLibrary.ts` 里的纯数据数组（当前 463 个条目：462 个精选静态模型与 1 个角色模型、17 类）；模型文件放 `client/public/models/` 由前端静态服务。模型库是"策展型"资产（由开发流程提取入库），不是用户上传型资产。用户在详情页执行的删除只写入按模型 ID 独立保存的隐藏状态，目录读取服务端可见性覆盖后再展示；模型文件、材质、缩略图和已有分镜引用都不删除。若将来需要用户上传，再单独引入服务端资产存储与生命周期接口，不把用户操作混入静态目录生成流程。
- **数量决策（2026-08-29、2026-08-31、2026-09-01 批A）**：曾一次性扩到 509 个，因质量参差先回退到 44 个，再按前景交互边界收敛；当前按可追踪白名单发布 462 个静态前景模型，另有 1 个非静态角色资源供动画/分镜使用但不计入静态前景目录展示。另有 22 个已经确认不适合前景的候选保留在可恢复隔离区并写入导入历史，不再进入模型卡片。batch3 池子的可策展净余已全部消化——重跑选择脚本只能得到零件/灰模/超尺寸/重复变体，后续扩量必须回到 UE 做定向导出（建筑构件、门窗、窗帘等缺口）。格式确认用 **GLB**（浏览器通用标准；FBX 浏览器不能直接加载，管线本来就 UE→FBX→GLB）。
...
- **前景准入与历史台账**：`scripts/models/model-library-selection.json` 的 `foregroundAdmission` 是模型进入前景库的统一规则：世界空间最大轴必须在 `0.1m` 到 `5m`（含边界）之间；碎屑堆、单片草丛等地面散布物，以及已确认预览异常的资产，只能按显式 ID/Mesh 记录拒绝，不能用宽泛关键词误杀。`scripts/models/model-library-import-history.json` 为每个源模型保存规范化包路径 + Mesh 资产键、源行 SHA-256、最新状态、失败阶段、原因、证据和事件序列。源指纹未变化的 rejected 记录在转换前返回 `previously-rejected` 并跳过；源指纹变化或人工重开审查才重新进入流程。当前台账覆盖 462 个 approved 和 22 个 rejected 结论。
