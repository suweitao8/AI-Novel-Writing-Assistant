...
- **角色资源与模型展示边界**：角色动画资源不是 Cine57 静态道具入库清单的一部分，也不在模型库页面提供普通角色卡片或 3D 预览；分镜、动画预览和动画缩略图复用同一套蓝色代理材质，并以主体色 + 同色系浅色关节和下颚线区分动作结构。正式角色纹理必须换成带有效 UV 的角色资产，再接入真实纹理生成/保存链路。
