...
- **模型库分页按可用空间自适应**：搜索和分类筛选完成后，只把当前页卡片挂载到 DOM；每页根据当前网格列数、可用高度、卡片高度和行间距计算，只展示完整行，页大小始终是列数的整数倍并限制在最多 8 行。窗口、侧栏或响应式列数变化时重新计算，筛选结果变化回到第一页，分页仍与 320px 视口门控一起限制缩略图生成压力。
